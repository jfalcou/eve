...
.. code-block:: c++
   ...
   namespace eve
   {
      constexpr /* implementation defined */ atan2pi = {};
   }

Function object computing the quadrant aware `atan2pi` function.
...
* [1] Computes element-wise ``atan(v/w)`` in :math:`\pi` multiples with quadrant awareness.
* [2,3] Computes element-wise  ``atan(v/s)`` in :math:`\pi` multiples with quadrant awareness.
* [4] Computes  ``atan(t/s)`` in :math:`\pi` multiples with quadrant awareness.
...
*******
    ...
    - For any real arguments  ``x`` and  ``y`` not both equal to zero, ``atan2pi(y, x)``
    ...
    - Following IEEE norms,  we should have:
      ...
      -  If y is :math:`\pm0` and x is negative or :math:`-0`, :math:`\pm1` is returned
      ...
      -  If y is :math:`\pm\infty` and x is :math:`-\infty`, :math:`\pm0.75` is returned
      -  If y is :math:`\pm\infty` and x is :math:`+\infty`, :math:`\pm0.5` is returned
      -  If x is :math:`\pm0` and y is negative, :math:`-90` is returned
      -  If x is :math:`\pm0` and y is positive, :math:`+90`  is returned
      -  If x is :math:`-\infty` and y is finite and positive, :math:`+1` is returned
      -  If x is :math:`-\infty` and y is finite and negative, :math:`-1` is returned
     ...
     The ``:ref:`pedantic_ <feature-decorator>``` decorator (call to ``pedantic_(eve::atan2pi)(y, x)`` )
...
.. include:: ../../../../test/doc/core/atan2pi.cpp
  :literal:

Possible output:

.. include:: ../../../../test/doc/core/atan2pi.txt
  :literal:
