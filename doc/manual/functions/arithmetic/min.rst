...
*******
    ...
    * with :ref:`pedantic_ <feature-decorator>`, in which case the call is equivalent to ``if_else(x < y, x, y)``;

    *  with :ref:`numeric__ <feature-decorator>`, in which case if an element of ``x`` or ``y`` is a nan the corresponding result is the other value.
