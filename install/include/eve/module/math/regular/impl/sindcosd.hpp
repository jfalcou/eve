...
namespace eve::detail
{
 template<floating_real_value T>
  EVE_FORCEINLINE constexpr auto sindcosd_(EVE_SUPPORTS(cpu_), quarter_circle_type const &, T x) noexcept
  {
    if constexpr( has_native_abi_v<T> )
    {
      return eve::quarter_circle(sinpicospi)(div_180(x));
    }
    else
      return apply_over2(quarter_circle(sindcosd), x);
  }
  ...
  EVE_FORCEINLINE constexpr auto sindcosd_(EVE_SUPPORTS(cpu_), T const &a0) noexcept
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      if( eve::all(eve::abs(x) <= T(45)) )
        return quarter_circle(sindcosd)(a0);
      else
        return big(sindcosd)(a0);
    }
    else
      return apply_over2(sindcosd, a0);
  }
