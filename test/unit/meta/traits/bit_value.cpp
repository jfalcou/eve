...
template<typename T>
constexpr eve::bit_value_t<T,eve::wide<float, 4>> g(T) { return {}; }
...
TTS_CASE("SIMD bit_value")
{
  ...
  [[maybe_unused]] eve::wide<double, 4> x = {};
...
TTS_CASE("Mixed scalar/SIMD bit_value")
{
  ...
  TTS_TYPE_IS((eve::bit_value_t<double, eve::wide<float, 2>
                                      , eve::wide<int, 2>
                                      , std::int64_t
                                      , eve::wide<char, 8>
                                      >)
  ...
  TTS_EXPR_IS(g(x), (eve::wide<double, 2>) );
  TTS_EXPECT_COMPILES(x, { g(x); });

  [[maybe_unused]] eve::wide<std::int32_t, 4>  y = {};
  TTS_EXPR_IS(f(y), (eve::wide<std::int32_t, 4>));
