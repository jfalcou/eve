...
<typename T>(tts::type<T>)
{
  ...
  TTS_EQUAL ( sizeof(eve::wide<udt::label_position>)
            , sizeof(kumi::tuple<eve::wide<float,d_v>,eve::wide<std::uint8_t,d_v>>)
            , REQUIRED
            );
};
...
<typename T>(tts::type<T>)
{
  ...
  eve::wide<kumi::tuple<std::uint8_t,float>, w_t::size()> revert(gen_i8,gen_f);

  eve::wide<kumi::tuple<std::uint8_t,float,std::uint8_t>, w_t::size()>
...
TTS_CASE( "Check eve::wide<udt> slice behavior")
{
  ...
  {
    constexpr int s = w_t::size() / 2;

    eve::wide<udt::grid2d>    vp  = [](int i, int c) { return udt::grid2d{  i, c  -i-1}; };
    eve::wide<udt::grid2d, s> vl  = [](int i, int c) { return udt::grid2d{  i, c+s-i-1}; };
    eve::wide<udt::grid2d, s> vh  = [](int i, int  ) { return udt::grid2d{s+i,   s-i-1}; };
