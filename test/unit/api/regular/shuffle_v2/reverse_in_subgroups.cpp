...
all_subgroups()
{
  ...
  if constexpr( eve::current_api == eve::sse2 )
  {
    ...
    { return kumi::tuple {eve::lanes<1 << i>...}; }(std::make_index_sequence<ssz> {});
  }
  else
  {
    // removing some cases to reduce compile times
    if constexpr( ssz == 1 ) { return kumi::tuple {eve::lanes<1>}; }
    else if constexpr( ssz < 3 ) { return kumi::tuple {eve::lanes<2>}; }
    else
    {
      // We can go 0 to ssz but we want to remove:
      // i = 0, i = 1, i = T::size()
      return []<std::size_t... i>(std::index_sequence<i...>)
      { return kumi::tuple {eve::lanes<1 << (i + 2)>...}; }(std::make_index_sequence<ssz - 3> {});
    }
  }
}
