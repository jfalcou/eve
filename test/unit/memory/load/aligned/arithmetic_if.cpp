...
//==================================================================================================
TTS_CASE_TPL( "Check conditional load to wides from aligned pointer", eve::test::simd::all_types)
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr(T::size() == eve::expected_cardinal_v<v_t>)
  {
    ...
    auto loaded = eve::load[eve::unsafe2](ptr);
    TTS_EXPECT(eve::any(loaded == x));

    loaded = eve::load[eve::unsafe2](ptr.get());
    TTS_EXPECT(eve::any(loaded == x));

    for (auto ignore_ptr = ptr.get(); (&x - ignore_ptr) < T::size(); --ignore_ptr)
    {
      loaded = eve::load[eve::unsafe2][eve::ignore_first(ptr.get() - ignore_ptr)](ignore_ptr);
      TTS_EXPECT(eve::any(loaded == x));
    }
  }
  ...
  [&]<std::ptrdiff_t...N>( std::integer_sequence<std::ptrdiff_t,N...> )
  {
    auto test = [&]<std::ptrdiff_t A>(eve::fixed<A> n)
    {
      ...
      auto loaded = eve::load[eve::unsafe2](ptr, n);

      TTS_EXPECT(eve::any(loaded == x));

      for (auto ignore_ptr = ptr.get(); (&x - ignore_ptr) < n(); --ignore_ptr)
      {
        loaded = eve::load[eve::unsafe2][eve::ignore_first(ptr.get() - ignore_ptr)](ignore_ptr, n);
        TTS_EXPECT(eve::any(loaded == x));
      }
    };
