...
<typename T>(T a0, T a1)
{
  ...
  TTS_ABSOLUTE_EQUAL(eve::airy_bi(v_t(-1500)), boost::math::airy_bi(v_t(-1500)), abstol);
  TTS_ABSOLUTE_EQUAL(eve::airy_bi(v_t(-500)), boost::math::airy_bi(v_t(-500)), abstol);
  TTS_ULP_EQUAL(eve::airy_bi(v_t(0)), v_t(0.614926627446001), 1.5);

  TTS_ABSOLUTE_EQUAL(eve::airy_bi(T(-1500)), T(boost::math::airy_bi(v_t(-1500))), abstol);
  TTS_ABSOLUTE_EQUAL(eve::airy_bi(T(-500)), T(boost::math::airy_bi(v_t(-500))), abstol);
  TTS_ULP_EQUAL(eve::airy_bi(T(0)), T(0.614926627446001), 0.5);

  TTS_ABSOLUTE_EQUAL(eve::airy_bi(a0), map(std__airy_bi, a0), abstol);
  TTS_RELATIVE_EQUAL(eve::airy_bi(a1), map(std__airy_bi, a1), 0.001);
};
