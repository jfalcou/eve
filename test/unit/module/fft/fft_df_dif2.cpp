...
<typename T>(tts::type<T>)
{
  ...
  {
    auto pr = [](auto name, auto v, auto maxi){
      size_t m = v.size() > size_t(maxi) ? maxi : v.size();
      std::cout << name << " = (";
    ...
    pr("a apres ifft", a, 16);

    for(int i=0; i < ssize(a) ; ++i) TTS_ULP_EQUAL(a[i], orig[i], 20000);
  }
};
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr(std::same_as<eve::element_type_t<T>, float>&& eve::cardinal_v<T> == 1)
  {
    auto pr = [](auto name, auto v, auto maxi){
      size_t m = v.size() > size_t(maxi) ? maxi : v.size();
      std::cout << name << " = (";
      for(size_t i=0; i < m; ++i) std::cout << v.get(i) << " ";
      std::cout << ")\n";
    };
    size_t N = 1 << 4;
    std::cout << tts::typename_<T> << std::endl; ;
    ...
    for(int i=0; i < ssize(a) ; ++i) TTS_ULP_EQUAL(a.get(i), orig.get(i), 20000);
