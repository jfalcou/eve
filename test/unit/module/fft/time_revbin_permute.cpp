...
void timeit(std::string const & title, F const & rvbp, D const &, T , int max = 16)
{
...
TTS_CASE_TPL("Check duration for revbin_permute", eve::test::simd::ieee_reals)
<typename T>(tts::type<T>)
{
  if(eve::cardinal_v<T> == 1)
  {
    using e_t = eve::element_type_t<T>;
    timeit("eve::aos(eve::revbin_permute)", eve::aos(eve::revbin_permute), eve::aos_type{}, e_t(), 16);
    using e_t = eve::element_type_t<T>;
    timeit("eve::soa(eve::revbin_permute)", eve::soa(eve::revbin_permute), eve::soa_type{}, e_t(), 16);
    std::cout << "==========================================================================================================================" << std::endl;
  }
  TTS_EQUAL(0, 0);
  return 0;
