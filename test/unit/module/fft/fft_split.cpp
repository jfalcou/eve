...
TTS_CASE_TPL("Check fft_split on aos", eve::test::simd::ieee_reals)
...
//    auto tic =  std::chrono::steady_clock::now();
    eve::aos(eve::fft_split)(a, e_t(1.0));
...
TTS_CASE_TPL("Check fft_split on soa", eve::test::simd::ieee_reals)
<typename T>(tts::type<T>)
{
  using namespace std::literals;
  if constexpr(std::same_as<eve::element_type_t<T>, float>&& eve::cardinal_v<T> == 1)
  {
    ...
    pr("a avant", a);
    auto tic =  std::chrono::steady_clock::now();
    eve::soa(eve::fft_split)(a, e_t(1.0));
    auto toc =  std::chrono::steady_clock::now();
    std::cout << "duration = " << (toc-tic) /1ms << "ms\n" << std::endl;
    pr("a apres", a);
//    eve::soa(eve::ifft)(eve::soa(eve::fft_dit4), a, e_t(1)/N);
//     pr("a apres ifft", a);
    eve::soa(eve::fft_dit4)(orig, e_t(1.0));
    pr("o apres", orig);

    for(int i=0; i < ssize(a) ; ++i) TTS_ULP_EQUAL(a.get(i), orig.get(i), 100000);
  }
};
