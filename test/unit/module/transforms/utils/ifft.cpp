...
//     eve::naive_ft(a, fac, e_t(1));  //inverse ft with s = 1
//     eve::ifft(eve::naive_ft, b, fac);  //inverse ft with swapping
...
TTS_CASE_TPL("Check ifft with soa complex range", eve::test::simd::ieee_reals)
  <typename T>(tts::type<T>)
{
  if constexpr(std::same_as<eve::element_type_t<T>, double>&& eve::cardinal_v<T> == 1)
  {
    ...
    using e_t = eve::element_type_t<T>;
    using c_t = eve::complex<e_t>;
    eve::algo::soa_vector<c_t> a(N), b(N), orig(N);
    auto fac = e_t(1);
    for(size_t i=0; i < N ; ++i) {
      a.set(i, c_t(i+1, i+2));
      orig.set(i, a.get(i));
    ...
    for(size_t i = 0; i < N; i++){
      TTS_ABSOLUTE_EQUAL(a.get(i), b.get(i), 0.001);
    }
  }
};
  ...
  <typename T>(tts::type<T>)
{
  if constexpr(std::same_as<eve::element_type_t<T>, double> && eve::cardinal_v<T> == 1)
  {
    ...
    auto fac = e_t(1);
    eve::naive_ft(ar, ai, fac, e_t(1));    //inverse ft with s = 1
    eve::ifft(eve::naive_ft, br, bi,fac);  //inverse ft with swapping
