...
//     eve::naive_ft(a, e_t(1.0), e_t(-1));
//     for(size_t i=0; i <N ; ++i){
//       TTS_ULP_EQUAL(          a[i],            ref[i] , 10000);
//     }
//     eve::naive_ft(a, e_t(1)/N, e_t(1)); //inverse fft
//     for(size_t i=0; i <N ; ++i){
...
<typename T>(tts::type<T>)
{
  if constexpr(std::same_as<eve::element_type_t<T>, double>&& eve::cardinal_v<T> == 1)
  {
    ...
    eve::naive_ft(a, e_t(1.0), e_t(-1.0) );
    eve::naive_ft(a, e_t(1)/N, e_t(1.0)  );//inverse fft
...
<typename T>(tts::type<T>)
{
  if constexpr(std::same_as<eve::element_type_t<T>, double> && eve::cardinal_v<T> == 1)
  {
    ...
    eve::naive_ft(ar, ai, e_t(1)/N, e_t(1)); //inverse fft
    for(size_t i=0; i <N ; ++i){
