...
TTS_CASE_WITH( "Check behavior of max on scalar"
...
TTS_CASE_WITH( "Check behavior of max on wide"
  ...
  <typename T>(T const& a0, T const& a1, T const & a2, T const & a3)
{
  ...
  auto amz = decltype(z1)(eve::detail::map(eve::max, z1, z2));
  TTS_EQUAL ( eve::max(z1, z2), amz);
};
