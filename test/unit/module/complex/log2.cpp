...
EVE_TEST_TYPES( "Check return types of eve::log2", eve::test::scalar::ieee_reals)
  <typename T>(eve::as<T>)
{
  using eve::as;
  ...
  auto scale = [](auto a,  auto b){
    return eve::complex<T>(a, b)/eve::log_2(as<e_t>());
  };
  ...
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::zero(as<T>()))), scale(eve::minf(as<T>()), 0), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::zero(as<T>()),  eve::zero(as<T>()))),  scale(eve::minf(as<T>()), eve::zero(as<T>())), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::minf (as<T>()), eve::inf(as<T>()))),  scale(eve::inf (as<T>()), 3*eve::pio_4(as<T>())), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::inf  (as<T>()), eve::inf(as<T>()))),  scale(eve::inf (as<T>()), eve::pi(as<T>())/4), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::minf (as<T>()), -eve::inf(as<T>()))),  scale(eve::inf (as<T>()), -3*eve::pio_4(as<T>())), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::inf  (as<T>()), -eve::inf(as<T>()))),  scale(eve::inf (as<T>()), -eve::pi(as<T>())/4), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::mzero(as<T>()))), scale(eve::minf(as<T>()), eve::pi(as<T>())), 0.5);
  ...
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::mone(as<T>()))), scale(eve::zero(as<T>()), eve::pi(as<T>())), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::one(as<T>()))), scale(eve::zero(as<T>()), 0), 0.5);
  TTS_ULP_EQUAL((eve::log2)(c_t(eve::zero(as<T>()), eve::one(as<T>()))),scale(eve::zero(as<T>()), eve::pio_2(as<T>())), 0.5);

  TTS_ULP_EQUAL(eve::log2(c_t(-4)), scale(eve::log(T(4)), eve::pi(as<T>())), 0.5);
  TTS_ULP_EQUAL(eve::log2(T(-4)), eve::nan(as<T>()), 0);
//TTS_ULP_EQUAL(eve::cmplx(eve::log2)(T(-4)), scale(eve::log2(T(4)), eve::pi(as<T>())), 0);
};
