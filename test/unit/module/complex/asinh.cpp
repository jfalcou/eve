  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_ULP_EQUAL(eve::asinh(eve::complex<e_t>(e, f)),  cv(std::asinh(c_t(e, f))), 3.5);
    }
  }
};
  ...
  <typename T>(eve::as<T>)
{
  ...
  // specific values tests
   ...
   TTS_ULP_EQUAL(eve::asinh(c_t(eve::nan  (as<T>()), eve::inf(as<T>()))),  c_t(eve::inf (as<T>()), eve::nan(as<T>())), 0.75);
