...
TTS_CASE_WITH( "Check behavior of erf on scalar"
...
<typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_ULP_EQUAL(eve::erf(eve::complex<e_t>(e, f)),  cv(std::erf(c_t(e, f))), 2.0);
      TTS_ULP_EQUAL(eve::erf(eve::complex<e_t>(e, f)),  cv(std::erf(c_t(e, f))), 2.0);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  using c_t = std::complex<e_t>;
  auto std_erf = [](auto x, auto y){return cv(std::erf(c_t(x, y))); };
  auto init_with_std = [std_erf](auto a0,  auto a1){
    z_t b;
    for(int i = 0; i !=  eve::cardinal_v<T>; ++i)
    {
      ce_t z = std_erf(a0.get(i), a1.get(i));
      b.set(i, z);
    }
    return b;
  };
  TTS_ULP_EQUAL(eve::erf(z_t{a0,a1}), init_with_std(a0, a1), 2);
};
...
<typename T>(tts::type<T>)
{
  ...
  TTS_ULP_EQUAL(eve::erf(c_t{one, zer}),   (c_t{e, zer}), 0.5);
  TTS_ULP_EQUAL(eve::erf(c_t{zer, zer}),   (c_t{one, zer}), 0.5);
};
