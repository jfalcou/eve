...
TTS_CASE_TPL( "Check corner cases of exp2", eve::test::scalar::ieee_reals)
  <typename T>(tts::type<T>)
{
  ...
  using eve::conj;
  using eve::exp2;
  auto test_exp2 = [](auto z){ return cv(std::exp(eve::log_2(eve::as<e_t>())*sc(z))); };
  for(int i=0; i < N; ++i)
  {
    TTS_IEEE_EQUAL(exp2(inputs[i]), test_exp2(inputs[i])) << "i " << i << " -> " << inputs[i] << "\n";
    TTS_IEEE_EQUAL(exp2(conj(inputs[i])), conj(exp2(inputs[i])));
  }
};
