...
TTS_CASE_TPL("Check eve::lognormal_distribution behavior", EVE_TYPE)
{
  auto inf  = eve::inf(eve::as<T>());

  auto no = eve::lognormal_distribution{T(1.0), T(2.0)};
...
TTS_CASE_TPL("Check eve::lognormal_distribution behavior", EVE_TYPE)
{
  ...
  auto no = eve::lognormal_distribution{eve::zero, T(2.0)};
...
TTS_CASE_TPL("Check eve::lognormal_distribution behavior", EVE_TYPE)
{
  ...
  auto no = eve::lognormal_distribution_01<T>;
  TTS_ULP_EQUAL(eve::median(no)               , T(0.0)  , 0);
  TTS_ULP_EQUAL(eve::mean(no)                 , T(0.0)  , 0);
