  ...
  <typename T, typename I>(T const& a0,I const & i0)
{
  ...
  for(unsigned int n=0; n < 5; ++n)
  {
    ...
    TTS_ULP_EQUAL(eve__legendrev(n, a0), T(boost_legendre), 15);
  }
  auto boost_legendrev =  [&](auto i, auto) { return boost::math::legendre_p(i0.get(i), a0.get(i)); };
  TTS_ULP_EQUAL(eve__legendrev(i0    , a0), T(boost_legendrev), 15);
  for(unsigned int j=0; j < eve::cardinal_v<T>; ++j)
  {
    auto boost_legendre2 =  [&](auto i, auto) { return boost::math::legendre_p(i0.get(i), a0.get(j)); };
    TTS_ULP_EQUAL(eve__legendrev(i0 , a0.get(j)), T(boost_legendre2), 15);
  }
  for(unsigned int j=0; j < eve::cardinal_v<T>; ++j)
  {
    for(unsigned int n=0; n < eve::cardinal_v<T>; ++n)
    {
      TTS_ULP_EQUAL(eve__legendrev(i0.get(j) , a0.get(n)), v_t(boost::math::legendre_p(i0.get(j), a0.get(n))), 15);
    }
  }
};
...
//     for(unsigned int n=0; n < eve::cardinal_v<T>; ++n)
//     {
//       TTS_ULP_EQUAL(eve__legendrev(i0.get(j) , a0.get(n)), v_t(boost::math::legendre_p_prime(i0.get(j), a0.get(n))), 15);
//     }
//  }
};
