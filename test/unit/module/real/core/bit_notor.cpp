  ...
  <typename T>(T const& a0, T const& a1)
{
  ...
  for(std::size_t i = 0;  i < a0.size(); ++i)
   TTS_IEEE_EQUAL( bit_notor(a0[i], a1[i]), bit_cast(~bit_cast(a0[i], as(i_t()))
                                                   | bit_cast(a1[i], as(i_t())), as(v_t())
                                                  )
                 );
};

EVE_TEST( "Check behavior of bit_notor on integers types"
  ...
  <typename T>(T const& a0, T const& a1)
{
  ...
  for(std::size_t i = 0;  i < a0.size(); ++i)
    TTS_IEEE_EQUAL( bit_notor(a0[i], a1[i]), v_t(~a0[i] | a1[i]));
};
