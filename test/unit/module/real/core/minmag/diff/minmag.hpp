...
TTS_CASE_TPL("Check diff(minmag) return type", EVE_TYPE)
{
  ...
  {
    TTS_EXPR_IS(eve::diff_<1>(eve::minmag)(T(), T()), T);
    TTS_EXPR_IS(eve::diff_<2>(eve::minmag)(T(), T()), T);
  }
  else
  {
...
TTS_CASE_TPL("Check eve::diff(eve::minmag) behavior", EVE_TYPE)
{
  if constexpr(eve::floating_value<T>)
  {
    ...
    TTS_EQUAL(eve::diff_<3>(eve::minmag)(T(1), T(2), T(3), T(4), T(5)),T(0));
    TTS_EQUAL(eve::diff_<6>(eve::minmag)(T(1), T(2), T(3), T(4), T(5)),T(0));
    TTS_EQUAL(eve::diff_<4>(eve::minmag)(v_t(1), T(3), T(0), T(0), T(5)),T(1));
  }
  else
  {
    TTS_PASS("Unsupported type");
