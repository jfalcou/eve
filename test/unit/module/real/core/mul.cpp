...
#include <eve/function/promoted/mul.hpp>
...
//   //promoted
//   using pv_t = eve::detail::upgrade_t<v_t>;
//   using pT = eve::wide<pv_t, eve::cardinal_t<T>>;
//   TTS_EXPR_IS( eve::promoted(eve::mul)(T(), T()  ) , pT);
//   TTS_EXPR_IS( eve::promoted(eve::mul)(T(), v_t()) , pT);
//   TTS_EXPR_IS( eve::promoted(eve::mul)(v_t(), T()) , pT);
//   TTS_EXPR_IS( eve::promoted(eve::mul)(v_t(), v_t()) , pv_t);
...
<typename T>( T const& a0, T const& a1 )
{
  if constexpr (sizeof(eve::element_type_t<T>) < 8& sizeof(eve::element_type_t<T>) !=  1 && eve::cardinal_v<T> !=  1)
  {
    ...
    using v_t =  eve::element_type_t<T>;
    using pv_t = eve::detail::upgrade_t<v_t>;
//     auto a = eve::promoted(mul)(a0, a1);
//     std::cout << a << std::endl;
//     TTS_EQUAL(0, 0);
//    TTS_EQUAL( eve::promoted(mul)(a0, a2), map([](auto e, auto f) { return mul(pv_t(e), pv_t(f)); }, a0, a2));
    TTS_EQUAL( eve::promoted(mul)(a0, a1), map([](auto e, auto f) { return mul(pv_t(e), pv_t(f)); }, a0, a1));
//   constexpr int S = 8;
//   eve::wide < uint32_t, eve::fixed<S>> a(eve::valmax(eve::as<uint32_t>()));
//   eve::wide < uint32_t, eve::fixed<S>> b{[](auto i, auto){return i; }};
//   std::cout << std::hex << " a "<< a << std::endl;
//   std::cout <<" b "<< b << std::endl;
//   auto z = eve::promoted(eve::mul)(a, b);
//   std::cout << " pa*pb "<< z << std::endl;
//   eve::wide < uint64_t, eve::fixed<S>> la(eve::valmax(eve::as<uint32_t>()));
//   eve::wide < uint64_t, eve::fixed<S>> lb{[](auto i, auto){return i; }};
