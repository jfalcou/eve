...
<typename T>(tts::type<T>)
{
  ...
  if constexpr( eve::floating_value<T> )
  {
    ...
    TTS_EXPR_IS(eve::ceil(T(), eve::as<uint32_t>()), (eve::as_integer_t<T, unsigned>));
    TTS_EXPR_IS(eve::ceil(v_t(), eve::as<uint32_t>()), (eve::as_integer_t<v_t, unsigned>));
...
<typename T>(T const& a0)
{
  ...
  if constexpr( eve::floating_value<T> )
  {
    ...
    TTS_EQUAL(eve::ceil(eve::abs(a0), eve::as<uint32_t>()),
