...
<typename T>(tts::type<T>)
{
  ...
  TTS_EXPR_IS(eve::ceil(T(), eve::as<signed>()), (eve::as_integer_t<T, signed>));
  TTS_EXPR_IS(eve::ceil(v_t(), eve::as<signed>()), (eve::as_integer_t<v_t, signed>));
...
// almost tests
//==================================================================================================
TTS_CASE_TPL("Check  with specific values", eve::test::simd::ieee_reals) //TO DO fp16 ?
<typename T>(tts::type<T>)
{
...
<typename T>(T const& a0)
{
  ...
  if constexpr( eve::floating_value<T> )
  {
    TTS_EQUAL(eve::ceil(a0), tts::map([&](auto e) -> v_t { return v_t(eve::ceil(e)); }, a0));
