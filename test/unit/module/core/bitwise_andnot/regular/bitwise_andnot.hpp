...
TTS_CASE("Check bitwise_andnot return type")
{
  ...
  TTS_EXPR_IS(eve::bitwise_andnot(si_t(), Type()), si_t);
}

TTS_CASE( "Check bitwise_andnot behavior")
{
  TTS_EQUAL(eve::bitwise_andnot(eve::Zero<Type>(), eve::One<Type>()), eve::Zero<Type>());
  TTS_EQUAL(eve::bitwise_andnot(eve::Zero<Type>(), eve::Zero<Type>()), eve::Zero<Type>());
  ...
  if constexpr(std::is_integral_v<Type>)
  {
    ...
    TTS_EQUAL(eve::bitwise_andnot(eve::One<si_t>(), eve::Zero<Type>()), eve::One<si_t>());
  }
}
