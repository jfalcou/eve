//==================================================================================================
/**
  EVE - Expressive Vector Engine
  Copyright : EVE Project Contributors
  SPDX-License-Identifier: BSL-1.0
**/
//==================================================================================================
#include "test.hpp"
#include "unit/module/core/logical_ops.hpp"

#include <eve/module/core.hpp>

//==================================================================================================
//== Types tests
//==================================================================================================
TTS_CASE_TPL("Check return types of eve::logical_ornot(scalar)", eve::test::scalar::all_types)
<typename T>(tts::type<T>)
{
  logical_test_scalar_return_type<T>(eve::logical_ornot);
};

TTS_CASE_TPL("Check return types of eve::logical_ornot(simd)", eve::test::simd::all_types)
<typename T>(tts::type<T>)
{
  logical_test_simd_return_type<T>(eve::logical_ornot);
};

//==================================================================================================
//== Tests for eve::logical_ornot
//==================================================================================================
TTS_CASE("Check behavior of eve::logical_ornot(bool)")
{
  TTS_EQUAL(eve::logical_ornot(true, true), true);
  TTS_EQUAL(eve::logical_ornot(true, false), true);
  TTS_EQUAL(eve::logical_ornot(false, true), false);
  TTS_EQUAL(eve::logical_ornot(false, false), true);
};

TTS_CASE_TPL("Check behavior of eve::logical_or(invalid)", tts::cartesian_square<eve::test::scalar::all_types>)
<typename T, typename U>(tts::type<kumi::tuple<T, U>>)
{
  logical_test_compile_reject<T, U>(eve::logical_ornot);
};

TTS_CASE_WITH("Check behavior of eve::logical_ornot(simd)",
              eve::test::simd::all_types,
              tts::generate(tts::logicals(0, 3), tts::logicals(1, 2), tts::randoms(0, 2)))
<typename M, typename T>(M const& l0, M const& l1, T const& a0)
{
  using l_t = eve::element_type_t<M>;

  TTS_EQUAL(eve::logical_ornot(l0, true), l0);
  TTS_EQUAL(eve::logical_ornot(true, l1), eve::true_(eve::as<M>()));
  TTS_EQUAL(eve::logical_ornot(false, l1), tts::map([](auto e) -> l_t { return !e; }, l1));
  TTS_EQUAL(eve::logical_ornot(l0, false), eve::true_(eve::as<M>()));

  logical_test_simd<M, T>(eve::logical_ornot, [](auto e, auto f) { return e || !f; }, l0, l1, a0);
};
