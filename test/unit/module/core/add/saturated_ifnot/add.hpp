...
TTS_CASE_TPL("Check add.not_ return type", EVE_TYPE)
{
  TTS_EXPR_IS( (eve::saturated_(eve::add.not_[ T()              ])(T(), T())), T);
  TTS_EXPR_IS( (eve::saturated_(eve::add.not_[ eve::logical<T>()])(T(), T())), T);
  TTS_EXPR_IS( (eve::saturated_(eve::add.not_[ true                    ])(T(), T())), T);
}

TTS_CASE_TPL("Check saturated conditional add.not_ behavior", EVE_TYPE)
{
  T tv{eve::Valmax<T>()};
  T fv{3};
  auto t = eve::True<T>();
