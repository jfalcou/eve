...
<typename M, typename T>(M const& l0, [[maybe_unused]] M const& l1, [[maybe_unused]] T const& a0)
{
  ...
  TTS_EQUAL(logical_and(l0, true), map([](auto e) -> l_t { return e; }, l0));
  TTS_EQUAL(logical_and(true, l1), map([](auto e) -> l_t { return e; }, l1));
  TTS_EQUAL(logical_and(l0, true), map([](auto e) -> l_t { return e; }, l0));
  TTS_EQUAL(logical_and(l0, false), eve::false_(eve::as<M>()));
  TTS_EQUAL(logical_and(l0, l1), map([](auto e, auto f) -> l_t { return e && f; }, l0, l1));
  TTS_EQUAL(logical_and(l0, l1.get(0)), map([&](auto e) -> l_t { return e && l1.get(0); }, l0));
  TTS_EQUAL(logical_and(l0.get(0), l1), map([&](auto f) -> l_t { return l0.get(0) && f; }, l1));

  using v_t  = element_type_t<T>;
  using d_t  = detail::downgrade_t<v_t>;
  auto da0   = convert(a0, eve::as<d_t>());
  using dl_t = as_logical_t<d_t>;
  TTS_EQUAL(logical_and(l1, da0 > 1), map([](auto e, auto f) -> l_t { return (f > 1) && e; }, l1, da0));
  TTS_EQUAL(logical_and(da0 > 1, l1), map([](auto e, auto f) -> dl_t { return bool((e > 1) && f); }, da0, l1));
};
