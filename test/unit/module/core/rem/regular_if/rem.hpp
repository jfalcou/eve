...
TTS_CASE_TPL("Check eve::rem[condition] behavior", EVE_TYPE)
{
  ...
  // All basic TRUE
  TTS_EQUAL(eve::rem[ 1 ](tv, fv)     , eve::rem(tv, fv));
  TTS_EQUAL(eve::rem[ 1.0 ](tv, fv)   , eve::rem(tv, fv));
  TTS_EQUAL(eve::rem[ true ](tv, fv)  , eve::rem(tv, fv));
  TTS_EQUAL(eve::rem[ t ](tv, fv)     , eve::rem(tv, fv));
  ...
  TTS_EQUAL(eve::rem[ m ](tv, fv) , eve::if_else(m,eve::rem(tv, fv), tv) );
}
