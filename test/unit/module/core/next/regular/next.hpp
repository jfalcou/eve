...
TTS_CASE_TPL("Check eve::next one parameter behavior", EVE_TYPE)
{
  ...
  {
    TTS_IEEE_EQUAL(eve::next(eve::Inf<T>()), (eve::Nan<T>()));
    TTS_EQUAL(eve::next(eve::Valmax<T>())  , (eve::Inf<T>()));
    TTS_EQUAL(eve::next(eve::Minf<T>())    , (eve::Valmin<T>()));
    TTS_EQUAL(eve::next(eve::Mone<T>())    , (eve::Mone<T>()+eve::Eps<T>()/2));
    TTS_EQUAL(eve::next(eve::One<T>())     , (eve::One<T>()+eve::Eps<T>()));
...
TTS_CASE_TPL("Check eve::next two parameters behavior", EVE_TYPE)
{
  ...
  {
    TTS_IEEE_EQUAL(eve::next(eve::Inf<T>(), i_t(2))   , (eve::Nan<T>()));
    TTS_IEEE_EQUAL(eve::next(eve::Valmax<T>(), i_t(2)), (eve::Nan<T>()));
    TTS_EQUAL(eve::next(eve::Mone<T>(), i_t(2))       , (eve::Mone<T>()+eve::Eps<T>()));
    TTS_EQUAL(eve::next(eve::One<T>(), i_t(2))        , (eve::One<T>()+eve::Eps<T>()*2));

    TTS_IEEE_EQUAL(eve::next(eve::Inf<T>(), si_t(2))   , (eve::Nan<T>()));
    TTS_IEEE_EQUAL(eve::next(eve::Valmax<T>(), si_t(2)), (eve::Nan<T>()));
    TTS_EQUAL(eve::next(eve::Mone<T>(), si_t(2))       , (eve::Mone<T>()+eve::Eps<T>()));
    TTS_EQUAL(eve::next(eve::One<T>(), si_t(2))        , (eve::One<T>()+eve::Eps<T>()*2));

    TTS_IEEE_EQUAL(eve::next(eve::Inf<v_t>(), i_t(2))   , (eve::Nan<T>()));
    TTS_IEEE_EQUAL(eve::next(eve::Valmax<v_t>(), i_t(2)), (eve::Nan<T>()));
    TTS_EQUAL(eve::next(eve::Mone<v_t>(), i_t(2))       , (eve::Mone<T>()+eve::Eps<T>()));
    TTS_EQUAL(eve::next(eve::One<v_t>(), i_t(2))        , (eve::One<T>()+eve::Eps<T>()*2));

    TTS_EQUAL(eve::next(T( 0 ), 1)             , (eve::Mindenormal<T>()));
    TTS_EQUAL(eve::next(T(-0.), 1)            , (eve::Mindenormal<T>()));
    TTS_EQUAL(eve::next(-eve::Mindenormal<T>(), 1)     , (T( 0 )));
