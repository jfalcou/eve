...
// almost tests
//==================================================================================================
TTS_CASE_TPL("Check  with nans and infs", eve::test::simd::ieee_reals_wf16)
<typename T>(tts::type<T>)
{
...
<typename T>(T const& a0)
{
 ...
 using uwi_t = eve::as_integer_t<T, unsigned>;
  ...
  if constexpr( eve::floating_value<T> )
  {
    TTS_EQUAL(eve::trunc(a0), T([&](auto i, auto) { return v_t(std_trunc(a0.get(i))); }));
    TTS_EQUAL(eve::trunc(a0, eve::as<signed>()), wi_t([&](auto i, auto) { return i_t(a0.get(i)); }));
    TTS_EQUAL(eve::trunc(eve::abs(a0), eve::as<unsigned>()), uwi_t([&](auto i, auto) { return ui_t(std_abs(a0.get(i))); }));
  }
