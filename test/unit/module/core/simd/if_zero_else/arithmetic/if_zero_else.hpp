...
TTS_CASE_TPL( "Check if_zero_else behavior on homogeneous wide"
...
TTS_CASE_TPL( "Check if_zero_else behavior on wide + scalar"
            , fixed<1>,fixed<2>,fixed<4>,fixed<8>,fixed<16>,fixed<32>,fixed<64>
            )
{
  using eve::wide;
  ...
  auto z = eve::Zero<wide<Type,T>>();
  auto z_= eve::zero_; 
  TTS_EQUAL(eve::if_else(lhs, z, Type(7)) , eve::if_else(lhs, z_, Type(7)));  
}
