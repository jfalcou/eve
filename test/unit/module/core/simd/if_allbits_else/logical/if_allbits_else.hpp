...
TTS_CASE_TPL( "Check if_allbits_else behavior on homogeneous logical  wide"
            ...
            )
{
  ...
  auto z = eve::Allbits<wide<Type,T>>();
...
TTS_CASE_TPL( "Check if_allbits_else behavior on logical wide + scalar"
            , fixed<1>,fixed<2>,fixed<4>,fixed<8>,fixed<16>,fixed<32>,fixed<64>
            )
{
  using eve::wide;
