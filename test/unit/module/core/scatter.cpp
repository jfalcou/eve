...
template<typename Index, typename T, typename M>
void test_scatter(T const& values, M const& cond, auto ptr)
{
  using e_t = eve::element_type_t<T>;
  eve::wide<Index, eve::fixed<T::size()>> indexes = [](auto i, auto) { return 2*i; };
  ...
  auto m = cond.mask( eve::as<eve::as_logical_t<T>>{} );

  for( std::size_t n = 0; n < T::size(); ++n )
  {
    if(m.get(n)) ref[indexes.get(n)] = values.get(n);
  }

  eve::scatter[cond](values, ptr, indexes);
...
TTS_CASE_TPL("Check eve::scatter behavior with pointer and mask", eve::test::simd::all_types)
<typename T>(tts::type<T>)
{
  using e_t = eve::element_type_t<T>;
  e_t data[2*T::size()];
  ...
  test_scatter<std::int32_t >(values, eve::ignore_first(1), &data[0]);
  test_scatter<std::uint32_t>(values, eve::ignore_first(1), &data[0]);
  test_scatter<std::int64_t >(values, eve::ignore_first(1), &data[0]);
  test_scatter<std::uint64_t>(values, eve::ignore_first(1), &data[0]);
};
...
<typename T>(tts::type<T>)
{
  ...
  test_scatter<std::int32_t >(values,eve::ignore_first(1),eve::as_aligned(&data[0],typename T::cardinal_type{}));
  test_scatter<std::uint32_t>(values,eve::ignore_first(1),eve::as_aligned(&data[0],typename T::cardinal_type{}));
  test_scatter<std::int64_t >(values,eve::ignore_first(1),eve::as_aligned(&data[0],typename T::cardinal_type{}));
  test_scatter<std::uint64_t>(values,eve::ignore_first(1),eve::as_aligned(&data[0],typename T::cardinal_type{}));
};
