...
{
  using t_t = eve::wide<std::int8_t, T::size()>;

  TTS_EXPR_IS(eve::convert(T(), eve::as<std::int8_t>()), t_t);
  TTS_EXPR_IS(eve::convert[eve::saturated](T(), eve::as<std::int8_t>()), t_t);
};
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr( sizeof(v_t) >= 2 )
  {
    ...
    else { TTS_EQUAL(eve::convert[eve::saturated](eve::valmin(eve::as<T>()), tgt), t_t(0)); }
  }
  else
  {
...
<typename T>(tts::type<T>)
{
  using t_t          = eve::logical<eve::wide<std::int8_t, T::size()>>;
