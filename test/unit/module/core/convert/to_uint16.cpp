...
<typename T>(eve::as<T>)
{
  ...
  TTS_EXPR_IS(eve::uint16(T())                                           , t_t);
  TTS_EXPR_IS(eve::saturated(eve::convert)(T(), eve::as<std::uint16_t>()), t_t);
};
...
<typename T>(eve::as<T>)
{
  ...
  if constexpr(eve::integral_value<T>)
  {
    ...
    {
      TTS_EQUAL(eve::saturated(eve::convert)(eve::valmax(eve::as<T>()), tgt), t_t(eve::valmax(eve::as<v_t>())) );
    }
    else
    {
