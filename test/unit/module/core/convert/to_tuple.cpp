...
TTS_CASE("Conversion from/to UDTs &nd tuples")
{
  TTS_EQUAL((eve::convert(udt::grid2d {-9, 70}, eve::as<udt::label_position>())),
            (udt::label_position {-9.f, std::uint8_t{'F'}}));

  TTS_EQUAL((eve::convert(udt::label_position {-9.f, std::uint8_t{'F'}}, eve::as<udt::grid2d>())),
            (udt::grid2d {-9, 70}));

  TTS_EQUAL((eve::convert(udt::grid2d {66, -9}, eve::as<kumi::tuple<std::int8_t, float>>())),
            (kumi::tuple {std::uint8_t{'B'}, -9.f}));

  TTS_EQUAL((eve::convert(kumi::tuple {std::uint8_t{'B'}, -9.f}, eve::as<udt::grid2d>())),
            (udt::grid2d {66, -9}));
