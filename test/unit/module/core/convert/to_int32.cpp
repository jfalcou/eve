...
{
  using t_t = eve::wide<std::int32_t, T::size()>;

  TTS_EXPR_IS(eve::convert(T(), eve::as<std::int32_t>()), t_t);
  TTS_EXPR_IS(eve::convert[eve::saturated](T(), eve::as<std::int32_t>()), t_t);
};
...
<typename T>(tts::type<T>)
{
  ...
  constexpr auto tgt = eve::as<std::int32_t>();
...
<typename T>(tts::type<T>)
{
  using t_t          = eve::logical<eve::wide<std::int32_t, T::size()>>;
  constexpr auto tgt = eve::as<eve::logical<std::int32_t>>();
