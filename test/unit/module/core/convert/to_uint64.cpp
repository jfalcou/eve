...
{
  using t_t = eve::wide<std::uint64_t, T::size()>;

  TTS_EXPR_IS(eve::convert(T(), eve::as<std::uint64_t>()), t_t);
  TTS_EXPR_IS(eve::convert[eve::saturated](T(), eve::as<std::uint64_t>()), t_t);
};
...
<typename T>(tts::type<T>)
{
  ...
  constexpr auto tgt = eve::as<std::uint64_t>();
...
<typename T>(tts::type<T>)
{
  ...
  TTS_EQUAL(eve::convert[eve::saturated](eve::valmin(eve::as<T>()), tgt), static_cast<t_t>(0));
...
<typename T>(tts::type<T>)
{
  using t_t          = eve::logical<eve::wide<std::uint64_t, T::size()>>;
  constexpr auto tgt = eve::as<eve::logical<std::uint64_t>>();
