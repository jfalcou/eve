...
<typename T>(tts::type<T>)
{
  ...
  TTS_EXPR_IS(eve::convert[saturated](T(), eve::as<std::uint8_t>()), t_t);
};
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr( eve::integral_value<T> )
  {
    ...
    {
      TTS_EQUAL(eve::convert[saturated](eve::valmax(eve::as<T>()), tgt), t_t(127));
    }
    else { TTS_EQUAL(eve::convert[saturated](eve::valmax(eve::as<T>()), tgt), t_t(255)); }
  }
};
