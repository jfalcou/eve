...
TTS_CASE("Check eve::saturated_(eve::convert) return type")
{
  ...
  TTS_EXPR_IS(eve::saturated_(eve::convert)(Type(), eve::as<std::int64_t>()), target_t);
  TTS_EXPR_IS(eve::saturated_(eve::convert)(Type(), eve::int64_)     , target_t);
}
...
TTS_CASE("Check eve::saturated_(eve::convert) behavior")
{
  ...
  if constexpr(std::is_integral_v<Value>)
  {
    ...
    else
    {
      ...
      {
        TTS_EQUAL(eve::saturated_(eve::convert)(eve::Valmax<Type>(), eve::int64_), (eve::Valmax<target_t>()) );   
      }       
    }
  }
}
