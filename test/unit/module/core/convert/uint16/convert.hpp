...
TTS_CASE_TPL("Check eve::convert behavior", EVE_TYPE)
{
  ...
  if constexpr(eve::integral_value<T>)
  {
    ...
    TTS_EQUAL(eve::convert(eve::Valmin<T>(), eve::as<std::uint16_t>()), static_cast<target_t>(eve::Valmin<v_t>()) );
    TTS_EQUAL(eve::convert(eve::Valmax<T>(), eve::as<std::uint16_t>()), static_cast<target_t>(eve::Valmax<v_t>()) );
  }
}
