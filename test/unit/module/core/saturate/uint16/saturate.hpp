...
{
  TTS_EQUAL(eve::saturate(EVE_TYPE(0)    , eve::as<std::uint16_t>() ), EVE_TYPE(0)     );
  TTS_EQUAL(eve::saturate(EVE_TYPE(42.69), eve::as<std::uint16_t>() ), EVE_TYPE(42.69) );
  ...
  if constexpr(eve::signed_value<EVE_VALUE>)
  {
    ...
    {
      TTS_EQUAL(eve::saturate(eve::valmax(eve::as<T>()), eve::as<std::uint16_t>()), EVE_TYPE(eve::valmax(eve::as<std::uint16_t>())) );
    }
  }
  else
  {
