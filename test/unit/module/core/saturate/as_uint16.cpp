...
<typename T>(tts::type<T>) { TTS_EXPR_IS(eve::saturate(T(), eve::as<std::uint16_t>()), T); };
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr( eve::signed_value<T> )
  {
    ...
    {
      TTS_EQUAL(eve::saturate(eve::valmax(eve::as<T>()), eve::as<std::uint16_t>()),
                eve::valmax(eve::as<T>()));
    }
    else
    {
      TTS_EQUAL(eve::saturate(eve::valmax(eve::as<T>()), eve::as<std::uint16_t>()),
                T(eve::valmax(eve::as<std::uint16_t>())));
    }
  }
  else
  {
