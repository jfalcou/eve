...
{
  TTS_EXPR_IS(eve::is_not_denormal(Type(0)),  eve::as_logical_t<Type>);
}
...
TTS_CASE("Check eve::is_not_denormal behavior")
{
  ...
  {
    TTS_EQUAL ( eve::is_not_denormal(eve::Smallestposval<Type>()/2)
              , eve::logical<Type>(!eve::platform::supports_denormals)
              );
    TTS_EQUAL(eve::is_not_denormal(eve::Inf<Type>()), eve::True<Type>());
    TTS_EQUAL(eve::is_not_denormal(eve::Nan<Type>()), eve::True<Type>());
  }
