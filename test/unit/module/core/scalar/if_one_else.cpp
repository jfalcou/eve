...
TTS_CASE( "Check if_else_one returnn type" )
{
  auto z = eve::one_; 
...
TTS_CASE( "Check if_else_one behavior" )
{
  ...
  TTS_EQUAL ( eve::if_else( eve::False<eve::logical<std::int32_t>>(), z, 2), 2);
  TTS_EQUAL ( eve::if_else( 1, z, 2u), eve::One<std::uint32_t>()); 
  TTS_EQUAL ( eve::if_else( 1.0, z, 2u),  eve::One<std::uint32_t>()); 
  TTS_EQUAL ( eve::if_else( true, z, 2u),  eve::One<std::uint32_t>()); 
  TTS_EQUAL ( eve::if_else( eve::True<eve::logical<std::int32_t>>(), z, 2u), eve::One<std::uint32_t>());
}
