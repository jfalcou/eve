...
{
  logical_test_case<ManualTestAny>(eve::any, true);
  logical_test_case<ManualTestAny>(eve::any, false);
};

TTS_CASE_TPL("Check eve::any behavior on scalars", eve::test::scalar::all_types)
<typename T>(tts::type<T>)
{
  logical_test_case<ManualTestAny>(eve::any, eve::logical<T>{true});
  logical_test_case<ManualTestAny>(eve::any, eve::logical<T>{false});
};

TTS_CASE_TPL("Check eve::any behavior on wides and top_bits", eve::test::simd::all_types)
<typename T>(tts::type<T>)
{
  logical_simd_test_cases<ManualTestAny>(eve::any, eve::as<T>{});
};
