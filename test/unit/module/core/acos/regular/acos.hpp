...
TTS_CASE("Check eve::acos behavior")
{
  ...
  {
    TTS_ULP_EQUAL(eve::acos(EVE_TYPE(2)) , eve::Nan<EVE_TYPE>(), 0);
    TTS_ULP_EQUAL(eve::acos(EVE_TYPE(-2)), eve::Nan<EVE_TYPE>(), 0);
  }

  TTS_ULP_EQUAL(eve::acos(EVE_TYPE( 0.5)) , eve::Pi<EVE_TYPE>()/3   , 0.5 );
  TTS_ULP_EQUAL(eve::acos(EVE_TYPE(-0.5)) , 2*eve::Pi<EVE_TYPE>()/3 , 0.5 );
  TTS_ULP_EQUAL(eve::acos(EVE_TYPE(-1. )) , eve::Pi<EVE_TYPE>()     , 0   );
  TTS_ULP_EQUAL(eve::acos(EVE_TYPE( 1. )) , EVE_TYPE(0)             , 0   );
  TTS_ULP_EQUAL(eve::acos(EVE_TYPE( 0. )) , eve::Pio_2<EVE_TYPE>()  , 0   );
