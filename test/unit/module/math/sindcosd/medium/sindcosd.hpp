...
TTS_CASE_TPL("Check eve::medium_(eve::sindcosd) return type", EVE_TYPE)
{
  TTS_EXPR_IS(eve::medium_(eve::sindcosd)(T()), (std::tuple<T, T>));
}

TTS_CASE_TPL("Check eve::medium_(eve::sindcosd) behavior", EVE_TYPE)
{
  ...
  for(auto v : x)
  {
    auto [sin_, cos_] = eve::medium_(eve::sindcosd)(T(v));
