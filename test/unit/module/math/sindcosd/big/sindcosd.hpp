...
TTS_CASE("Check eve::big(eve::sindcosd) return type")
{
  TTS_EXPR_IS(eve::big(eve::sindcosd)(EVE_TYPE()), (std::tuple<EVE_TYPE,EVE_TYPE>));
}

TTS_CASE("Check eve::big(eve::sindcosd) behavior")
{
  ...
  for(auto v : x)
  {
    auto [sin_, cos_] = eve::big(eve::sindcosd)(EVE_TYPE(v));
