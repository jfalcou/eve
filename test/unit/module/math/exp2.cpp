...
<typename T>(T const& a0, T const& a1)
{
  ...
  TTS_ULP_EQUAL(
      eve::pedantic(eve::exp2)(a0), map([](auto e) -> v_t { return std::exp2(e); }, a0), 30);
  TTS_ULP_EQUAL(
      eve::pedantic(eve::exp2)(a1), map([](auto e) -> v_t { return std::exp2(e); }, a1), 2);
};
...
<typename T>(tts::type<T>)
{
  ...
  if constexpr( eve::floating_value<T> )
  {
    ...
    {
      TTS_ULP_EQUAL(eve::pedantic(eve::exp2)(eve::minlog2(eve::as<T>())),
                    T(std::exp2(eve::minlog2(eve::as<elt_t>()))),
                    0.5);
      TTS_ULP_EQUAL(eve::pedantic(eve::exp2)(eve::prev(eve::minlog2(eve::as<T>()))),
...
TTS_CASE_TPL("Check float64 & float32(eve::exp2) behavior", eve::test::simd::integers)
<typename T>(tts::type<T>)
{
  using v_t  = eve::element_type_t<T>;
