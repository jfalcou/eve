...
<typename T>(tts::type<T>)
{
  ...
  TTS_EQUAL(eve::sqrt_pi(eve::as<T>()), T(std::sqrt(3.141592653589793238462643l)));
};
...
{
  using eve::as;
  TTS_EXPECT(eve::all(eve::test::is_near(eve::sqrt_pi[eve::downward](as<T>()), eve::sqrt_pi[eve::upward](as<T>()))));
};
