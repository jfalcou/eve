...
<typename T>(tts::type<T>)
{
  ...
  {
    TTS_EXPECT(eve::exp_pi[eve::lower](eve::as<T>()) <= std::exp(3.141592653589793238462643l));
    TTS_EXPECT(eve::exp_pi[eve::upper](eve::as<T>()) >= std::exp(3.141592653589793238462643l));
  }
...
{
  using eve::as;
  TTS_EXPECT(eve::all(eve::test::is_near(eve::exp_pi[eve::lower](as<T>()), eve::exp_pi[eve::upper](as<T>()))));
};
