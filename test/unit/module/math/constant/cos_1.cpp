...
<typename T>(tts::type<T>)
{
  ...
  {
    TTS_EXPECT(eve::cos_1[eve::lower](eve::as<T>()) <= std::cos(1.0l));
    TTS_EXPECT(eve::cos_1[eve::upper](eve::as<T>()) >= std::cos(1.0l)); 
  }
...
{
  using eve::as;
  TTS_EXPECT(eve::all(eve::test::is_near(eve::cos_1[eve::lower](as<T>()), eve::cos_1[eve::upper](as<T>()))));
};
