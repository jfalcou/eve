...
<typename T>(tts::type<T>)
{
  ...
  {
    TTS_EXPECT(eve::sqrt_2pi[eve::lower](eve::as<T>()) <= std::sqrt(2*3.141592653589793238462643l));
    TTS_EXPECT(eve::sqrt_2pi[eve::upper](eve::as<T>()) >= std::sqrt(2*3.141592653589793238462643l));
  }
...
{
  using eve::as;
  TTS_EXPECT(eve::all(eve::test::is_near(eve::sqrt_2pi[eve::lower](as<T>()), eve::sqrt_2pi[eve::upper](as<T>()))));
};
