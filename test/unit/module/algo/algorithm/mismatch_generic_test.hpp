...
namespace algo_test
{
  ...
  void mismatch_generic_test(eve::as<T> tgt, Algo alg, Check check)
  {
    ...
    mismatch_one_algo_test(tgt, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<2>], check, gen);
    mismatch_one_algo_test(tgt, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<3>], check, gen);
  }
}
