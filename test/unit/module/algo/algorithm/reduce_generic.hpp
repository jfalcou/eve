...
void reduce_generic_specific_tests(eve::as<T>, Algo alg)
{
...
{
  reduce_generic_specific_tests(tgt, alg);

  algo_test::page_ends_test(tgt, reduce_ptr_test{init, alg});
}

template <typename T, typename Alg>
void reduce_generic_all_test_cases(eve::as<T> tgt, Alg basic_reduce)
{
  using e_t = eve::element_type_t<T>;
