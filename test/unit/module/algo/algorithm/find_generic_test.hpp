...
namespace algo_test
{
  ...
  void find_generic_test(eve::as<T> as_t, Algo alg, Check check)
  {
    ...
    find_generic_test_page_ends(as_t, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<1>], check);
    find_generic_test_page_ends(as_t, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<2>], check);
    find_generic_test_page_ends(as_t, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<3>], check);
    find_generic_test_page_ends(as_t, alg[eve::algo::force_width<T::size()>][eve::algo::unroll<4>], check);
  }
}
