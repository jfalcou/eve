...
<typename T>(tts::type<T>)
{
  ...
  auto f = eve::views::iota_with_step(e_t{5}, e_t{2}).width_cast(eve::lanes<T::size()>);
  auto l = f + T::size();
...
<typename T>(tts::type<T>)
{
  ...
  auto expected_bytes = eve::views::iota_with_step(std::int8_t{0}, std::int8_t{1}).width_cast(eve::lanes<eve::nofs_width_v<T>>);
