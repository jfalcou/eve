  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_RELATIVE_EQUAL(eve::coth(eq_t(e, f, e, f)),  eve::rec(cvtbq_eq(bm::tanh(bq_t(e, f, e, f)))), tol);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1, T const& a2, T const& a3 )
{
  ...
  auto tol = 3000.0*eve::eps(eve::as<e_t>());
  using z_t = eve::as_quaternion_t<T>;
  auto z = z_t{a0,a1,a2,a3};
  z_t r;
  for(int i = 0; i < eve::cardinal_v<T>; ++i){
    bm::quaternion<e_t> bq(a0.get(i), a1.get(i), a2.get(i), a3.get(i));
    eve::quaternion<e_t> eq= eve::rec(cvtbq_eq(bm::tanh(bq)));
    r.set(i, eq);
  }

  TTS_RELATIVE_EQUAL(eve::coth(z), r, tol);
};
