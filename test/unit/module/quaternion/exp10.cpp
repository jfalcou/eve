  ...
  <typename T>(T const& a0, T const& a1, T const& a2, T const& a3 )
{
  ...
  for(size_t i = 0; i < a0.size(); ++i)
  {
    ...
    auto l10 = eve::log_10(eve::as<e_t>());
    bq_t bz = bq_t(a0[i],a1[i],a2[i],a3[i])*l10;
    TTS_RELATIVE_EQUAL(eve::exp10(z),  cvtbq_eq(bm::exp(bz)), tol);
  }
};
  ...
  <typename T>(T const& a0, T const& a1, T const& a2, T const& a3 )
{
  ...
  for(int i = 0; i < eve::cardinal_v<T>; ++i){
    ...
    eve::quaternion<e_t> eq= cvtbq_eq(bm::exp(bq*l10));
    r.set(i, eq);
  }

  TTS_RELATIVE_EQUAL(eve::exp10(z), r, tol);
};
