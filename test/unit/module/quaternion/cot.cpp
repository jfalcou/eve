  ...
  <typename T>(T const& a0, T const& a1 )
{
  ...
  for(auto e : a0)
  {
    for(auto f : a1)
    {
      TTS_RELATIVE_EQUAL(eve::cot(eq_t(e, f, e, f)),  eve::rec(cvtbq_eq(bm::tan(bq_t(e, f, e, f)))), tol);
    }
  }
};
  ...
  <typename T>(T const& a0, T const& a1, T const& a2, T const& a3 )
{
  ...
  for(int i = 0; i < eve::cardinal_v<T>; ++i){
    ...
    eve::quaternion<e_t> eq= eve::rec(cvtbq_eq(bm::tan(bq)));
    r.set(i, eq);
  }

  TTS_RELATIVE_EQUAL(eve::cot(z), r, tol);
};
