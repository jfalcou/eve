...
namespace eve
{
  ...
  template<typename T> inline bool compare_equal(logical<T> const& l, logical<T> const& r)
  {
    if constexpr( eve::simd_value<T> )
      return l.bitmap() == r.bitmap();
    else
      return l == r;
  }

  template<typename T> inline std::string to_string(logical<T> const& l)
  {
  ...
  template<typename T> inline std::string to_string(detail::top_bits<T> const& l)
  {
    std::ostringstream str;
    str << l;
    return str.str();
