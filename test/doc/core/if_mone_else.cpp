...
int main()
{
  ...
  std::cout << "---- simd" << '\n'
            ...
            << " <- lsi =                             " << lsi << '\n'
            << " <- pi =                              " << pi << '\n'
            << " -> eve::if_else(si, eve::mone, pi) =         "
            << eve::if_else(si, eve::mone, pi) << '\n'
            << " -> eve::if_else(lsi, eve::mone, pi_) =        "
            << eve::if_else(lsi, eve::mone, pi) << '\n';
  ...
  std::cout << "---- scalar" << '\n'
            ...
            << " lssi =                               " << lssi << '\n'
            << " xi =                                 " << xi << '\n'
            << " -> eve::if_else(ssi, eve::mone, xi) =        "
            << eve::if_else(ssi, eve::mone, xi) << '\n'
            << " -> eve::if_else(lssi, xi, eve::mone, xi) =       "
            << eve::if_else(lssi, eve::mone, xi) << '\n';
  return 0;
}
