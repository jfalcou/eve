...
namespace eve
{
  ...
  //! #### Example
  //!
  //! @godbolt{doc/core/euler_pow_pi.cpp}
  //!
  //! @}
  //================================================================================================
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto euler_pow_pi_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.724046p+4;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.724046eb0933ap+4;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.724048p+4;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.724046eb0933ap+4;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.724046p+4;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.724046eb09339p+4;
      }
    }
  }
}
