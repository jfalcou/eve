...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto log_phi_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.ecc2cap-2;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.ecc2caec5160ap-2;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.ecc2ccp-2;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.ecc2caec5160ap-2;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.ecc2cap-2;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.ecc2caec51609p-2;
      }
    }
  }
}
