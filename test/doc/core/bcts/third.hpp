...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto third_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.555556p-2;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.5555555555555p-2;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.555556p-2;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.5555555555556p-2;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.555554p-2;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.5555555555555p-2;
      }
    }
  }
