...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto khinchin_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.57bce4p+1;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.57bce423c6d0dp+1;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.57bce6p+1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.57bce423c6d0ep+1;
      }
