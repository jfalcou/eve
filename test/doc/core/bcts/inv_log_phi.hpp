...
namespace eve
{
  ...
  //! #### Example
  //!
  //! @godbolt{doc/core/inv_log_phi.cpp}
  //!
  //! @}
  //================================================================================================
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto inv_log_phi_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.09fecp+1;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.09fec09279922p+1;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.09fec2p+1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.09fec09279922p+1;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.09fecp+1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.09fec09279921p+1;
      }
    }
  }
}
