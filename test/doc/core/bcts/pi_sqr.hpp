...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto pi_sqr_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.3bd3ccp+3;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.3bd3cc9be45dep+3;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.3bd3cep+3;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.3bd3cc9be45dfp+3;
      }
