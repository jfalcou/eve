...
const int TEST_ITERATIONS = 10000; //000;
int main() {
  {
    ...
    detail::print_m256(aligned_vec, "Aligned load result");

    // Demonstrate unaligned load
    __m256 unaligned_vec = _mm256_loadu_ps(unaligned_ptr);
    detail::print_m256(unaligned_vec, "Unaligned load result");
    ...
    detail::benchmark_comparison("Load Operations", aligned_load, unaligned_load, 10);
    std::cout << std::endl;
...
// //     detail::print_m256(masked_vec, "Masked load result (even indices only)");
// //     std::cout << std::endl;
...
// //     // Performance comparison
// //     detail::Timer timer2("Aligned vs. Unaligned Store Performance");
...
// //     detail::benchmark_comparison("Store Operations", aligned_store, unaligned_store, 10);
// //     std::cout << std::endl;
...
// //     float* large_array = detail::aligned_alloc<float>(LARGE_SIZE, 32);
...
//     std::cout << "End Intel ==================================================== " << std::endl;
  }
  {
    ...
    using w8_t =  eve::wide<float, eve::fixed<8>>;
    // Demonstrate aligned load
    w8_t aligned_vec = eve::load(aligned_data);
    print(aligned_vec, "Aligned load result");

    // Demonstrate unaligned load
    w8_t unaligned_vec = eve::load(unaligned_ptr);
    print(unaligned_vec, "Unaligned load result");
    ...
    auto aligned_load = [&]() {
      w8_t result;
      for (int i = 0; i < TEST_ITERATIONS; ++i) {
        result = _mm256_load_ps(aligned_data);//eve::load(aligned_data);
      }
      return result;
    };
    ...
    auto unaligned_load = [&]() {
      w8_t  result;
      for (int i = 0; i < TEST_ITERATIONS; ++i) {
        result = _mm256_loadu_ps(aligned_data);//eve::load(unaligned_ptr);
      }
      return result;
    };
