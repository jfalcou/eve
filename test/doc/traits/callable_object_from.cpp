...
namespace my_lib
{
  struct func_t : eve::basic_callable<func_t>
  {
  ...
  namespace impl
  {
    auto func_(EVE_REQUIRES(eve::cpu_), auto, eve::integral_value auto x) {  return x*x;  }
    auto func_(EVE_REQUIRES(eve::cpu_), auto, double x)                   {  return 1./x; }
  }
}
