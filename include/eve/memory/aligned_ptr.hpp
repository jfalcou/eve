...
namespace eve
{
  ...
  template< typename Type
          , cardinal_type Lanes = expected_cardinal_v<Type>
          >
  ...
  struct aligned_ptr
  {
    ...
    static constexpr auto alignment() noexcept { return Lanes * sizeof(Type); }
  ...
  //================================================================================================
  template<std::size_t A, typename T, cardinal_type Other>
  ...
  template<typename T, cardinal_type A>  struct element_type<aligned_ptr<T,A>> { using type = T; };
  template<typename T>          struct element_type<aligned_ptr<T>>   { using type = T; };

  template<typename T1, cardinal_type A1, typename T2, cardinal_type A2>
  ...
  template<typename Type, cardinal_type Lanes>
  ...
  template<typename Type, cardinal_type Lanes>
  aligned_ptr<Type, Lanes> operator-(aligned_ptr<Type, Lanes> const &p, cardinal_type o) noexcept
  {
    auto that = p;
    return that -= o;
  }
  ...
  //! @param n      Lanes count to satisfy
  ...
  {
    return previous_aligned_address(p, fixed<eve::expected_cardinal_v<std::remove_cvref_t<T>>>{});
  }
  ...
  {
    return next_aligned_address(p, fixed<eve::expected_cardinal_v<std::remove_cvref_t<T>>>{});
  }
