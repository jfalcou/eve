  ...
  template<typename T, typename Size> struct as_register<T, Size, eve::ppc_>
  {
    ...
    static constexpr auto find()
    {
      ...
      else if constexpr( std::is_integral_v<T> )
      {
        ...
        if constexpr( signed_v && size_check(1, 16) )
          return (__vector signed char) {};
        if constexpr( signed_v && size_check(2, 8) )
          return (__vector signed short) {};
        if constexpr( signed_v && size_check(4, 4) )
        ...
        if constexpr( spy::simd_instruction_set == spy::vsx_ )
        {
          if constexpr( signed_v && size_check(8, 2) )
            return (__vector signed long) {};
          if constexpr( !signed_v && size_check(8, 2) )
            return (__vector unsigned long) {};
        }
        else
        {
          if constexpr( size_check(8, 1) )
            return emulated_ {};
        }
      }
    }
