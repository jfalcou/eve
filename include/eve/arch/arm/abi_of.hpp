...
namespace eve { namespace ext
{
  ...
  template<int N> struct abi_of<double, N, std::enable_if_t<(N==1)>>  { using type = neon64_; };
#endif

  template<int N> struct abi_of<float, N, std::enable_if_t<(N==4)>>  { using type = neon128_; };
  ...
  template<int N> struct abi_of<float, N, std::enable_if_t<(N<=2)>>  { using type = neon64_; };

  template<typename T, int N>
  struct abi_of < T, N
                , std::enable_if_t<std::is_integral_v<T> && (N <= 8/sizeof(T))>
                >
  {
    using type = neon64_;
  };
