...
namespace eve
{
  ...
  namespace ext
  {
    template<typename Size> struct as_register<double, Size, eve::avx_> { using type = __m256d; };
    template<typename Size> struct as_register<float , Size, eve::avx_> { using type = __m256;  };
