    ...
    : detail::wide_storage<as_logical_register_t<Type, Cardinal, abi_t<Type, Cardinal>>>
{
  ...
  {
    return sizeof(Type) * Cardinal::value;
  }
      ...
      : storage_base(
          [&]()
          {
            constexpr auto c = Cardinal::value == 1 && sizeof(Type) == 8
                               && std::is_same_v<abi_type, arm_64_> && current_api != asimd;
            if constexpr( c ) return value_type(r);
            else return r;
          }())
  {}
  ...
  //! Constructs a eve::logical by splatting a boolean value in all lanes
  ...
  EVE_FORCEINLINE logical(logical<wide<Type, Half>> const& l,
                          logical<wide<Type, Half>> const& h) noexcept
  ...
  EVE_FORCEINLINE auto bitmap() const noexcept
  {
    return detail::to_bitmap(eve::current_api, *this);
  }
  ...
  template<typename U>
  friend EVE_FORCEINLINE auto operator&&(logical const                   & v,
                                         logical<wide<U, Cardinal>> const& w) noexcept
  {
    return detail::self_logand(eve::current_api, v, w);
  }

  //! Perform a logical and operation between a eve::logical and a boolean
  friend EVE_FORCEINLINE auto operator&&(logical const& v, bool w) noexcept
  {
    return v && logical {w};
  }

  //! Perform a logical and operation between a boolean and a eve::logical
  friend EVE_FORCEINLINE auto operator&&(bool v, logical const& w) noexcept { return w && v; }
  ...
  //! Perform a logical or operation between a eve::logical and a boolean
  friend EVE_FORCEINLINE auto operator||(logical const& v, bool w) noexcept
  {
    return v || logical {w};
  }

  //! Perform a logical or operation between a boolean and a eve::logical
  friend EVE_FORCEINLINE auto operator||(bool v, logical const& w) noexcept { return w || v; }
  ...
  EVE_FORCEINLINE void set(std::size_t i, logical<Type> v) noexcept { detail::insert(*this, i, v); }
  ...
  //!
  //==============================================================================================
  EVE_FORCEINLINE auto slice() const requires(Cardinal::value > 1) { return detail::slice(*this); }
  ...
  template<std::convertible_to<logical> S>
