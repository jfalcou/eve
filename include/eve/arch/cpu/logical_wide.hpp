...
namespace eve
{
  template<typename Type, typename N, typename ABI> struct EVE_MAY_ALIAS logical<wide<Type, N, ABI>>
  {
    ...
    template<typename T, typename C = expected_cardinal_t<T>> using rebind = logical<wide<T, C>>;
    ...
    EVE_FORCEINLINE logical(T0 const &v0, T1 const &v1, Ts const &...vs) noexcept requires(
        std::convertible_to<T0, logical<Type>> &&std::convertible_to<
            T0,
            logical<
                Type>> && (... && std::convertible_to<Ts, logical<Type>>)&&(static_size == 2 + sizeof...(Ts)))
    ...
    EVE_FORCEINLINE logical(Generator &&g) noexcept
        ...
        : data_(detail::fill(as_<logical> {}, abi_type {}, [&](int i, int c) {
    ...
    EVE_FORCEINLINE logical(logical<wide<Type, halfSize, Other>> const &l,
                            logical<wide<Type, halfSize, Other>> const &h) noexcept
    ...
    //==============================================================================================
    template<typename Index> EVE_FORCEINLINE logical operator[](wide<Index, N> const &idx) noexcept
    {
    ...
    // elementwise access
    //==============================================================================================
    EVE_FORCEINLINE void set(std::size_t i, value_type v) noexcept { data_.set(i, v); }
