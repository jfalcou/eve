...
namespace eve
{
  ...
  struct EVE_MAY_ALIAS EVE_STRUCT_ABI wide
      : _::wide_split_type_helper<Type, Width>,
        _::wide_storage<as_register_t<translate_t<Type>, Width, abi_t<translate_t<Type>, Width>>>
  {
    using storage_base = _::wide_storage<as_register_t<translate_t<Type>, Width, abi_t<translate_t<Type>, Width>>>;
    ...
    using translated_type = wide<translated_element_type, Width>;
    ...
    {
      EVE_ASSERT(std::distance(std::begin(r), std::end(r)) == Width,
    ...
    EVE_FORCEINLINE Type back() const noexcept { return get(Width - 1); }
    ...
    //!
    //==============================================================================================
    EVE_FORCEINLINE auto slice() const requires(Width > 1)
    {
      return _::slice(*this);
    }
    ...
    EVE_FORCEINLINE auto slice(slice_t<Slice> s) const requires(Width > 1)
    {
    ...
    template<scalar_value U, width_type M>
    ...
    //! Do not participate to overload resolution if `sizeof(Type) != sizeof(S)`
    template<scalar_value S>
    friend EVE_FORCEINLINE wide<S, Width> operator&(S s, wide const& w) noexcept
#if !defined(EVE_DOXYGEN_INVOKED)
        requires(!eve::product_type<Type> && supports_bitwise_call<S, wide>)
#endif
    {
      return bit_and(bit_cast(w, as<wide<S, Width>>()), s);
    }
...
#endif
    {
      return bit_or(bit_cast(w, as<wide<S, Width>>{}), s);
    }
...
#endif
    {
      return bit_xor(bit_cast(w, as<wide<S, Width>>()), s);
    }
    ...
    friend auto& operator<<(std::basic_ostream<C, Ct>& os, wide p)
    {
      if constexpr( eve::product_type<Type> )
      {
        if constexpr( requires(Type t) { os << t; } )
        {
          ...
          os << '(' << p.get(0);
          for( width_type i = 1; i != p.size(); ++i ) os << ", " << p.get(i);
          return os << ')';
        }
        else
        {
      ...
      else
      {
        ...
        for( width_type i = 1; i != p.size(); ++i ) os << ", " << as_printable(that[i]);
        return os << ')';
      }
    }
  };
  ...
  template<scalar_value S> wide(S const&) -> wide<S, expected_width_v<S>>;
  ...
  //==============================================================================================
  template<std::size_t I, eve::product_type T, width_type N>
  EVE_FORCEINLINE auto& get(wide<T, N>& w) noexcept
  {
    return kumi::get<I>(w.storage());
  }

  template<std::size_t... Idx, eve::product_type T, width_type N>
#if !defined(EVE_DOXYGEN_INVOKED)
...
// Product type Support
//================================================================================================
template<std::size_t I, eve::product_type T, eve::width_type N>
struct std::tuple_element<I, eve::wide<T, N>>
    : std::tuple_element<I, typename eve::wide<T, N>::storage_type>
{};

template<eve::product_type T, eve::width_type N>
