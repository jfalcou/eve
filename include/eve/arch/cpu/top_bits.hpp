...
template <logical_simd_value Logical>
EVE_FORCEINLINE Logical  constexpr to_logical(eve::top_bits<Logical> mmask) noexcept
{
  ...
  else if constexpr ( x86_abi<abi> )
  {
    ...
    constexpr auto bits_per_element = top_bits<Logical>::bits_per_element;
    constexpr auto element_mask = detail::set_lower_n_bits<bits_et>(bits_per_element);
