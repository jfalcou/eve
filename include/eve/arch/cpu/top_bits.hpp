...
namespace eve
{
  ...
  struct top_bits
  {
   ...
   public:
    ...
    EVE_FORCEINLINE explicit top_bits(ignore_none_ i)
    {
    ...
    EVE_FORCEINLINE explicit top_bits(ignore_all_ i)
    {
      if constexpr( !is_aggregated )  storage = storage_type{0};
      else                            storage[0] = storage[1] = top_bits<half_logical>{i};
    }

    EVE_FORCEINLINE explicit top_bits(keep_last ignore)
    {
      if constexpr( !is_aggregated )
      {
    ...
    EVE_FORCEINLINE explicit  top_bits(keep_first ignore)
    {
      if constexpr( !is_aggregated )
      {
    ...
    EVE_FORCEINLINE explicit  top_bits(ignore_first ignore)
                                      : top_bits( keep_last{static_size - ignore.count_} )
    {}

    EVE_FORCEINLINE explicit top_bits(ignore_last ignore)
                                      : top_bits( keep_first{static_size - ignore.count_} )
    {}

    EVE_FORCEINLINE explicit top_bits(keep_between ignore)
    {
      if constexpr( !is_aggregated )
      {
    ...
    EVE_FORCEINLINE explicit top_bits(ignore_extrema ignore)
    ...
    EVE_FORCEINLINE explicit top_bits(Ignore) : top_bits{ignore_all} {}

    template <relative_conditional_expr Ignore>
      requires(Ignore::is_complete && Ignore::is_inverted)
    EVE_FORCEINLINE explicit top_bits(Ignore) : top_bits{ignore_none} {}
    ...
    //! setter
    EVE_FORCEINLINE void set(std::ptrdiff_t i, bool x)
    {
      if constexpr ( !is_aggregated )
      {
    ...
    // getter
    EVE_FORCEINLINE bool get(std::ptrdiff_t i) const
    {
    ...
    EVE_FORCEINLINE auto as_int() const
    ...
    EVE_FORCEINLINE std::strong_ordering operator<=>(const top_bits&) const = default;

    template<typename X>
    EVE_FORCEINLINE friend auto operator==(top_bits const& a, top_bits<X> const& b)
    {
    ...
    EVE_FORCEINLINE friend top_bits operator~(const top_bits& x)
    {
