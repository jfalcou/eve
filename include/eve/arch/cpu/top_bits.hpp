  ...
  struct top_bits
  {
   ...
   private:
    ...
    static constexpr bool is_cheap_impl()
    {
      ...
      if constexpr ( ppc_abi<abi_type> ) return true;

      if constexpr ( arm_abi<abi_type> )
      {
        if constexpr ( static_size == 1 ) return true;
        if constexpr ( static_size * sizeof(scalar_type) <= 4 ) return true;
        if constexpr ( current_api >= eve::asimd )
        {
...
EVE_FORCEINLINE Logical to_logical(eve::top_bits<Logical> mmask) noexcept
{
  ...
  else if constexpr ( x86_abi<abi> )
  {
    ...
    using fit_wide  = wide<bits_et, expected_width_v<bits_et, abi_t>>;
  ...
  else
  {
    ...
    {
      using bits_et   = element_type_t<bits_wide>;
      using fit_wide  = logical<wide<bits_et, expected_width_v<bits_et, abi_t>>>;
