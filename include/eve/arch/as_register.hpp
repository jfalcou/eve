...
namespace eve
{
  ...
  template<typename T, size S, typename ABI>
  consteval auto as_register(as<T> t, as_type_t<size, S> n, ABI abi)
  {
  ...
  template<typename T, size S, typename ABI>
  using as_register_t = decltype(as_register(as<T>{}, as_type_t<size, S>{}, ABI{}));

  template<typename T, size S, typename ABI>
  consteval auto as_logical_register(as<T> t, as_type_t<size, S> n, ABI abi)
  {
  ...
  template<typename T, size S, typename ABI>
  using as_logical_register_t = decltype(as_logical_register(as<T>{}, as_type_t<size, S>{}, ABI{}));
}
