...
namespace eve::detail
{
  ...
  template<typename T, typename Sign> struct as_integer<logical<T>, Sign>
  {
    using type = logical<as_integer_t<T, Sign>>;
  };

  template<typename T, typename N, typename ABI> struct as_floating_point<wide<T, N, ABI>>
  {
    using type = wide<as_floating_point_t<T>, N>;
  };

  template<typename T> struct as_floating_point<logical<T>>
  {
