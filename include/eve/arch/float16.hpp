  ...
  namespace _
  {
    ...
    struct float16_t {
      ...
      public:
        ...
        constexpr EVE_ABI explicit operator bool()               const noexcept { return into<bool>(); }

        constexpr EVE_ABI explicit operator float()              const noexcept { return into<float>(); }
        constexpr EVE_ABI explicit operator double()             const noexcept { return into<double>(); }
        constexpr EVE_ABI explicit operator long double()        const noexcept { return into<long double>(); }

        constexpr EVE_ABI explicit operator char()               const noexcept { return into<char>(); }
        constexpr EVE_ABI explicit operator signed char()        const noexcept { return into<signed char>(); }
        constexpr EVE_ABI explicit operator unsigned char()      const noexcept { return into<unsigned char>(); }
        constexpr EVE_ABI explicit operator short()              const noexcept { return into<short>(); }
        constexpr EVE_ABI explicit operator unsigned short()     const noexcept { return into<unsigned short>(); }
        constexpr EVE_ABI explicit operator int()                const noexcept { return into<int>(); }
        constexpr EVE_ABI explicit operator unsigned int()       const noexcept { return into<unsigned int>(); }
        constexpr EVE_ABI explicit operator long()               const noexcept { return into<long>(); }
        constexpr EVE_ABI explicit operator unsigned long()      const noexcept { return into<unsigned long>(); }
        constexpr EVE_ABI explicit operator long long()          const noexcept { return into<long long>(); }
        constexpr EVE_ABI explicit operator unsigned long long() const noexcept { return into<unsigned long long>(); }

        constexpr EVE_ABI float16_t& operator+=(float16_t const& other) noexcept
        {
        ...
        constexpr EVE_ABI float16_t operator+(float16_t const& other) const noexcept
        {
        ...
        constexpr EVE_ABI float16_t operator-() const noexcept
        {
          return float16_t{-into<float>()};
        }

        constexpr EVE_ABI float16_t& operator++() noexcept
        {
          *this += float16_t{1.0f};
          return *this;
        }

        constexpr EVE_ABI float16_t operator++(int) noexcept
        {
          float16_t tmp = *this;
          ++(*this);
          return tmp;
        ...
        constexpr EVE_ABI std::partial_ordering operator<=>(float16_t const& other) const noexcept
        {
          return _::emulated_fp16_compare(data, other.data);
        }

        constexpr EVE_ABI bool operator==(float16_t const& other) const noexcept
        {
    ...
    template <typename T>
    constexpr EVE_ABI float16_t operator+(T const& lhs, float16_t const& rhs) noexcept
