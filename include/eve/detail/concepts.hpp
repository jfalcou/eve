...
namespace std
{
  ...
  template<typename F, typename... Ts> concept invocable = requires(F &&f, Ts &&...ts)
  {
  ...
  template<typename T> struct indirectly_readable_traits<T *>
  {
    using value_type = std::remove_cv_t<T>;
  };

  template<typename T> requires std::is_array_v<T> struct indirectly_readable_traits<T>
  {
    using value_type = std::remove_cv_t<std::remove_extent_t<T>>;
  };

  template<typename T> struct indirectly_readable_traits<const T> : indirectly_readable_traits<T>
  {
  };
  ...
  template<typename T> requires std::is_object_v<T> struct incrementable_traits<T *>
  {
    using difference_type = std::ptrdiff_t;
  };

  template<typename T> struct incrementable_traits<const T> : incrementable_traits<T>
  {
  };
  ...
  template<typename I> concept input_iterator = requires(I i)
  {
    // Requires c++20 stdc++
