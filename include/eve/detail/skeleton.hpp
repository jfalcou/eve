...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE constexpr decltype(auto) at(T &&t, std::size_t i) noexcept
      requires(has_random_access<T>)
  {
    return std::forward<T>(t)[i];
  }

  template<typename T>
  EVE_FORCEINLINE constexpr decltype(auto) at(T &&t, std::size_t) noexcept
      requires(!has_random_access<T>)
  {
  ...
  template<typename F, typename... Ts> struct wide_result
  {
  ...
  template<typename Out, typename... Bs> EVE_FORCEINLINE auto rebuild(Bs const &...ps) noexcept
  {
  ...
  {
    template<typename T> static EVE_FORCEINLINE constexpr auto subpart(T &t) noexcept
    {
      if constexpr( simd_value<T> )
        return t.storage().template get<I>();
      else
        return t;
    }

    template<typename T> static EVE_FORCEINLINE constexpr auto subpart(T const &t) noexcept
    {
      if constexpr( simd_value<T> )
        return t.storage().template get<I>();
      else
  ...
  template<typename T> constexpr auto replication() noexcept
  {
    if constexpr( scalar_value<T> )
      ...
      return T::storage_type::replication;
    else
      return 1;
  }
  ...
  template<typename Func, typename... Ts> EVE_FORCEINLINE auto aggregate(Func &&f, Ts &&...ts)
  {
    ...
    if constexpr( is_fully_sliceable<wide_t, std::decay_t<Ts>...>() )
    {
      ...
      detail::apply<replication<wide_t>()>([&]<typename... I>(I const &...) {
        ((aggregate_step<I::value>::perform(
             std::forward<Func>(f), that.storage(), std::forward<Ts>(ts)...)),
         ...);
      });
