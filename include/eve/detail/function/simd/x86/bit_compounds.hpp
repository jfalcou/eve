...
**/
//==================================================================================================
#ifndef EVE_DETAIL_FUNCTION_SIMD_X86_BIT_COMPOUNDS_HPP_INCLUDED
#define EVE_DETAIL_FUNCTION_SIMD_X86_BIT_COMPOUNDS_HPP_INCLUDED
...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE decltype(auto) self_bitand( wide<T,N,sse_>& self, U const& other )
  ...
  template<scalar_value T, value U, typename N>
  EVE_FORCEINLINE decltype(auto) self_bitand( wide<T,N,avx_>& self, U const& other )
  ...
  //================================================================================================
  template<scalar_value T, value U, typename N>
  EVE_FORCEINLINE decltype(auto) self_bitor( wide<T,N,sse_>& self, U const& other )
  requires( (sizeof(wide<T,N,sse_>) == sizeof(U)) || (sizeof(T) == sizeof(U)) )
  {
    using type = wide<T, N, sse_>;
    ...
    else if constexpr( simd_value<U> && sizeof(self) == sizeof(other) )
    {
      auto bit_other = detail::bit_cast_(EVE_RETARGET(cpu_), other , as_<type>{});
           ...
           if constexpr(std::same_as<T, float>)  self = _mm_or_ps(self, bit_other);
      else if constexpr(std::same_as<T, double>) self = _mm_or_pd(self, bit_other);
      else if constexpr(std::integral<T>)        self = _mm_or_si128(self, bit_other);
      return self;
    }
  }

  template<scalar_value T, value U, typename N>
  EVE_FORCEINLINE decltype(auto) self_bitor( wide<T,N,avx_>& self, U const& other )
  requires( (sizeof(wide<T,N,avx_>) == sizeof(U)) || (sizeof(T) == sizeof(U)) )
  {
    using type = wide<T, N, avx_>;
    ...
    else if constexpr( simd_value<U> && sizeof(self) == sizeof(other) )
    {
      auto bit_other = detail::bit_cast_(EVE_RETARGET(cpu_), other , as_<type>{});
           ...
           if constexpr(std::same_as<T, float>)  self = _mm256_or_ps(self, bit_other);
      else if constexpr(std::same_as<T, double>) self = _mm256_or_pd(self, bit_other);
      else if constexpr( integral_value<T> )
      {
        if constexpr(current_api >= avx2) self = _mm256_or_si256(self, bit_other);
        else  self = _mm256_castps_si256(_mm256_or_ps ( _mm256_castsi256_ps(self)
                                                      , _mm256_castsi256_ps(bit_other)
                                                      )
                                        );
      }
  ...
  //================================================================================================
  template<scalar_value T, value U, typename N>
  EVE_FORCEINLINE decltype(auto) self_bitxor( wide<T,N,sse_>& self, U const& other )
  requires( (sizeof(wide<T,N,sse_>) == sizeof(U)) || (sizeof(T) == sizeof(U)) )
  {
    using type = wide<T, N, sse_>;
  ...
  template<scalar_value T, value U, typename N>
  EVE_FORCEINLINE decltype(auto) self_bitxor( wide<T,N,avx_>& self, U const& other )
  requires( (sizeof(wide<T,N,avx_>) == sizeof(U)) || (sizeof(T) == sizeof(U)) )
  {
    using type = wide<T, N, avx_>;
