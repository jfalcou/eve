...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto slice(wide<double, N, sse_> const &a,
                             Slice const &) noexcept requires(wide<double, typename N::split_type>,
                                                              If<(N::value > 1)>)
  {
    using that_t = wide<double, typename N::split_type>;

    if constexpr(Slice::value) { return that_t(_mm_shuffle_pd(a, a, 0x01)); }
    else
    {
  ...
  template<typename N, typename Slice>
  EVE_FORCEINLINE auto slice(wide<float, N, sse_> const &a,
                             Slice const &) noexcept requires(wide<float, typename N::split_type>,
                                                              If<(N::value > 1)>)
  {
