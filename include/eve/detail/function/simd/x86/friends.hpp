...
namespace eve::detail
{
  ...
  self_less(wide<T,N,ABI> const &v0, wide<T,N,ABI> const &v1) noexcept
  {
    constexpr auto c = categorize<wide<T, N, ABI>>();
    ...
    else
    {
      ...
      else
      {
        ...
        [[maybe_unused]]  auto unsigned_cmp = [](auto v0, auto v1)
        {
          ...
          return bit_cast( (bit_cast(v0,as(sm))-sm) < (bit_cast(v1,as(sm))-sm),as_<l_t>{});
        };
        ...
        else  if constexpr( c == category::int64x2  )             return map(lt, v0, v1);
        ...
        else  if constexpr( c == category::uint64x2 )             return map(lt,v0,v1);
        ...
        else                                                      return aggregate(lt,v0,v1);
      }
    }
  }
}
