...
namespace eve::_
{
  ...
  //================================================================================================
  template<callable_options O, scalar_value T, cardinal_type N>
  ...
  requires (N > 1) && x86_abi<abi_t<T,N>>
  {
    ...
    else if constexpr( match(c,category::int16x8, category::uint16x8) )
    {
      ...
      {
        wide<T, 8> v01 = _mm_unpacklo_epi16(v0.storage(),v1.storage());
        if constexpr(N == 2 )
        {
    ...
    else if constexpr( match(c,category::int8x16, category::uint8x16) )
    {
      ...
      {
        wide<T, 16> v01 = _mm_unpacklo_epi8(v0.storage(),v1.storage());
  ...
  requires (N > 1) && x86_abi<abi_t<T,N>>
  {
    ...
    if constexpr( c == category::float32x4 )
    {
      ...
      else if constexpr(N == 4)
      {
        ...
        auto dy   = eve::bit_cast(v1, eve::as<wide<double, 2>>());
             dy   = _mm_unpackhi_pd(dy,eve::bit_cast(v0, eve::as<wide<double, 2>>()));
