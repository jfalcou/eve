  ...
  EVE_FORCEINLINE auto to_bitmap(sse2_ const &, logical<wide<T, N, ABI>> const &p) noexcept
  {
    ...
    if constexpr( std::is_same_v<ABI, x86_128_> )
    {
      if constexpr( std::is_same_v<T, float> )
        return type(_mm_movemask_ps(p.mask()));
      else if constexpr( std::is_same_v<T, double> )
        return type(_mm_movemask_pd(p.mask()));
      else if constexpr( std::is_integral_v<T> )
      {
        if constexpr( sizeof(T) == 1 )
          return type(_mm_movemask_epi8(p.mask()));
        else if constexpr( sizeof(T) == 2 )
        {
          if constexpr( current_api >= ssse3 )
          {
            __m128i mask = _mm_setr_epi8(0x01,
                                         0x03,
                                         0x05,
                                         0x07,
                                         0x09,
    ...
    {
      if constexpr( std::is_same_v<T, float> )
        return type(_mm256_movemask_ps(p.mask()));
      else if constexpr( std::is_same_v<T, double> )
        return type(_mm256_movemask_pd(p.mask()));
      else if constexpr( sizeof(T) >= 4 )
      {
