...
namespace eve::_
{
  template<callable_options O, typename T, typename N, typename Slice>
   auto slice_(EVE_REQUIRES(neon128_), O const&, wide<T, N> a, Slice) noexcept
      requires arm_abi<abi_t<T, N>>
  {
    ...
    if constexpr( Slice::value )
    {
      ...
      else  if constexpr( c == category::float16x8 )  return type{vget_high_f16(a)};
    ...
    else
    {
      ...
      else  if constexpr( c == category::float16x8 )  return type{vget_low_f16(a)};
  ...
  template<callable_options O, typename T, typename N>
   auto slice_(EVE_REQUIRES(neon128_), O const&, wide<T, N> a) noexcept
      requires arm_abi<abi_t<T, N>>
  {
