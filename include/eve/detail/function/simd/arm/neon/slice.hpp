...
namespace eve::detail
{
      ...
      requires arm_abi<abi_t<T, N>>
  {
    ...
    {
      using type = wide<translate_t<T>, N>;
      return bit_cast(slice(translate(a), s), as<type> {});
    }
    else
    {
      ...
      requires arm_abi<abi_t<T, N>>
  {
    if constexpr(has_plain_translation<T>)
    {
      auto [l, h] = slice(translate(a));
      using target_type = as<wide<T, typename N::split_type>>;
      return std::array{bit_cast(l, target_type{}), bit_cast(h, target_type{})};
    }
    else
    {
