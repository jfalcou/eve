...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> load_( EVE_SUPPORTS(cpu_), C const& c, unsafe_type const&
  ...
  requires(std::same_as<abi_t<T, N>, bundle_>)
  {
    ...
    {
      kumi::for_each( [=]<typename M>(M& m, auto p) { m = load(c,unsafe,as<M>{},p); }
                     , that.storage(), ptr
                     );
    }
