...
{
  template<simd_value Wide> EVE_FORCEINLINE auto gather_high_(EVE_SUPPORTS(cpu_), Wide v) noexcept
  {
    // Well, we dunno how to do this, so we just reshuffle
    return v[gather_high_n<cardinal_v<Wide>>];
  }

  template<typename Wide, shuffle_pattern Pattern>
  EVE_FORCEINLINE auto gather_high_( EVE_SUPPORTS(cpu_)
  ...
  EVE_FORCEINLINE auto gather_high_(EVE_SUPPORTS(cpu_), Wide v, Pattern p) noexcept
  {
