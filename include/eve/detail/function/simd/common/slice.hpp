...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto slice_(EVE_REQUIRES(cpu_), O const&, logical<wide<T, N>> const& a) noexcept
  {
  ...
  EVE_FORCEINLINE auto slice_(EVE_REQUIRES(cpu_), O const&, logical<wide<T, N>> const &a, Slice const &s) noexcept
  {
    if constexpr (is_native_v<abi_t<T, N>> && abi_t<T, N>::is_wide_logical)
    {
       using l_t = logical<wide<T, typename N::split_type>>;
  ...
  EVE_FORCEINLINE auto slice_(EVE_REQUIRES(cpu_), O const&, wide<T, N> const &a) noexcept
  ...
  EVE_FORCEINLINE auto slice_(EVE_REQUIRES(cpu_), O const&, wide<T, N> const &a, Slice const &s) noexcept
      requires non_native_abi<abi_t<T, N>>
  {
