...
namespace eve::_
{
  ...
  template<typename P> EVE_FORCEINLINE auto slice_impl(P const &a) noexcept
  {
    ...
    if constexpr( aggregated_abi<abi_t> )  return a.storage().slice();
