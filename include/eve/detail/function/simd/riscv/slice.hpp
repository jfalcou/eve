...
requires rvv_abi<abi_t<T, N>>
{
  ...
  constexpr auto out_lmul = rvv_lmul_v<T, N / 2>;
  if constexpr( in_lmul == out_lmul ) return a.storage();
  else { return call_simd_cast(a, as<wide<T, N / 2>> {}); }
}
...
requires rvv_abi<abi_t<T, N>>
{
  constexpr auto shift_size = N / 2;
  wide<T, N>     res        = __riscv_vslidedown(a, shift_size, N);
  constexpr auto in_lmul    = rvv_lmul_v<T, N>;
  constexpr auto out_lmul   = rvv_lmul_v<T, N / 2>;
  ...
  else
  {
    ...
    return call_simd_cast(res, as<wide<T, N / 2>> {});
  }
}
...
requires rvv_abi<abi_t<T, N>>
{
  return call_simd_cast(a, as<logical<wide<T, N / 2>>> {});
}
...
requires rvv_abi<abi_t<T, N>>
{
  ...
  return call_simd_cast(neq, as<logical<wide<T, N / 2>>> {});
}
