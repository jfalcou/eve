...
{
  template<typename Type> struct is_wide : std::false_type
  {
  };

  template<typename Type, typename N> struct is_wide<eve::wide<Type, N>> : std::true_type
  {
  };

  template<typename Type> requires(rebindable<Type>) struct is_wide<Type>
  {
