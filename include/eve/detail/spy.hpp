...
namespace spy::detail
{
  ...
  template<archs Arch> std::ostream &operator<<(std::ostream &os, arch_info<Arch> const &)
  {
    if( Arch == archs::x86_ )
      return os << "X86";
    if( Arch == archs::amd64_ )
      return os << "AMD64";
    if( Arch == archs::ppc_ )
      return os << "PowerPC";
    if( Arch == archs::arm_ )
...
namespace spy::detail
{
  ...
  template<int M1, int N1, int P1, int M2, int N2, int P2>
  constexpr bool operator!=(version_id<M1, N1, P1> const &v1,
                            version_id<M2, N2, P2> const &v2) noexcept
  {
    return !(v1 == v2);
  }
  template<int M1, int N1, int P1, int M2, int N2, int P2>
  ...
  template<int M1, int N1, int P1, int M2, int N2, int P2>
  constexpr bool operator<=(version_id<M1, N1, P1> const &a,
                            version_id<M2, N2, P2> const &b) noexcept
  {
    return !(a > b);
  }
  template<int M1, int N1, int P1, int M2, int N2, int P2>
...
namespace spy::detail
{
  ...
  {
    if( C == compilers::msvc_ )
      return os << "Microsoft Visual Studio " << c.version;
    if( C == compilers::intel_ )
      return os << "Intel icpc " << c.version;
    if( C == compilers::clang_ )
      return os << "clang " << c.version;
    if( C == compilers::gcc_ )
      return os << "g++ " << c.version;
...
namespace spy
{
  using data_model_type =
      detail::data_model_info<sizeof(short), sizeof(int), sizeof(long), sizeof(void *)>;
...
namespace spy::detail
{
  ...
  {
    if( c.vendor == libC::cloudabi_ )
      return os << "CloudABI Standard C Library " << c.version;
    if( c.vendor == libC::uc_ )
      return os << "uClibc Standard C Library " << c.version;
    if( c.vendor == libC::vms_ )
      return os << "VMS Standard C Library " << c.version;
    if( c.vendor == libC::zos_ )
      return os << "z/OS Standard C Library " << c.version;
    if( c.vendor == libC::gnu_ )
      return os << "GNU Standard C Library " << c.version;
...
#  define SPY_LIBC_IS_ZOS
  using libc_type = detail::zos_t<(__LIBREL__ & 0xF000000)> > 24, (__LIBREL__ & 0xFF0000) >> 16,
        (__LIBREL__ & 0xFFFF) > ;
#else
...
namespace spy::detail
{
  ...
  {
    if( SLib == stdlib::libcpp_ )
      return os << "libc++ Standard C++ Library";
    if( SLib == stdlib::gnucpp_ )
      return os << "GNU Standard C++ Library";
    return os << "Undefined Standard C++ Library";
  }
...
#if !defined(SPY_SIMD_DETECTED)                                                                    \
    && (defined(__ARM_NEON__) || defined(_M_ARM) || defined(__aarch64__))
...
namespace spy::detail
{
  ...
  struct simd_info
  {
    ...
    {
      if constexpr( Version == simd_version::sse1_ )
        os << "X86 SSE";
        ...
        os << "X86 AVX512";
      else if constexpr( Version == simd_version::vmx_ )
        os << "PPC VMX";
      else if constexpr( Version == simd_version::vsx_ )
        os << "PPC VSX";
    ...
    constexpr bool operator==(simd_info<OInsSetArch> const &) const noexcept
    {
...
{
#if defined(SPY_SIMD_DETECTED)
  constexpr inline auto simd_instruction_set =
      detail::simd_info<SPY_SIMD_VENDOR, SPY_SIMD_DETECTED> {};
#else
...
namespace spy::detail
{
  ...
  template<systems OpSys> std::ostream &operator<<(std::ostream &os, os_info<OpSys> const &)
  {
    if( OpSys == systems::android_ )
      return os << "Android";
    if( OpSys == systems::bsd_ )
      return os << "BSD";
    if( OpSys == systems::cygwin_ )
      return os << "Cygwin";
    if( OpSys == systems::ios_ )
      return os << "iOs";
    if( OpSys == systems::linux_ )
      return os << "Linux";
    if( OpSys == systems::macos_ )
      return os << "MacOs";
    if( OpSys == systems::unix_ )
      return os << "UNIX";
    if( OpSys == systems::windows_ )
      return os << "Windows";
