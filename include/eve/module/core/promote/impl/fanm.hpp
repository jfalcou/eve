...
namespace eve::detail
{
  ...
  fanm_(EVE_SUPPORTS(cpu_), promote_type const&, T const& a, U const& b, V const& c) noexcept
  {
    ...
    {
      return fanm(convert(a, cv_t), convert(b, cv_t), convert(c,cv_t));
    }
    else
      return apply_over(fanm, convert(a, cv_t), convert(b, cv_t), convert(c, cv_t));
  }
}
