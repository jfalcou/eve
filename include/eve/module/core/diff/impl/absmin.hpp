...
namespace eve::detail
{
  ...
  auto absmin_(EVE_SUPPORTS(cpu_), diff_type<N>
  ...
  EVE_FORCEINLINE auto absmin_(EVE_SUPPORTS(cpu_), C const &cond, diff_type<N> const &
                               , T const &t, Ts ... ts ) noexcept
  {
    return mask_op(  cond, eve::diff_nth<N>(eve::absmin), t, ts...);
  } 
}
