...
struct almost_type : decorator_
{
  ...
  {
    return [f](auto&&...args) { return f(almost_type {}, EVE_FWD(args)...); };
  }
};
...
//! @var almost
//!
//! @brief  Higher-order @callable imbuing a tolerant to little errors semantic onto other
//! @callable{s}.
//!
...
struct definitely_type : decorator_
{
  template<typename Function> constexpr EVE_FORCEINLINE auto operator()(Function f) const noexcept
  {
    return [f](auto&&...args) { return f(definitely_type {}, EVE_FWD(args)...); };
  }
};
...
//! @var definitely
//!
//! @brief  Higher-order @callable imbuing a tolerant to small errors semantic onto other
//! @callable{s}.
//!
//! @groupheader{Synopsis}
//!
...
struct tolerant_type : decorator_
{
  template<typename Function> constexpr EVE_FORCEINLINE auto operator()(Function f) const noexcept
  {
    return [f](auto&&...args) { return f(tolerant_type {}, EVE_FWD(args)...); };
  }
};
