...
{
  template<typename T> EVE_FORCEINLINE constexpr auto horn(T const &) noexcept
  {
    return eve::zero(eve::as<T>());
  }

  template<typename T, auto Coef> EVE_FORCEINLINE constexpr auto horn(T const &) noexcept
  {
