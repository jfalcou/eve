...
**/
//==================================================================================================
#ifndef EVE_MODULE_CORE_DETAIL_SCALAR_IEEE_754_REM_PIO2_HPP_INCLUDED
#define EVE_MODULE_CORE_DETAIL_SCALAR_IEEE_754_REM_PIO2_HPP_INCLUDED
...
#define SET_LOW_WORD(d,v)                                               \
  ...
  EVE_FORCEINLINE
  std::int32_t __ieee754_rem_pio2(double x, double *y) noexcept
  {
  ...
  recompute:
    ...
    for(i=0,j=jz,z=q[jz];j>0;i++,j--) {
      fw    =  (double)((std::int32_t)(twon24* z));
      iq[i] =  (std::int32_t)(z-two24*fw);
      z     =  q[j-1]+fw;
    }

    /* compute n */
    z  = std::ldexp(z,q0); //eve::pedantic_(ldexp)(z,q0);        /* actual value of z */ //TODO
    ...
    if(ih>0) {    /* q > 0.5 */
      ...
      if(ih==2) {
        z = one - z;
        if(carry!=0) z -= std::ldexp(one, q0); // bs::pedantic_(ldexp)(one,q0); TODO
      }
    }
    ...
    if(z==0.0) {
      ...
      z = std::ldexp(z, -q0); // bs::pedantic_(ldexp)(z,-q0); TODO
      if(z>=two24) {
        fw = (double)((std::int32_t)(twon24*z));
    ...
    fw = std::ldexp(one, q0); //bs::pedantic_(ldexp)(one,q0); TODO
    for(i=jz;i>=0;i--) {
