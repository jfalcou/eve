...
struct swap_adjacent_t
{
  ...
  static consteval std::ptrdiff_t level(eve::as<T> tgt, eve::fixed<G> g)
  {
    ...
    constexpr std::size_t fund_size = eve::fundamental_cardinal_v<std::uint8_t>;

    if constexpr ( current_api >= sve )
    {
      if constexpr( arithmetic_simd_value<T> )
      {
        if constexpr (g_size <= 4 || g_size == fund_size / 2) return 2;
        else return 3;
      }
      else
      {
        if constexpr ( g_size == 8 && fund_size == 16 ) return 2;
    ...
    else if constexpr ( current_api == avx512 && logical_simd_value<T> )
    {
      return level(detail::mask_type(tgt), g) + 4;
    }

    else if constexpr( current_api == avx && reg_size >= 32 && g_size <= 2 )
    {
      ...
      if constexpr ( reg_size > 32 ) return half_l;
      else return detail::idxm::add_shuffle_levels({half_l, half_l, 4});
    }
