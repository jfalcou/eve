  ...
  static constexpr std::ptrdiff_t level(eve::as<T> tgt, eve::fixed<G> g)
  {
    ...
    if constexpr( current_api >= sve )
    {
      if constexpr( !logical_value<T> )
      {
        ...
        if constexpr( is_expected_cardinal && g_size <= 8 ) return 2;
        if constexpr( is_expected_cardinal && g_size == reg_size / 2 ) return 2;
        return 3;
      }
      if constexpr( is_expected_cardinal && g_size <= 8 ) return 2;
    ...
    if constexpr( current_api >= vmx ) return 3;

    if constexpr( current_api == avx512 && logical_value<T> ) { return level(_::mask_type(tgt), g) + 4; }

    if constexpr( current_api >= avx2 && reg_size >= 32 )
    {
      if constexpr( g_size >= 16 ) return 2;
      if constexpr( g_size >= 8 ) return reg_size == 64 ? 3 : 2;
      if constexpr( g_size >= 4 ) return 3;
      if constexpr( g_size == 2 && current_api >= avx512 ) return 3;
      return 5;
    }

    if constexpr( current_api == avx && reg_size >= 32 )
    {
      if constexpr( g_size >= 16 ) return 2;
      if constexpr( g_size >= 4 ) return 4;
      if constexpr( g_size == 2 && current_api >= avx512 ) return 3;
      return 9;
    }

    if constexpr( g_size >= 4 ) return 2;
    if constexpr( g_size == 2 && reg_size <= 8 ) return 2;

    if constexpr( current_api >= ssse3 ) return 3;

    if constexpr( g_size == 2 ) return 6;

    //chars on sse2
    if constexpr( reg_size == 2 ) return 6;

    //swap chars + reverse shorts
    if constexpr( reg_size <= 8 ) return 8;
    return 12;
  }
};
