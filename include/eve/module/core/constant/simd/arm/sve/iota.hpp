...
{
  template<typename T>
  EVE_FORCEINLINE T sveindex(element_type_t<T> start, element_type_t<T> step)
  {
  ...
  EVE_FORCEINLINE T iota_(EVE_REQUIRES(sve_), callable_options auto const& opts, as<T> tgt)
  {
