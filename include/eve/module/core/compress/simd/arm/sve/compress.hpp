...
namespace eve::_
{
  ...
  {
    constexpr std::size_t min_size = kumi::min_flat( kumi::as_tuple_t<T>{}, [](auto m) { return sizeof(m); });
    constexpr std::size_t max_size = kumi::max_flat( kumi::as_tuple_t<T>{}, [](auto m) { return sizeof(m); });

    // We can compress 1 full register of at least 4 bytes elements.
    if constexpr (min_size == 1U                  ) return N() > eve::expected_cardinal_v<std::int32_t> / 4;
    if constexpr (min_size == 2U || max_size == 8U) return N() > eve::expected_cardinal_v<std::int32_t> / 2;
