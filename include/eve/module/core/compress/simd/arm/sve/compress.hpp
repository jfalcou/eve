...
namespace eve::_
{
  template <typename U, width_type N>
  ...
  template<typename T, width_type N> constexpr bool compress_sve_should_split()
  {
    ...
    if (min_size == 1U                  ) return N > eve::expected_width_v<std::int32_t> / 4;
    if (min_size == 2U || max_size == 8U) return N > eve::expected_width_v<std::int32_t> / 2;
    return N > eve::expected_width_v<std::int32_t>;
  }
