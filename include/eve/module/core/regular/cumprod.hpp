...
namespace eve
{
  ...
  struct cumprod_t : callable<cumprod_t, Options, widen_option>
  {
    template<typename T>
...
//! @{
//!   @var cumprod
//!   @brief `tuple_callable` utility to convert a pack of values into
//!     a tuple of the cumulative prod of its values.
//!
...
//!     3. same of 1. or 2., but upgrading the computation of elements of the result.
//!
//!  @groupheader{Example}
  ...
  namespace _
  {
    ...
    EVE_FORCEINLINE constexpr auto cumprod_(EVE_REQUIRES(cpu_), O const & o, PT tup) noexcept
    {
      ...
      else
      {
        ...
        PT htupl =  kumi::pop_back(kumi::push_front(htup, r_t(1)));
