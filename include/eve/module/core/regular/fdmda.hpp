...
namespace eve
{
  ...
  {
    template<value T,  value U, value V,  value W,  value X>
    requires(eve::same_lanes_or_scalar<T, U, V, W, X>)
    constexpr EVE_FORCEINLINE common_value_t<T, U, V, W, X> operator()(T a, U b, V c, W d, X e) const
    { return EVE_DISPATCH_CALL(a, b, c, d, e); }
      ...
      else if constexpr(floating_value<T> && (O::contains(lower) || O::contains(upper) ))
      {
        ...
        else
        {
          ...
          if constexpr(O::contains(upper))
          {
            ...
            return add[o](add[o][is_finite(err)](dop, err), e);
          }
          else
          {
