...
namespace eve::detail
{
  ...
  bit_reverse_(EVE_SUPPORTS(cpu_), T x) noexcept
  {
    ...
    {
      x = bit_swap_adjacent(x, std::integral_constant<size_t, 1>());
      x = bit_swap_adjacent(x, std::integral_constant<size_t, 2>());
      x = bit_swap_adjacent(x, std::integral_constant<size_t, 4>());
      if constexpr(sizeof(T) >= 2)
        x = bit_swap_adjacent(x, std::integral_constant<size_t, 8>());
      else
        return x;
      if constexpr(sizeof(T) >= 4)
        x = bit_swap_adjacent(x, std::integral_constant<size_t, 16>());
      else
        return x;
      if constexpr(sizeof(T) ==  8)
        x = bit_swap_adjacent(x, std::integral_constant<size_t, 32>());
      return x;
    }
  ...
  template<unsigned_value T,  auto N>
  EVE_FORCEINLINE T
  bit_reverse_(EVE_SUPPORTS(cpu_), T x
              , std::integral_constant<size_t, N>) noexcept
  {
    using e_t =  element_type_t<T>;
    constexpr size_t S = sizeof(e_t)*8;
    if constexpr(N == 0u)        return x;
    else
    {
      auto y =  bit_reverse(x);
      if constexpr(2*N >= S)     return y;
      else if constexpr(N == 1u) return bit_swap_pairs(x, 0u, S-1u);
      else
      {
