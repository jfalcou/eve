...
lohi_(EVE_SUPPORTS(cpu_), T const& a0) noexcept
{
  using std::endian;
  if constexpr( has_native_abi_v<T> )
  {
    ...
    else
    {
      ...
      if constexpr( simd_value<T> )
      {
        ...
        auto uia0  = bit_cast(a0, as<ui_t>());
        auto constexpr p =
            as_pattern([](auto i, auto c) { return (i < c / 2) ? 2 * i : 2 * (i - c / 2) + 1; });
        auto z = eve::shuffle(uia0, p).slice();

        if constexpr( endian::native == endian::little )
          return kumi::tuple<vi_t, vi_t> {bit_cast(z[0], as<vi_t>()), bit_cast(z[1], as<vi_t>())};
        else
          return kumi::tuple<vi_t, vi_t> {bit_cast(z[1], as<vi_t>()), bit_cast(z[0], as<vi_t>())};
      }
      else
      {
        using si_t = downgrade_t<as_integer_t<elt_t, unsigned>>;
        ...
        if constexpr( endian::native == endian::little )
          return kumi::tuple<si_t, si_t> {z[0], z[1]};
