...
requires rvv_abi<abi_t<T, N>>
{
  ...
  else
  {
    if constexpr( !C::has_alternative )
    {
      ...
      if constexpr( match(c, category::uint_) ) return __riscv_vdivu_tumu(m, v, v, w, N::value);
      if constexpr( match(c, category::float_) ) return __riscv_vfdiv_tumu(m, v, v, w, N::value);
    }
