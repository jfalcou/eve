...
namespace eve::_
{
    ...
    requires (rvv_abi<abi_t<T, N>> && same_element_type<T, U>)
  {
    ...
    else
    {
      ...
      if      constexpr (match(c, category::int_))   return __riscv_vmslt(a, b, N);
      else if constexpr (match(c, category::uint_))  return __riscv_vmsltu(a, b, N);
      else if constexpr (match(c, category::float16) && !_::supports_fp16_vector_ops)
      {
        return apply_fp16_as_fp32(is_less, a, b);
      }
      else if constexpr (match(c, category::float_)) return __riscv_vmflt(a, b, N);
    }
  }
}
