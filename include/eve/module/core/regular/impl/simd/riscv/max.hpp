...
requires rvv_abi<abi_t<T, N>>
{
  ...
  else
  {
    if constexpr( !C::has_alternative )
    {
      ...
      if constexpr( match(c, category::int_) ) return __riscv_vmax_tumu(m, v, v, w, N::value);
      if constexpr( match(c, category::uint_) ) return __riscv_vmaxu_tumu(m, v, v, w, N::value);
    }
