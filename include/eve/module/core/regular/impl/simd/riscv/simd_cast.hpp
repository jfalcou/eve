...
{
  constexpr auto size_cmp = sizeof(In) <=> sizeof(Out);
  using i8_tgt_t          = wide_bytes<Out>;
  if constexpr( size_cmp == 0 ) return bit_cast(x, tgt);
  else if constexpr( size_cmp < 0 )
  {
    auto i8_x     = bit_cast(x, as<wide_bytes<In>> {});
