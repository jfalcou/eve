...
template<scalar_value T, cardinal_type N, scalar_value U, cardinal_type M>
...
template<callable_options O, scalar_value T, cardinal_type N, scalar_value U, cardinal_type M>
EVE_FORCEINLINE logical<wide<U, M>>
...
template<cardinal_type N, cardinal_type M>
...
requires(rvv_abi<typename In::abi_type> && rvv_abi<typename Out::abi_type>)
{
  ...
  using i8_in_t  = wide<std::int8_t, sizeof(In)>;
  using i8_tgt_t = wide<std::int8_t, sizeof(Out)>;
