...
convert_impl(EVE_SUPPORTS(rvv_), wide<T, N> const& v, as<T> const& tgt) noexcept
...
requires rvv_abi<abi_t<T, N>> && (sizeof(T) == sizeof(U))
{
  ...
  if constexpr( match(in_c, category::float_) )
  {
    ...
    if constexpr( match(out_c, category::uint_) ) return __riscv_vfcvt_rtz_xu(v, N::value);
  }
...
requires rvv_abi<abi_t<T, N>> && (sizeof(T) > sizeof(U))
{
  ...
  if constexpr( Scale == 2 )
  {
    ...
    if constexpr( match(in_c, category::float_) )
    {
      ...
      if constexpr( match(out_c, category::uint_) ) return __riscv_vfncvt_rtz_xu(v, N::value);
      if constexpr( match(out_c, category::float_) ) return __riscv_vfncvt_f(v, N::value);
    }
    if constexpr( match(in_c, category::int_) )
    {
      if constexpr( match(out_c, category::int_) ) return __riscv_vncvt_x(v, N::value);
      if constexpr( match(out_c, category::uint_) )
      {
    ...
    if constexpr( match(in_c, category::uint_) )
    {
      ...
      if constexpr( match(out_c, category::int_) )
      {
        auto part_done = convert(v, as<downgrade_t<T>> {});
        return convert(part_done, tgt);
      }
         ...
         && (riscv_rvv_dyn_::getLMUL<U>(N::value) <= 8)
{
  ...
  if constexpr( Scale == 2 )
  {
    if constexpr( match(in_c, category::int_) )
    {
      ...
      if constexpr( match(out_c, category::float_) ) return __riscv_vfwcvt_f(v, N::value);
    }
    if constexpr( match(in_c, category::uint_) )
    {
    ...
    if constexpr( match(in_c, category::float_) )
    {
      ...
      if constexpr( match(out_c, category::uint_) ) return __riscv_vfwcvt_rtz_xu(v, N::value);
      if constexpr( match(out_c, category::float_) ) return __riscv_vfwcvt_f(v, N::value);
    }
  }
  else
