...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto minimum_(EVE_REQUIRES(sve_), O const& opts, wide<T, N> v) noexcept
    requires sve_abi<abi_t<T, N>>
  {
    if constexpr (O::contains(splat2))
    {
      // Attempts to force the use of a broadcasting mov
      return wide<T, N>{ wide<T>{ minimum[opts.drop(splat2)].retarget(current_api, v) } };
    }
    else
    {
      // svminv is expensive, for small lanes count this approach is faster
