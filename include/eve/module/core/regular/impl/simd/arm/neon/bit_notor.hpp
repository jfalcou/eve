...
EVE_FORCEINLINE wide<T, N> bit_notor_(EVE_REQUIRES(neon128_),
                                      O const          &opts,
                                      wide<T, N> const &v0,
...
requires arm_abi<abi_t<T, N>>
{
  ...
  if constexpr( cat == category::int64x1 ) return vorn_s64(v1, v0);
  ...
  else  if constexpr( match(cat, category::float_) )
  {
    ...
    return bit_cast(bit_notor.behavior(current_api, opts, bit_cast(v0, tgt), bit_cast(v1, tgt)), as<wide<T, N>>{});
  }
}
}
