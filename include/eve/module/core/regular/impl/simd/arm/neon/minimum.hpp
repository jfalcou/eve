...
EVE_FORCEINLINE T
minimum_(EVE_SUPPORTS(neon128_), wide<T, N> v) noexcept requires arm_abi<abi_t<T, N>>
{
  ...
  else { return splat(minimum)(v).get(0); }
}

template<real_scalar_value T, typename N>
EVE_FORCEINLINE wide<T, N>
                minimum_(EVE_SUPPORTS(neon128_),
                         splat_type const&,
                         wide<T, N> const                &v) noexcept requires arm_abi<abi_t<T, N>>
{
  if constexpr( eve::current_api >= eve::asimd ) { return wide<T, N>(minimum(v)); }
  else
  {
