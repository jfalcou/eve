...
namespace eve::_
{
    ...
    requires arm_abi<abi_t<T, N>>
  {
    ...
    {
      return minimum.behavior(cpu_{}, opts, v);
    }
    else if constexpr (O::contains(splat))
    {
      if constexpr( eve::current_api >= eve::asimd )
      {
        return wide<T, N>{ minimum.retarget(current_api, v) };
      }
      else
      {
        constexpr auto pairwise_min = []<typename W>(W a, W b)
        {
    ...
    {
      // Regular case - return scalar minimum
      ...
      if constexpr( eve::current_api >= eve::asimd )
      {
        ...
        else if constexpr( N < ec ) return butterfly_reduction(v, eve::min).get(0);
        ...
        else if constexpr( c == category::float16x4 && fp16v) return vminv_f16(v);
        else if constexpr( c == category::float16x8 && fp16v) return vminvq_f16(v);
