...
namespace eve::_
{
    ...
    requires arm_abi<abi_t<T, N>>
  {
    ...
    else if constexpr (O::contains(splat))
    {
      ...
      {
        return wide<T, N>{ maximum.retarget(current_api, v) };
      }
      else
      {
        ...
        else if constexpr( std::same_as<abi_t<T, N>, arm_64_> )
        {
          if( N == expected_cardinal_v<T, abi_t<T, N>> )
          {
            ...
            if constexpr( N >= 4 ) s = pairwise_max(s, s);
            if constexpr( N >= 8 ) s = pairwise_max(s, s);
            return s;
          }
        ...
        {
          auto [l, h] = v.slice();
          if constexpr( N >= 4 ) l = pairwise_max(l, h);
          if constexpr( N >= 8 ) l = pairwise_max(l, l);
          if constexpr( N >= 16 ) l = pairwise_max(l, l);
          l = pairwise_max(l, l);
    ...
    else
    {
      ...
      {
        return maximum[splat].retarget(current_api, v).get(0);
      }
    }
  }
}
