...
namespace eve::_
{
  template<callable_options O, typename T, cardinal_type N, arithmetic_scalar_value S>
  ...
  template<callable_options O, typename T, cardinal_type N, std::ptrdiff_t S>
    ...
    requires arm_abi<abi_t<T, N>>
  {
    ...
    else if constexpr( c == category::int64x1  ) return vshl_n_s64 (w, S);
