...
namespace eve::_
{
  ...
  requires sve_abi<abi_t<T, N>>
   wide<T, N> fma_(EVE_REQUIRES(sve_), O const& opts, wide<T, N> a, wide<T, N> b, wide<T, N> c) noexcept
  {
  ...
  requires sve_abi<abi_t<T, N>>
   wide<T, N> fma_(EVE_REQUIRES(sve_), C cond, O const& opts, wide<T,N> a, wide<T,N> b, wide<T,N> c) noexcept
  {
    if constexpr(O::contains(lower) || O::contains(upper))
    ...
    else
    {
      ...
      else if constexpr( !C::has_alternative )  return svmad_m(expand_mask(cond, as(a)), a, b, c);
