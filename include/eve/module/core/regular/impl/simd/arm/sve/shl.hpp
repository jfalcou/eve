...
namespace eve::_
{
  template<callable_options O, typename T, typename N, integral_scalar_value S>
   wide<T, N> shl_(EVE_REQUIRES(sve_), O const&, wide<T, N> v, wide<S, N> s) noexcept
  ...
  template<callable_options O, typename T, typename N, integral_scalar_value S>
   wide<T, N> shl_(EVE_REQUIRES(sve_), O const&, wide<T,N> v, S s) noexcept
    requires sve_abi<abi_t<T, N>>
  {
  ...
  template<callable_options O, typename T, typename N, std::ptrdiff_t S>
   wide<T, N> shl_(EVE_REQUIRES(sve_), O const&, wide<T,N> v, index_t<S>) noexcept
    requires sve_abi<abi_t<T, N>>
  {
