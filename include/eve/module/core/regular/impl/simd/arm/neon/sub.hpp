...
namespace eve::_
{
  template<callable_options O, arithmetic_scalar_value T, typename N>
   upgrade_t<wide<T, N>> sub_(EVE_REQUIRES(neon128_), O const& opts,
  ...
  template<callable_options O, arithmetic_scalar_value T, typename N>
   wide<T, N> sub_(EVE_REQUIRES(neon128_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
  requires (arm_abi<abi_t<T, N>> && !O::contains(widen) && !O::contains(mod))
  {
    ...
    {
      if      constexpr (c == category::int64x1    ) return vqsub_s64 (a, b);
      else if constexpr (c == category::int64x2    ) return vqsubq_s64(a, b);
      else if constexpr (c == category::uint64x1   ) return vqsub_u64 (a, b);
      else if constexpr (c == category::uint64x2   ) return vqsubq_u64(a, b);
      else if constexpr (c == category::int32x2    ) return vqsub_s32 (a, b);
      else if constexpr (c == category::int32x4    ) return vqsubq_s32(a, b);
      else if constexpr (c == category::uint32x2   ) return vqsub_u32 (a, b);
      else if constexpr (c == category::uint32x4   ) return vqsubq_u32(a, b);
      else if constexpr (c == category::int16x4    ) return vqsub_s16 (a, b);
      else if constexpr (c == category::int16x8    ) return vqsubq_s16(a, b);
      else if constexpr (c == category::uint16x4   ) return vqsub_u16 (a, b);
      else if constexpr (c == category::uint16x8   ) return vqsubq_u16(a, b);
      else if constexpr (c == category::int8x8     ) return vqsub_s8  (a, b);
      else if constexpr (c == category::int8x16    ) return vqsubq_s8 (a, b);
      else if constexpr (c == category::uint8x8    ) return vqsub_u8  (a, b);
      else if constexpr (c == category::uint8x16   ) return vqsubq_u8 (a, b);
    }
    else
    {
