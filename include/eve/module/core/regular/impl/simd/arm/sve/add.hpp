...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto add_(EVE_REQUIRES(sve_), C const& mask, O const& opts, wide<T, N> v, wide<T, N> w) noexcept
  requires sve_abi<abi_t<T, N>>
  {
    ...
    if constexpr(O::contains_any(narrow, widen) || (O::contains_any(lower, upper) && floating_value<T>) ||
