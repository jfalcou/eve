...
namespace eve::_
{
  template<floating_value T, cardinal_type N, callable_options O>
  ...
  template<conditional_expr C,floating_value T, cardinal_type N, callable_options O>
  EVE_FORCEINLINE wide<T, N> trunc_(EVE_REQUIRES(sve_),
                                    C          const& cond,
                                    O          const& opts,
                                    wide<T, N> const& v) noexcept
  requires sve_abi<abi_t<T, N>>
  {
    ...
    else                                                  return trunc[opts][cond].retarget(cpu_{}, v);
  }
}
