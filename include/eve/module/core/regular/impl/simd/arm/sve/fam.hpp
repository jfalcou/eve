...
namespace eve::_
{
  ...
  requires sve_abi<abi_t<T, N>>
   wide<T, N> fam_(EVE_REQUIRES(sve_), O const& opts, wide<T, N> a, wide<T, N> b, wide<T, N> c) noexcept
  {
  ...
  requires sve_abi<abi_t<T, N>>
   wide<T, N> fam_(EVE_REQUIRES(sve_), C cond, O const& opts, wide<T,N> a, wide<T,N> b, wide<T,N> c) noexcept
  {
    if constexpr(O::contains(lower) || O::contains(upper))
      return fam[opts][cond].retarget(cpu_{}, a, b, c);
    else
    {
      // We don't care about PEDANTIC as this is a proper FMA.
      // We don't care about PROMOTE as we only accept similar types.
      ...
      else if constexpr( !C::has_alternative )  return svmla_m(expand_mask(cond, as(a)), a, b, c);
