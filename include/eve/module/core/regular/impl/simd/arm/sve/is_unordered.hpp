...
namespace eve::_
{
  ...
  template<callable_options O, floating_scalar_value T, typename N, conditional_expr C>
   logical<wide<T, N>> is_unordered_(EVE_REQUIRES(sve_), C const& mask, O const& opts,
  ...
  template<callable_options O, floating_scalar_value T, typename N>
   logical<wide<T, N>> is_unordered_(EVE_REQUIRES(sve_), O const&,
                                 wide<T, N> v, wide<T, N> w) noexcept
  requires (sve_abi<abi_t<T, N>>)
  {
    return svcmpuo(expand_mask(keep_first(N::value), as(v)), v, w);
  }
}
