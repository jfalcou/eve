...
namespace eve::_
{
  ...
  template<callable_options O, floating_scalar_value T, cardinal_type N, conditional_expr C>
  ...
  template<callable_options O, floating_scalar_value T, cardinal_type N>
  EVE_FORCEINLINE logical<wide<T, N>> is_unordered_(EVE_REQUIRES(sve_), O const&,
                                 wide<T, N> v, wide<T, N> w) noexcept
  requires (sve_abi<abi_t<T, N>>)
  {
    return svcmpuo(expand_mask(keep_first(N), as(v)), v, w);
  }
}
