...
namespace eve::_
{
  template<callable_options O, typename W>
   as_logical_t<W> is_equal_(EVE_REQUIRES(sve_), O const& opts, W a, W b) noexcept
  ...
  template<callable_options O, conditional_expr C, typename W>
   as_logical_t<W> is_equal_(EVE_REQUIRES(sve_), C const& cx, O const& opts, W a, W b) noexcept
    requires sve_abi<typename W::abi_type>
  {
