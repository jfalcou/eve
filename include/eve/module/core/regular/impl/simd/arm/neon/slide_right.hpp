...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> slide_right_arm_extract( wide<T,N> x, wide<T,N> y, index_t<Shift> )
  {
    ...
    else
    {
      ...
      return bit_cast( slide_right_arm_extract(bit_cast(x,as<i_t>{}), bit_cast(y,as<i_t>{}), index<Shift>), as(x) );
    }
  }
  ...
  template<real_scalar_value T, typename N, std::ptrdiff_t Shift>
  EVE_FORCEINLINE wide<T,N> slide_right_( EVE_SUPPORTS(neon128_)
                                          , wide<T,N> v, index_t<Shift> s
                                        ) noexcept
  requires (0 < Shift && Shift < N::value ) && arm_abi<abi_t<T, N>>
  {
