...
namespace eve::_
{
template<relative_conditional_expr C, scalar_value T, cardinal_type N, simd_compatible_ptr<wide<T, N>> Ptr>
  ...
  requires ppc_abi<abi_t<T, N>> && (!has_store_equivalent<wide<T, N>, Ptr>)
{
  ...
  else if constexpr( N * sizeof(T) == ppc_::bytes )
  {
  ...
  else memcpy(ptr, (T const *)(&value), N * sizeof(T));
}
}
