...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE logical<wide<T, N>> is_not_greater_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
  ...
  EVE_FORCEINLINE logical<wide<T, N>> is_not_greater_(EVE_REQUIRES(avx512_), C const& mask, O const& opts, wide<T, N> a, wide<T, N> b) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      else                                           return is_less_equal[mask](a, b);
    }
  }
}
