...
namespace eve::_
{
                                     ...
                                     wide<T, N> const & v1) noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(maxmag, v0, v1);
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return maxmag[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(maxmag, cx, v, w);
      ...
      else return maxmag[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
