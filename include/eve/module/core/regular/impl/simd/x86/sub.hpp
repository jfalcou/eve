...
namespace eve::_
{
  template<callable_options O, typename T, typename N>
   upgrade_t<wide<T, N>> sub_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> v, wide<T, N> w) noexcept
  ...
  template<callable_options O, typename T, typename N>
   wide<T, N> sub_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
  ...
  requires (x86_abi<abi_t<T, N>> && !O::contains(mod) && !O::contains(widen))
  {
    ...
    else
    {
      ...
      else if constexpr(O::contains(saturated))
      {
        ...
        else if constexpr( c == category::int16x16  ) return _mm256_mask_subs_epi16(src, m, v, w);
        else if constexpr( c == category::uint16x16 ) return _mm256_mask_subs_epu16(src, m, v, w);
        else if constexpr( c == category::int8x32   ) return _mm256_mask_subs_epi8(src, m, v, w);
        else if constexpr( c == category::uint8x32  ) return _mm256_mask_subs_epu8(src, m, v, w);
