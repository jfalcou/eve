...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> mean_value_(EVE_REQUIRES(sse2_),
                                      O          const & opts,
                                      wide<T, N> a,
                                      wide<T, N> b) noexcept
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return mean_value.behavior(cpu_{}, opts, a, b);
    }
    else
    {
      ...
      else return mean_value.behavior(cpu_{}, opts, a, b);
    }
  }
  ...
  EVE_FORCEINLINE wide<T, N>  mean_value_(EVE_REQUIRES(avx512_),
                                       C          const &cx,
                                       O          const &opts,
                                       wide<T, N> const &v,
  ...
  {
    if constexpr( O::contains(upper))
      return mean_value[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      else if constexpr( match(c, category::uint_) ) return mean_value[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
