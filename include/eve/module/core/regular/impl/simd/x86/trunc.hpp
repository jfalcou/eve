...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE
  wide<T, N> trunc_(EVE_SUPPORTS(sse2_), C const &cx, wide<T, N> const &v) noexcept
      requires x86_abi<abi_t<T, N>>
  {
    constexpr auto c = categorize<wide<T, N>>();
    ...
    else
    {
      ...
      auto m    = expand_mask(cx,as<wide<T, N>>{}).storage().value;
            ...
            if constexpr(c == category::float32x16) return _mm512_mask_roundscale_ps(src,m,v, _MM_FROUND_TO_ZERO);
      else  if constexpr(c == category::float64x8 ) return _mm512_mask_roundscale_pd(src,m,v, _MM_FROUND_TO_ZERO);
      else  if constexpr(c == category::float32x8 ) return _mm256_mask_roundscale_pd(src,m,v, _MM_FROUND_TO_ZERO);
      else  if constexpr(c == category::float64x4 ) return _mm256_mask_roundscale_pd(src,m,v, _MM_FROUND_TO_ZERO);
      else  if constexpr(c == category::float32x4 ) return _mm_mask_roundscale_pd(src,m,v, _MM_FROUND_TO_ZERO);
      else  if constexpr(c == category::float64x2 ) return _mm_mask_roundscale_pd(src,m,v, _MM_FROUND_TO_ZERO);
