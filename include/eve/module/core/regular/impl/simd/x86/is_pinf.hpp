...
namespace eve::_
{
  ...
  EVE_FORCEINLINE logical<wide<T, N>> is_pinf_(EVE_REQUIRES(avx512_),
                                                   O          const &opts,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      if      constexpr( !_::supports_fp16_vector_ops ) return is_pinf.behavior(cpu_{}, opts, a);
...
// masked  implementation
  ...
  EVE_FORCEINLINE auto is_pinf_(EVE_REQUIRES(avx512_),
                                     C const& cx,
                                     O const& o,
    ...
    requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return is_pinf[o][cx].retarget(cpu_{}, v);
    }
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return is_pinf[o][cx].retarget(cpu_{}, v);
