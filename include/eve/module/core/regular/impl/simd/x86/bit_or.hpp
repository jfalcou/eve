...
namespace eve::_
{
  template<callable_options O, arithmetic_scalar_value T, typename N>
   wide<T, N> bit_or_(EVE_REQUIRES(sse2_), O const&, wide<T, N> a, wide<T, N> b) noexcept
  ...
  template<callable_options O, conditional_expr C, arithmetic_scalar_value T, typename N>
   wide<T, N> bit_or_(EVE_REQUIRES(avx512_), C const& cx, O const&, wide<T, N> a, wide<T, N> b) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    constexpr auto c = categorize<wide<T, N>>();
