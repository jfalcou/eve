  ...
  else
  {
      ...
      if constexpr( size == 1 )
      {
        if constexpr( current_api >= ssse3 )
        {
          return _mm_shuffle_epi8(
              v.storage(), _mm_set_epi8(14, 15, 12, 13, 10, 11, 8, 9, 6, 7, 4, 5, 2, 3, 0, 1));
        }
        else
        {
    ...
    else if constexpr( std::same_as<abi_t<T, N>, x86_256_> )
    {
      ...
      else if constexpr( size == 4 )
      {
        ...
        else if constexpr( current_api >= avx2 ) { return _mm256_shuffle_epi32(v, sf4); }
        else
        {
      ...
      else if constexpr( size == 8 )
      {
        ...
        else if constexpr( current_api >= avx2 ) { return _mm256_permute4x64_epi64(v, sf4); }
        else
        {
          auto sd = bit_cast(v, as<typename that_t::template rebind<float>> {});
      ...
      {
        if constexpr( std::same_as<T, double> ) { return _mm256_permute2f128_pd(v, v, 0x21); }
        else if constexpr( std::same_as<T, float> )
        {
          auto [l, h] = v.slice();
          return that_t(h, l);
