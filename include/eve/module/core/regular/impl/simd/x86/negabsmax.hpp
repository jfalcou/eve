...
namespace eve::detail
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric2) || O::contains(pedantic2) || O::contains(saturated2))
      return negabsmax_(EVE_TARGETS(cpu_), opts, v0, v1);
    else
    {
      ...
      else return negabsmax_(EVE_TARGETS(cpu_), v0, v1);
    }
  }
  ...
  negabsmax_(EVE_SUPPORTS(avx512_),
             C          const & cx,
             O          const & opts,
             wide<T, N> const & v,
  ...
  {
    if constexpr(O::contains(numeric2) || O::contains(pedantic2) || O::contains(saturated2))
      return negabsmax_(EVE_TARGETS(cpu_), opts, v, w);
    else
    {
      ...
      else return negabsmax_(EVE_TARGETS(cpu_), cx, v, w);
    }
  }
}
