...
namespace eve::_
{
  ...
  EVE_FORCEINLINE logical<wide<T, N>> is_minf_(EVE_REQUIRES(avx512_),
                                                   O          const &opts,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      if      constexpr( !_::supports_fp16_vector_ops ) return is_minf.behavior(cpu_{}, opts, a);
...
// masked  implementation
  ...
  EVE_FORCEINLINE auto is_minf_(EVE_REQUIRES(avx512_),
                                     C const& cx,
                                     O const& o,
    ...
    requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return is_minf[o][cx].retarget(cpu_{}, v);
    }
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return is_minf[o][cx].retarget(cpu_{}, v);
