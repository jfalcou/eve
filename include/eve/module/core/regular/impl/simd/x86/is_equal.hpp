...
EVE_FORCEINLINE as_logical_t<wide<T, N>>
                is_equal_(EVE_SUPPORTS(avx512_),
                          C const                         &cx,
                          wide<T, N> const                &v,
                          wide<T, N> const                &w) noexcept requires x86_abi<abi_t<T, N>>
{
  ...
  {
    return is_equal_(EVE_RETARGET(cpu_), cx, v, w);
  }
  else
  {
