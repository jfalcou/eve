...
{
  template<floating_scalar_value T, integral_scalar_value U, typename N, callable_options O>
  EVE_FORCEINLINE wide<T, N>
  ldexp_(EVE_REQUIRES(avx512_), O const& opts, wide<T, N> a0, wide<U, N> a1)
    noexcept requires(x86_abi<abi_t<T, N>> && !(sizeof(T) == 2))
  {
    if constexpr((sizeof(T) == 2))
      return ldexp.behavior(cpu_{}, opts, a0, a1);
    else
      return ldexp[opts](a0, convert(a1, as<as_floating_point_t<T>>{}));
  }

  template<floating_scalar_value T, floating_scalar_value U, typename N, callable_options O>
  EVE_FORCEINLINE wide<T, N>
  ldexp_(EVE_REQUIRES(avx512_), O const& opts, wide<T, N> a0, wide<U, N> a1)
  ...
  template<floating_scalar_value T, integral_scalar_value U, typename N, conditional_expr C, callable_options O>
  EVE_FORCEINLINE wide<T, N>
  ldexp_(EVE_REQUIRES(avx512_), C const& mask, O const& opts, wide<T, N> a0, wide<U, N> a1)
    noexcept requires(x86_abi<abi_t<T, N>> && !(sizeof(T) == 2))
  {
    if constexpr((sizeof(T) == 2))
      return ldexp.behavior(cpu_{}, opts, a0, a1);
    else
      return ldexp[mask][opts](a0, convert(a1, as<as_floating_point_t<T>>{}));
  }

  template<floating_scalar_value T,  floating_scalar_value U, typename N, conditional_expr C, callable_options O>
  EVE_FORCEINLINE wide<T, N>
  ldexp_(EVE_REQUIRES(avx512_), C const& mask, O const& opts, wide<T, N> a0, wide<U, N> a1)
    noexcept requires x86_abi<abi_t<T, N>>
  {
