...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N> absmax_(EVE_REQUIRES(avx512_),
                                     O          const & opts,
                                     wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return absmax.behavior(cpu_{}, opts, v0, v1);
    }
    else
    {
      constexpr auto cat  = categorize<wide<T, N>>();
      constexpr auto ctrl = range_ctrl::max | range_ctrl::sign_clear;
      // float
      ...
      {
        return apply_fp16_as_fp32(eve::absmax, v0, v1);
      }
      else return absmax.behavior(cpu_{}, opts, v0, v1);
    }
  }
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return  absmax[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      {
        return apply_fp16_as_fp32_masked(eve::absmax[opts], cx, v, w);
      }
      else return absmax[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
