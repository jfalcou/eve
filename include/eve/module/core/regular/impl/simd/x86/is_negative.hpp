...
namespace eve::_
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(pedantic))
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return is_negative.behavior(cpu_{}, o, a);
...
// masked  implementation
    ...
    requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return is_negative[o][cx].retarget(cpu_{}, v);
    }
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return is_negative[o][cx].retarget(cpu_{}, v);
