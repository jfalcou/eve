...
namespace eve::detail
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!detail::supports_fp16_vector_ops) return apply_fp16_as_fp32(ceil, a0);
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!detail::supports_fp16_vector_ops) return apply_fp16_as_fp32_masked(ceil, cx, v);
