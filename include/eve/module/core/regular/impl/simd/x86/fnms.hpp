...
namespace eve::_
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32(fnms, a, b, c);
  ...
  template<typename T, cardinal_type N, conditional_expr C, callable_options O>
  ...
  noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else if constexpr( !C::has_alternative )
    {
      ...
      // Integral don't do anything special ----
      if constexpr( std::integral<T> ) return fnms[opts][mask].retarget(cpu_{}, a, b, c);
      // UPPER LOWER  ----
      else if constexpr(O::contains(lower) || O::contains(upper))
      {
        if constexpr(!O::contains(strict))
        {
          ...
          else                                             return fnms[opts][mask].retarget(cpu_{}, a, b, c);
        }
        else                                                return fnms[opts][mask].retarget(cpu_{}, a, b, c);
      }
      if ((O::contains(lower) || O::contains(upper))&& floating_value<T>) return if_else(mask, eve::fnms[opts](a, b, c), a);
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32_masked(fnms, mask, a, b, c);
