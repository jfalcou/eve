...
// Masked case
  ...
  EVE_FORCEINLINE wide<T, N> fnms_(EVE_SUPPORTS(avx512_),
                                   C          const &mask,
                                   O          const &,
                                   wide<T, N> const &v,
                                   wide<T, N> const &w,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else if constexpr( !C::has_alternative )
    {
      ...
      // No rounding issue with integers, so we just mask over regular FNMS
      else                                          return if_else(mask, eve::fnms(v, w, x), v);
    }
    else                                            return if_else(mask, eve::fnms(v, w, x), alternative(mask, v, as(v)));
  }
}
