...
namespace eve::_
{
  ...
  EVE_FORCEINLINE logical<wide<T, N>> is_ngtz_(EVE_REQUIRES(avx512_),
                                               O          const &,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32(is_ngtz, a);
...
// masked  implementation
  ...
  EVE_FORCEINLINE auto is_ngtz_(EVE_REQUIRES(avx512_),
                                C const& cx,
                                O const& o,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return is_ngtz[o][cx].retarget(cpu_{}, v);
    }
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32_masked(is_ngtz, cx, v);
