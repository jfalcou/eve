...
template<arithmetic_scalar_value In, typename N, arithmetic_scalar_value Out>
 wide<Out, N> convert_saturated(EVE_REQUIRES(sse2_), wide<In, N> v0, as<Out> tgt) noexcept
{
...
// 256 bits <-> 256 bits
template<arithmetic_scalar_value In, typename N, arithmetic_scalar_value Out>
 wide<Out, N> convert_saturated(EVE_REQUIRES(avx2_), wide<In, N> v0, as<Out> tgt) noexcept
{
...
template<signed_integral_scalar_value T, typename N, signed_integral_scalar_value U>
 logical<wide<U, N>> convert_impl(EVE_REQUIRES(sse2_), logical<wide<T, N>> v, as<logical<U>> tgt) noexcept
{
  ...
  {
    return convert(convert(v, as<logical<downgrade_t<as_integer_t<element_type_t<T>, signed>>>> {}), tgt);
  }
  //==============================================================================================
...
template<typename N, arithmetic_scalar_value U>
 wide<U, N> convert_impl(EVE_REQUIRES(sse2_), wide<double, N> v, as<U> tgt) noexcept
{
  ...
  else if constexpr( std::same_as<U, eve::float16_t> )
  {
    ...
    {
      if      constexpr (c_i == category::float64x2)                                   return _mm_cvtpd_ph(v);
      else if constexpr (c_i == category::float64x4)                                   return _mm256_cvtpd_ph(v);
      else if constexpr (c_i == category::float64x8)                                   return _mm512_cvtpd_ph(v);
    }
...
//================================================================================================
template<typename N, arithmetic_scalar_value U>
 wide<U, N> convert_impl(EVE_REQUIRES(sse2_), wide<float, N> v, as<U> tgt) noexcept
{
  constexpr auto c_i  = categorize<wide<float, N>>();
  constexpr auto c_o  = categorize<wide<U, N>>();
  constexpr bool a512 = current_api >= avx512;
  constexpr bool f16c = _::supports_fp16_vector_conversion;
...
//================================================================================================
template<typename N, arithmetic_scalar_value U>
 wide<U, N> convert_impl(EVE_REQUIRES(sse2_), wide<eve::float16_t, N> v, as<U> tgt) noexcept
{
  ...
  if constexpr (eve::_::supports_fp16_vector_ops)
  {
    if      constexpr (c_i == category::float16x8  && c_o == category::float32x4)        return _mm_cvtxph_ps(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::float32x8)        return _mm256_cvtxph_ps(v);
    else if constexpr (c_i == category::float16x16 && c_o == category::float32x16)       return _mm512_cvtxph_ps(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::float64x2)        return _mm_cvtph_pd(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::float64x4)        return _mm256_cvtph_pd(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::float64x8)        return _mm512_cvtph_pd(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::float64x8)        return _mm512_cvtph_pd(v);

    else if constexpr (c_i == category::float16x8  && c_o == category::int16x8)          return _mm_cvtph_epi16(v);
    else if constexpr (c_i == category::float16x16 && c_o == category::int16x16)         return _mm256_cvtph_epi16(v);
    else if constexpr (c_i == category::float16x32 && c_o == category::int16x32)         return _mm512_cvtph_epi16(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int32x4)          return _mm_cvtph_epi32(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int32x8)          return _mm256_cvtph_epi32(v);
    else if constexpr (c_i == category::float16x16 && c_o == category::int32x16)         return _mm512_cvtph_epi32(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x2)          return _mm_cvtph_epi64(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x4)          return _mm256_cvtph_epi64(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x8)          return _mm512_cvtph_epi64(v);

    else if constexpr (c_i == category::float16x8  && c_o == category::int16x8)          return _mm_cvtph_epu16(v);
    else if constexpr (c_i == category::float16x16 && c_o == category::int16x16)         return _mm256_cvtph_epu16(v);
    else if constexpr (c_i == category::float16x32 && c_o == category::int16x32)         return _mm512_cvtph_epu16(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int32x4)          return _mm_cvtph_epu32(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int32x8)          return _mm256_cvtph_epu32(v);
    else if constexpr (c_i == category::float16x16 && c_o == category::int32x16)         return _mm512_cvtph_epu32(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x2)          return _mm_cvtph_epu64(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x4)          return _mm256_cvtph_epu64(v);
    else if constexpr (c_i == category::float16x8  && c_o == category::int64x8)          return _mm512_cvtph_epu64(v);
    else if constexpr (sizeof(c_o) == 1)                                                 return convert(convert(v, as<upgrade_t<U>>{}), tgt);
    else                                                                                 return convert_impl(EVE_TARGETS(cpu_), v, tgt);
  }
...
template<integral_scalar_value T, typename N, arithmetic_scalar_value U>
 wide<U, N> convert_impl(EVE_REQUIRES(sse2_), wide<T, N> v, as<U> tgt) noexcept
    requires(sizeof(T) == 8)
{
  ...
  else if constexpr( std::same_as<U, eve::float16_t> )
  {
    ...
    {
      if      constexpr (c_i == category::int64x2)                                     return _mm_cvtepi64_ph(v);
      else if constexpr (c_i == category::int64x4)                                     return _mm256_cvtepi64_ph(v);
      else if constexpr (c_i == category::int64x8)                                     return _mm512_cvtepi64_ph(v);
      else if constexpr (c_i == category::uint64x2)                                    return _mm_cvtepu64_ph(v);
      else if constexpr (c_i == category::uint64x4)                                    return _mm256_cvtepu64_ph(v);
      else if constexpr (c_i == category::uint64x8)                                    return _mm512_cvtepu64_ph(v);
    }
    else if constexpr (_::supports_fp16_vector_conversion) return convert(convert(v, as<float> {}), tgt);
    else                                                        return convert_impl(EVE_TARGETS(cpu_), v, tgt);
  }
  // ===============================================================================================================
    ...
    requires(sizeof(T) == 4)
{
  ...
  else if constexpr (std::same_as<U, eve::float16_t>)
  {
    if constexpr (_::supports_fp16_vector_ops)
    {
      if      constexpr (c_i == category::int32x4)                                      return _mm_cvtepi32_ph(v);
      else if constexpr (c_i == category::int32x8)                                      return _mm256_cvtepi32_ph(v);
      else if constexpr (c_i == category::int32x16)                                     return _mm512_cvtepi32_ph(v);
      else if constexpr (c_i == category::uint32x4)                                     return _mm_cvtepu32_ph(v);
      else if constexpr (c_i == category::uint32x8)                                     return _mm256_cvtepu32_ph(v);
      else if constexpr (c_i == category::uint32x16)                                    return _mm512_cvtepu32_ph(v);
    }
    else if constexpr (_::supports_fp16_vector_conversion) return convert(convert(v, as<float>{}), tgt);
    else                                                        return convert_impl(EVE_TARGETS(cpu_), v, tgt);
  }
    ...
    requires(sizeof(T) == 2)
{
  ...
  else if constexpr (std::same_as<U, eve::float16_t>)
  {
    if constexpr (_::supports_fp16_vector_ops)
    {
      if      constexpr (c_i == category::int16x8  && c_o == category::float16x8)      return _mm_cvtepi16_ph(v);
      else if constexpr (c_i == category::int16x16 && c_o == category::float16x16)     return _mm256_cvtepi16_ph(v);
      else if constexpr (c_i == category::int16x32 && c_o == category::float16x32)     return _mm512_cvtepi16_ph(v);
      else if constexpr (c_i == category::uint16x8 && c_o == category::float16x8)      return _mm_cvtepu16_ph(v);
    ...
    else if constexpr (_::supports_fp16_vector_conversion) return convert(convert(v, as<upgrade_t<element_type_t<T>>>{}), tgt);
    else                                                        return convert_impl(EVE_TARGETS(cpu_), v, tgt);
  }
    ...
    requires(sizeof(T) == 1)
{
  ...
  using t_t =
      _::conditional_t<std::is_signed_v<T>, as_integer_t<U, signed>, as_integer_t<U, unsigned>>;
