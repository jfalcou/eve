    ...
    requires(sizeof(T) == 2)
{
  ...
  else if constexpr( mi16x8 && mo8x16                             ) return map(as<wide<U, N>>{}, convert, v, tgt);
  ...
  else if constexpr( mi16x8 && std::is_floating_point_v<U>       ) return convert(convert(v, as<upgrade_t<element_type_t<T>>> {}), tgt);
