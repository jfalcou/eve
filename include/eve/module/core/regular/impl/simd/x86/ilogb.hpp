...
namespace eve::detail
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
      ...
      {
        if      constexpr (!detail::supports_fp16_vector_ops) return convert[saturated](apply_fp16_as_fp32(eve::ilogb, a0), int_from<T>());
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!detail::supports_fp16_vector_ops) return convert[saturated](apply_fp16_as_fp32_masked(eve::ilogb, mask, v), int_from<T>());
