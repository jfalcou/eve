...
namespace eve::detail
{
                                        ...
                                        wide<T, N> const & v1) noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !detail::supports_fp16_vector_ops ) return apply_fp16_as_fp32(negminabs, v0, v1);
                                        ...
                                        wide<T, N> const                &w) noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !detail::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(negminabs, cx, v, w);
