...
namespace eve::detail
{
                                        ...
                                        wide<T, N> const & v1) noexcept requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic))
      return negminabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, v0, v1);
    else
    {
      ...
      else return negminabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, v0, v1);
    }
  }
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic))
      return negminabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, v, w);
    else
    {
      ...
      else return negminabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, v, w);
    }
  }
}
