...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N> negminabs_(EVE_REQUIRES(avx512_),
                                        O const          & opts,
                                        wide<T, N> const & v0,
                                        wide<T, N> const & v1) noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      constexpr auto ctrl = range_ctrl::absolute_min | range_ctrl::sign_set;
      // float
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(negminabs, v0, v1);
      ...
      else return negminabs.behavior(cpu_{}, opts, v0, v1);
    }
  }
                                        ...
                                        wide<T, N> const                &w) noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(negminabs, cx, v, w);
