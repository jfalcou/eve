...
template<typename T, typename S, typename N, callable_options O>
 wide<T,N> rotl_(EVE_REQUIRES(avx512_), O const&, wide<T, N> v, wide<S, N> s) noexcept
...
template<typename T, auto S, typename N, callable_options O>
 wide<T,N> rotl_(EVE_REQUIRES(avx512_), O const&, wide<T, N> v, index_t<S>) noexcept
requires(sizeof(T) >= 4 && x86_abi<abi_t<T, N>>)
{
  constexpr auto c = categorize<wide<T, N>>();
...
template<conditional_expr C, typename T, typename S, typename N, callable_options O>
 wide<T,N> rotl_(EVE_REQUIRES(avx512_), C const& cx, O const&, wide<T, N> v, wide<S, N> s) noexcept
requires(sizeof(T) >= 4 && x86_abi<abi_t<T, N>>)
{
  constexpr auto c = categorize<wide<T, N>>();
...
template<conditional_expr C, typename T, auto S, typename N, callable_options O>
 wide<T,N> rotl_(EVE_REQUIRES(avx512_), C const& cx, O const&, wide<T, N> v, index_t<S>) noexcept
requires(sizeof(T) >= 4 && x86_abi<abi_t<T, N>>)
{
  constexpr auto c = categorize<wide<T, N>>();
  auto src = alternative(cx, v, as<wide<T, N>> {});
