...
namespace eve::_
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      if      constexpr( !_::supports_fp16_vector_ops ) return is_infinite.behavior(cpu_{}, opts, a);
...
// masked  implementation
  ...
  EVE_FORCEINLINE auto is_infinite_(EVE_REQUIRES(avx512_),
                                     C const& cx,
                                     O const& o,
    ...
    requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return is_infinite[o][cx].retarget(cpu_{}, v);
