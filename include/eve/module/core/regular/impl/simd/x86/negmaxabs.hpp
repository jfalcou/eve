...
namespace eve::_
{
  template<floating_scalar_value T, cardinal_type N, callable_options O>
  EVE_FORCEINLINE wide<T, N> negmaxabs_(EVE_REQUIRES(avx512_),
                                        O          const & opts,
                                        wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
   if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return negmaxabs.behavior(cpu_{}, opts, v0, v1);
    else
    {
      constexpr auto cat  = categorize<wide<T, N>>();
      constexpr auto ctrl = range_ctrl::absolute_max | range_ctrl::sign_set;
      // float
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(negmaxabs, v0, v1);
      ...
      else return negmaxabs.behavior(cpu_{}, opts, v0, v1);
    }
  }

  // -----------------------------------------------------------------------------------------------
  // Masked case
  template<conditional_expr C, floating_scalar_value T, cardinal_type N, callable_options O>
  EVE_FORCEINLINE wide<T, N> negmaxabs_(EVE_REQUIRES(avx512_),
                                        C          const & cx,
                                        O          const & opts,
                                        wide<T, N> const & v,
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return negmaxabs[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(negmaxabs, cx, v, w);
      ...
      else return negmaxabs[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
