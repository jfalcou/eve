...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> maxabs_(EVE_REQUIRES(avx512_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return maxabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b);
    else
    {
      ...
      else return maxabs.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b);
    }
  }
