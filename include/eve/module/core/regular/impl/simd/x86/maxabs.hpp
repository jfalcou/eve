...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N> maxabs_(EVE_REQUIRES(avx512_),
                                     O          const & opts,
                                     wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(maxabs, v0, v1);
      ...
      else return maxabs.behavior(cpu_{}, opts, v0, v1);
    }
  }
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(maxabs, cx, v, w);
      ...
      else return maxabs[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
