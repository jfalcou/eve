...
namespace eve::detail
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    if      constexpr(sizeof(T) < 4)                    scatter.behavior(as<void>{}, cpu_{}, opts, v, ptr, idx);
    // Singular scatter goes  back to emulation
    if      constexpr(N::value == 1)                    scatter.behavior(as<void>{}, cpu_{}, opts, v, ptr, idx);
    ...
    else if constexpr(!std::same_as<mask_t,ignore_none_>)
    {
      ...
      else
      {
        ...
        else if constexpr(match(ci, category::int32x4 , category::uint32x4 ) && N::value == 2)
        {
          ...
          else scatter.behavior(as<void>{}, cpu_{}, opts, v, ptr, idx);
        }
      }
    }
    else
