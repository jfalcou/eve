...
namespace eve::detail
{
  ...
  fracscale_(EVE_REQUIRES(avx512_),
             wide<T, N> const & a0,
             O          const & o,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      if      constexpr( c == category::float32x16) return _mm512_mask_reduce_ps(src, m, a0, s+V);
      else if constexpr( c == category::float64x8 ) return _mm512_mask_reduce_pd(src, m, a0, s+V);
      else if constexpr( c == category::float32x8 ) return _mm256_mask_reduce_ps(src, m, a0, s+V);
      else if constexpr( c == category::float64x4 ) return _mm256_mask_reduce_pd(src, m, a0, s+V);
      else if constexpr( c == category::float32x4 ) return _mm_mask_reduce_ps(src, m, a0, s+V);
      else if constexpr( c == category::float64x2 ) return _mm_mask_reduce_pd(src, m, a0, s+V);
    }
  }
}
