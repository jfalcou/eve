  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      m_imm_choice<D::base_type::value, r_t> ic;
            ...
            if constexpr(c == category::float32x16 )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm512_mask_reduce_ps(src, m, a, decltype(b)::value); });
      else  if constexpr(c == category::float64x8  )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm512_mask_reduce_pd(src, m, a, decltype(b)::value); });
      else  if constexpr(c == category::float32x8  )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm256_mask_reduce_ps(src, m, a, decltype(b)::value); });
      else  if constexpr(c == category::float64x4  )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm256_mask_reduce_pd(src, m, a, decltype(b)::value); });
      else  if constexpr(c == category::float32x4  )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm_mask_reduce_ps(src, m, a, decltype(b)::value); });
      else  if constexpr(c == category::float64x2  )  return ic(src, m, a0, s, [](auto src, auto m, auto a, auto b){return _mm_mask_reduce_pd(src, m, a, decltype(b)::value); });
    }
  }
