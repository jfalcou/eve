...
namespace eve::_
{
  template<auto S, floating_scalar_value T, cardinal_type N, callable_options O>
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      if      constexpr (!_::supports_fp16_vector_ops) return fracscale.behavior(cpu_{}, o, a0, idx);
  ...
  // -----------------------------------------------------------------------------------------------
  // Masked case
  template<auto S, conditional_expr C, floating_scalar_value T, cardinal_type N, callable_options O>
  EVE_FORCEINLINE wide<T, N> fracscale_(EVE_REQUIRES(avx512_),
                                        C          const &mask,
                                        O          const &o,
                                        wide<T, N> const &a0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else if constexpr(S > 15)      return fracscale[o][mask].retarget(cpu_{}, a0, S);
    else
    {
      ...
      else if constexpr( c == category::float64x8 ) return _mm512_mask_reduce_pd(src, m, a0, spv);
      else if constexpr( c == category::float32x8 ) return _mm256_mask_reduce_ps(src, m, a0, spv);
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return fracscale[o][mask].retarget(cpu_{}, a0, idx);
