...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N>  negabsmin_(EVE_REQUIRES(avx512_),
                                         O          const & opts,
                                         wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return negabsmin.behavior(cpu_{}, opts, v0, v1);
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(negabsmin, v0, v1);
      ...
      else return negabsmin.behavior(cpu_{}, v0, v1);
    }
  }
  ...
  negabsmin_(EVE_REQUIRES(avx512_),
             C          const & cx,
             O          const & opts,
             wide<T, N> const & v,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(negabsmin, cx, v, w);
