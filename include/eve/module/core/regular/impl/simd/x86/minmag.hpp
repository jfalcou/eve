...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N> minmag_(EVE_REQUIRES(avx512_),
                                     O          const & opts,
                                     wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return minmag.behavior(cpu_{}, opts, v0, v1);
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32(minmag, v0, v1);
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return minmag[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      {
        if      constexpr( !_::supports_fp16_vector_ops ) return apply_fp16_as_fp32_masked(minmag, cx, v, w);
