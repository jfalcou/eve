...
namespace eve::_
{
  template<callable_options O, typename T, typename N>
   upgrade_t<wide<T, N>> mul_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> v, wide<T, N> w) noexcept
  ...
  template<callable_options O, typename T, typename N>
   wide<T, N> mul_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
  requires (x86_abi<abi_t<T, N>> && !O::contains(mod)&& !O::contains(widen))
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32(mul, a, b);
  ...
  template<callable_options O, conditional_expr C, typename T, typename N>
   wide<T, N> mul_(EVE_REQUIRES(avx512_), C const& cx, O const& opts, wide<T, N> a, wide<T, N> b) noexcept
  requires (x86_abi<abi_t<T, N>> && !O::contains(mod)&& !O::contains(widen))
  {
    constexpr auto c = categorize<wide<T, N>>();
    ...
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32_masked(mul, cx, a, b);
