...
namespace eve::_
{
  ...
  template<arithmetic_scalar_value T, typename N, callable_options O>
   wide<T, N> max_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> v0, wide<T, N> v1) noexcept
  ...
  template<conditional_expr C, arithmetic_scalar_value T, typename N, callable_options O>
   wide<T, N> max_(EVE_REQUIRES(avx512_), C const& cx, O const& opts, wide<T, N> const& v, wide<T, N> const& w) noexcept
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic))
    {
