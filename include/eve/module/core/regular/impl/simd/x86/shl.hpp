...
namespace eve::_
{
  template<callable_options O, typename T, typename N, integral_scalar_value S>
   wide<T,N> shl_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> v, S s) noexcept
  ...
  template<callable_options O, typename T, typename N, integral_scalar_value S>
   wide<T,N> shl_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> v, wide<S, N> s) noexcept
    requires (x86_abi<abi_t<T, N>>)
  {
  ...
  template<callable_options O, conditional_expr C, integral_scalar_value T, typename S, typename N>
   wide<T,N> shl_(EVE_REQUIRES(avx512_), C const& cx, O const&, wide<T, N> v, wide<S, N> s) noexcept
  ...
  template<callable_options O, conditional_expr C, integral_scalar_value T, typename N>
   wide<T,N> shl_(EVE_REQUIRES(avx512_), C const& cx, O const&, wide<T, N> v, unsigned int s) noexcept
    requires((sizeof(T) >= 2) && x86_abi<abi_t<T, N>>)
  {
      constexpr auto c = categorize<wide<T, N>>();
      auto src = alternative(cx, v, as<wide<T, N>> {});
