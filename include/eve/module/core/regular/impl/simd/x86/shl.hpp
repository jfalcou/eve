...
namespace eve::_
{
  template<callable_options O, typename T, cardinal_type N, integral_scalar_value S>
    ...
    requires (x86_abi<abi_t<T, N>>)
  {
    ...
    [[maybe_unused]] auto shft = [](auto a, auto b )
    {
      ...
      using i16_t = wide<std::uint16_t, half_size>;
    ...
    else  if constexpr( (c == category::int8x16)  && (N != 1)) return shft(v, s);
    else  if constexpr( (c == category::uint8x16) && (N != 1)) return shft(v, s);
  ...
  template<callable_options O, conditional_expr C, integral_scalar_value T, typename S, cardinal_type N>
  ...
  template<callable_options O, conditional_expr C, integral_scalar_value T, cardinal_type N>
