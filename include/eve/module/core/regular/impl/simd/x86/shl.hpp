...
namespace eve::detail
{
    ...
    requires (x86_abi<abi_t<T, N>>)
  {
    ...
    else  if constexpr (sizeof(v) == 32)
    {
      ...
      {
        auto [ la, ha ] = v.slice();
        return wide<T,N>{ la << s, ha << s };
      }
    }
    else
    {
      return map( []<typename V>(V a, auto b) { return static_cast<V>(a << b); }, v, s);
    }
  }

  template<callable_options O, typename T, typename N, integral_scalar_value S>
  EVE_FORCEINLINE wide<T,N> shl_(EVE_REQUIRES(sse2_), O const&, wide<T, N> v, wide<S, N> s) noexcept
    requires (x86_abi<abi_t<T, N>>)
  {
    constexpr auto c = categorize<wide<T, N>>();
    constexpr bool is_avx2 = current_api >= avx2;
