...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> fma_(EVE_REQUIRES(sse2_), O const& opts, wide<T, N> a, wide<T, N> b, wide<T, N> c) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    ...
    else if constexpr (O::contains(lower) || O::contains(upper))
    {
      if (!O::contains(strict))
      {
        ...
        {
          constexpr auto dir = (O::contains(lower) ? _MM_FROUND_TO_NEG_INF : _MM_FROUND_TO_POS_INF) | _MM_FROUND_NO_EXC;
          ...
          else                                             return fma.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b, c);
        }
        else                                               return fma.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b, c);
      }
      else                                                 return fma.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b, c);
  ...
  EVE_FORCEINLINE wide<T, N> fma_( EVE_REQUIRES(avx512_), C const& mask, O const& opts, wide<T, N> a, wide<T, N> b, wide<T, N> c)
    noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return alternative(mask, a, as{a});
    }
    else if constexpr (!C::has_alternative)
    {
      ...
      else                                            return if_else(mask, fma[opts.drop(condition_key)](a, b, c), a);
    }
    else                                              return if_else(mask, fma[opts.drop(condition_key)](a, b, c), alternative(mask, a, as{a}));
  }
}
