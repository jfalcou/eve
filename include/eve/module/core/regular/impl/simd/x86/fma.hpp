...
namespace eve::_
{
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32(fma, a, b, c);
  ...
  noexcept requires x86_abi<abi_t<T, N>>
  {
    ...
    else if constexpr( !C::has_alternative )
    {
      ...
      // Integral don't do anything special ----
      if constexpr( std::integral<T> ) return fma[opts][mask].retarget(cpu_{}, a, b, c);
      // UPPER LOWER  ----
      else if constexpr(O::contains(lower) || O::contains(upper))
      {
        if constexpr(!O::contains(strict))
        {
          ...
          else                                             return fma[opts][mask].retarget(cpu_{}, a, b, c);
        }
        else                                               return fma[opts][mask].retarget(cpu_{}, a, b, c);
      }
      ...
      {
        if      constexpr (!_::supports_fp16_vector_ops) return apply_fp16_as_fp32_masked(fma, mask, a, b, c);
    ...
    else                                              return if_else(mask, eve::fma[opts](a, b, c), alternative(mask, a, as(a)));
  }
}
