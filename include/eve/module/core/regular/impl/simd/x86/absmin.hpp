...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> absmin_(EVE_REQUIRES(avx512_), O const& opts, wide<T, N> a, wide<T, N> b) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return absmin.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b);
    else
    {
      ...
      else  return absmin.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b);
    }
  }
  ...
  EVE_FORCEINLINE wide<T, N> absmin_(EVE_REQUIRES(avx512_), C const& cx, O const& opts, wide<T, N> a, wide<T, N> b) noexcept
    requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return absmin.behavior(as<wide<T, N>>{}, cpu_{}, opts, a, b);
