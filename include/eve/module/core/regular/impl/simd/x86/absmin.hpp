...
namespace eve::_
{
  ...
  EVE_FORCEINLINE wide<T, N> absmin_(EVE_REQUIRES(avx512_),
                                     O          const & opts,
                                     wide<T, N> const & v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return absmin.behavior(cpu_{}, opts, v0, v1);
    else
    {
      constexpr auto cat  = categorize<wide<T, N>>();
      constexpr auto ctrl = range_ctrl::min | range_ctrl::sign_clear;
      // float
      ...
      {
        return apply_fp16_as_fp32(eve::absmin, v0, v1);
      }
      else  return absmin.behavior(cpu_{}, opts, v0, v1);
    }
  }
  ...
  {
    if constexpr(O::contains(numeric) || O::contains(pedantic) || O::contains(saturated))
      return absmin[opts][cx].retarget(cpu_{}, v, w);
    else
    {
      ...
      {
        return apply_fp16_as_fp32_masked(eve::absmin[opts], cx, v, w);
      }
      else return absmin[opts][cx].retarget(cpu_{}, v, w);
    }
  }
}
