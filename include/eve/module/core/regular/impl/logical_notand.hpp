...
namespace eve::_
{
  template<callable_options O, relaxed_logical_value T>
   constexpr T logical_notand_(EVE_REQUIRES(cpu_), O const&, T a, T b) noexcept
  {
    return logical_andnot(b, a);
  }

  template<callable_options O, conditional_expr C, relaxed_logical_value T>
   constexpr T logical_notand_(EVE_REQUIRES(cpu_), C const& cx, O const&, T a, T b) noexcept
  {
