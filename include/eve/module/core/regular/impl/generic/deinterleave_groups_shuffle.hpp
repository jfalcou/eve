...
deinterleave_groups_shuffle_as_doubles(wide<T, N> v, lanes_t<G>)
{
  using doubles = wide<double, N * sizeof(T) / 8>;

  auto cast_v = bit_cast(v, eve::as<doubles> {});

  auto res = deinterleave_groups_shuffle(cast_v, lanes<G * sizeof(T) / 8>);

  return bit_cast(res, as(v));
}

template<width_type N, std::ptrdiff_t G>
EVE_FORCEINLINE auto
deinterleave_groups_shuffle_as_doubles(wide<float, N> v0, wide<float, N> v1, lanes_t<G>)
{
  ...
  auto res = deinterleave_groups_shuffle(cast_v0, cast_v1, lanes<G / 2>);
...
template<typename T, width_type N, std::ptrdiff_t G>
EVE_FORCEINLINE auto
deinterleave_groups_shuffle_(EVE_SUPPORTS(cpu_), wide<T, N> v, lanes_t<G>) requires(G <= N)
{
  if constexpr( G >= N / 2 ) return v;
  else if constexpr( has_aggregated_abi_v<wide<T, N>> )
  {
    auto [v0, v1] = v.slice();
    return deinterleave_groups_shuffle(v0, v1, lanes<G>);
  }
  else if constexpr( is_bundle_v<abi_t<T, N>> )
  {
    return wide<T, N>(
        kumi::map([](auto _v) { return deinterleave_groups_shuffle(_v, lanes<G>); }, v));
  }
                             ...
                             lanes_t<G>) requires(G <= N * 2)
{
  ...
  else if constexpr( is_bundle_v<abi_t<T, N>> )
  {
    return res_t(kumi::map(
        [](auto _v0, auto _v1) { return deinterleave_groups_shuffle(_v0, _v1, lanes<G>); }, v0, v1));
  }
  ...
  {
    v0 = deinterleave_groups_shuffle(v0, lanes<G>);
    v1 = deinterleave_groups_shuffle(v1, lanes<G>);
    return deinterleave_groups_shuffle(v0, v1, lanes<N / 2>);
  }
  else
  {
    res_t r;
