...
logical_ornot_(EVE_SUPPORTS(cpu_), logical<T> const& a, logical<U> const& b) noexcept
...
EVE_FORCEINLINE as_logical_t<T>
logical_ornot_(EVE_SUPPORTS(cpu_), logical<T> const& a, logical<U> const& b) noexcept requires
    has_native_abi_v<T> && has_native_abi_v<U> &&(cardinal_v<T> == cardinal_v<U>)
{
  ...
  {
    return bit_cast(bit_ornot(bit_mask(a), bit_mask(b)), as<as_logical_t<T>>());
  }
...
EVE_FORCEINLINE as_logical_t<T>
                logical_ornot_(EVE_SUPPORTS(cpu_),
                               logical<T> const                &a,
                               logical<U> const                &b) noexcept requires has_native_abi_v<T> && has_native_abi_v<U>
{
  ...
  if constexpr( !abi_t::is_wide_logical ) { return eve::logical_ornot(a, b.value()); }
...
EVE_FORCEINLINE auto
logical_ornot_(EVE_SUPPORTS(cpu_), T a, bool b) noexcept
{
...
EVE_FORCEINLINE auto
logical_ornot_(EVE_SUPPORTS(cpu_), bool a, U b) noexcept
{
  return a ? true_(as<U>()) : U {!b};
}

EVE_FORCEINLINE auto
logical_ornot_(EVE_SUPPORTS(cpu_), bool a, bool b) noexcept
{
  return a || !b;
}
}
