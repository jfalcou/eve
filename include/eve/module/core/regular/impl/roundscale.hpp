...
{
  if constexpr( has_native_abi_v<T> ) { return ldexp(nearest(ldexp(a0, scale)), -scale); }
...
roundscale_(EVE_SUPPORTS(cpu_), D const&, T const& a0, int scale) noexcept
    requires(is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
{
  if constexpr( has_native_abi_v<T> ) { return ldexp(D()(round)(ldexp(a0, scale)), -scale); }
  else { return apply_over(D()(roundscale), a0, scale); }
}
