...
{
  template<logical_simd_value T, relative_conditional_expr C, callable_options O>
  EVE_FORCEINLINE bool
  all_(EVE_REQUIRES(cpu_), O const &, C const& cond, T const& v) noexcept
  {
  ...
  EVE_FORCEINLINE bool
  all_(EVE_REQUIRES(cpu_), O const &, bool v) noexcept
  {
    return v;
  }
  ...
  EVE_FORCEINLINE bool
  all_(EVE_REQUIRES(cpu_), O const &, logical<T> const& v) noexcept
  {
    if constexpr( scalar_value<T> ) return bool(v);
    else return eve::all[ignore_none](v);
  }

  template<logical_simd_value Logical, callable_options O>
  EVE_FORCEINLINE bool
  all_(EVE_REQUIRES(cpu_), O const &, top_bits<Logical> mmask) noexcept
  {
