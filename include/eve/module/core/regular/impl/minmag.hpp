...
namespace eve::_
{
  ...
  minmag_(EVE_REQUIRES(cpu_), O const & o, T0 a0, Ts... cs) noexcept
  {
    ...
    {
      if constexpr(scalar_value<r_t> && (sizeof...(Ts)+1 >= expected_width_v<element_type_t<r_t>>))
      {
        auto head = eve::as_wides(eve::majorant(eve::as<r_t>()), a0, cs...);
