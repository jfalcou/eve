...
namespace eve::_
{
  template<callable_options O, typename T, typename U>
   constexpr auto rshl_(EVE_REQUIRES(cpu_), O const &, T a0, U a1) noexcept
  {
  ...
  template<callable_options O, integral_value T, std::ptrdiff_t S>
   constexpr auto rshl_(EVE_REQUIRES(cpu_), O const &, T v, index_t<S>) noexcept
  {
    if constexpr (S == 0) return v;
    else                  return rshl(v, S);
  }
}
