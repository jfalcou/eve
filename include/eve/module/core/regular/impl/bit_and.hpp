...
auto
bit_and_(EVE_SUPPORTS(cpu_), C const& cond, T0 a0, T1 a1, Ts... args) requires
...
bit_and_(EVE_SUPPORTS(cpu_), Ts const & args)
{
  ...
  else if constexpr(kumi::size_v<Ts> == 2) return bit_and(get<0>(args), get<1>(args));
...
{
  if constexpr(kumi::size_v<Ts> == 1) return get<0>(args);
  else return kumi::apply( [&](auto... m) { return mask_op(cond, eve::bit_and, m...); }, args);
}
