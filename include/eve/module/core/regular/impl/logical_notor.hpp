...
logical_notor_(EVE_SUPPORTS(cpu_), logical<T> const& a, logical<U> const& b) noexcept
{
...
EVE_FORCEINLINE as_logical_t<T>
logical_notor_(EVE_SUPPORTS(cpu_), logical<T> const& a, logical<U> const& b) noexcept requires
    has_native_abi_v<T> && has_native_abi_v<U> &&(cardinal_v<T> == cardinal_v<U>)
{
  ...
  {
    return bit_cast(bit_notor(a.bits(), b.bits()), as<as_logical_t<T>>());
  }
    ...
    requires(has_native_abi_v<T>&& has_native_abi_v<U>)
{
  ...
  if constexpr( !abi_t::is_wide_logical ) { return eve::logical_notor(a, b.value()); }
  else if constexpr( sizeof(elt_t) == sizeof(U) )
  {
    auto bb = is_nez(T(bit_cast(b, as<logical<elt_t>>())));
    return bit_cast(bit_notor(a.bits(), bb.bits()), as<as_logical_t<T>>());
  }
}

template<scalar_value T, simd_value U>
EVE_FORCEINLINE auto
logical_notor_(EVE_SUPPORTS(cpu_),
               logical<T> const& a,
...
EVE_FORCEINLINE auto
logical_notor_(EVE_SUPPORTS(cpu_), T a, bool b) noexcept
{
...
EVE_FORCEINLINE auto
logical_notor_(EVE_SUPPORTS(cpu_), bool a, U b) noexcept
{
  return a ? U {b} : true_(as<U>());
}

EVE_FORCEINLINE auto
logical_notor_(EVE_SUPPORTS(cpu_), bool a, bool b) noexcept
{
  return !a || b;
}
}
