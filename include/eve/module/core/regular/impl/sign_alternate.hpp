...
sign_alternate_(EVE_SUPPORTS(cpu_), T const& n) noexcept
{
  ...
  if constexpr( has_native_abi_v<T> )
  {
    ...
    else return if_else(is_odd(n), mone, one(as(n)));
  }
  else return apply_over(sign_alternate, n);
}
...
sign_alternate_(EVE_SUPPORTS(cpu_), C const& cond, U const& t) noexcept
{
