...
//      std::cout << "rezut et zut" << std::endl;
      ...
      auto doit = [a, b, c](){
        ...
        if constexpr(K == 0x1b) return bit_not(bit_select(c, a, b)); ////bit_or(bit_notand(a, c), bit_notand(c, bit_not(b)));
        if constexpr(K == 0x1c) return bit_notand(bit_and(a, c), bit_xor(a, b));
        if constexpr(K == 0x1d) return bit_not(bit_select(b, a, c)); //bit_or(bit_notand(a, b),  bit_notand(b, bit_not(c)));
        ...
        if constexpr(K == 0x27) return bit_not(bit_select(c, b, a)); //bit_or(bit_notand(b, c), bit_notand(c, bit_not(a)));
        ...
        if constexpr(K == 0x2e) return bit_select(b, bit_not(a), c); //bit_xor(bit_or(b, c), bit_and(a, b));
        ...
        if constexpr(K == 0x35) return bit_not(bit_select(a, b, c)); //bit_or(bit_notand(b, a), bit_notand(a,bit_not(c)));
        ...
        if constexpr(K == 0x3a) return bit_select(a, bit_not(b), c); //bit_or(bit_notand(b, a), bit_notand(a, c));
        ...
        if constexpr(K == 0x47) return bit_not(bit_select(b, c, a)); //bit_or(bit_notand(c, b), bit_notand(b, bit_not(a)));
        ...
        if constexpr(K == 0x4e) return bit_select(c, bit_not(a), b); //bit_or(bit_notand(a, c), bit_notand(c, b));
        ...
        if constexpr(K == 0x53) return bit_not(bit_select(a, c, b)); //bit_or(bit_notand(c, a), bit_notand(a, bit_not(b)));
        ...
        if constexpr(K == 0x5c) return bit_select(a, bit_not(c), b); //bit_or(bit_notand(c, a), bit_notand(a, b));
        ...
        if constexpr(K == 0x72) return bit_select(c, bit_not(b), a); //bit_or(bit_notand(b, c), bit_notand(c, a));
        if constexpr(K == 0x73) return bit_or(bit_notand(c, a), bit_not(b));
        if constexpr(K == 0x74) return bit_select(b, bit_not(c), a); //bit_or(bit_notand(c, b), bit_notand(b, a));
        ...
        if constexpr(K == 0x8b) return bit_select(b, c, bit_not(a)); //bit_or(bit_and(b, c),  bit_notand(b, bit_not(a)));
        if constexpr(K == 0x8c) return bit_notand(bit_notand(c, a), b);
        if constexpr(K == 0x8d) return bit_select(c, b, bit_not(a)); //bit_or(bit_and(c, b), bit_notand(c,bit_not(a)));
        ...
        if constexpr(K == 0xa3) return bit_select(a, c, bit_not(b)); //bit_or(bit_and(a, c), bit_notand(a,bit_not(b)));
        ...
        if constexpr(K == 0xb1) return bit_select(c, a, bit_not(b)); //bit_or(bit_and(a, c), bit_andnot(bit_not(b), c));
        ...
        if constexpr(K == 0xc5) return bit_select(a, b, bit_not(c)); //bit_or(bit_and(a, b), bit_notand(a, bit_not(c)));
        ...
        if constexpr(K == 0xd1) return bit_select(b, a, bit_not(c)); //bit_or(bit_not(bit_or(b, c)), bit_and(a, b));
