...
namespace eve
{
  ...
  struct bit_ternary_t : strict_elementwise_callable<bit_ternary_t, Options>
  {
    template<int K, eve::value T0, value T1, value T2>
    requires(eve::same_lanes_or_scalar<T0, T1, T2> && 0 <= K && K <= 255)
      EVE_FORCEINLINE constexpr bit_value_t<T0, T1, T2>
    operator()( std::integral_constant<decltype(K), K> k, T0 t0, T1 t1, T2 t2) const noexcept
    {
      return EVE_DISPATCH_CALL(k, t0,  t1, t2);
    }

    template<eve::value T0, value T1, value T2>
    requires(eve::same_lanes_or_scalar<T0, T1, T2>)
      EVE_FORCEINLINE constexpr bit_value_t<T0, T1, T2>
    operator()(detail::bit_tern k, T0 t0, T1 t1, T2 t2) const noexcept
    {
      return EVE_DISPATCH_CALL(k, t0,  t1, t2);
    }
...
//================================================================================================
  ...
  namespace detail
  {
    ...
    template < bit_tern K, integral_value T0, integral_value T1, integral_value T2>
    EVE_FORCEINLINE  bit_value_t<T0, T1, T2> bit_ternary_(EVE_SUPPORTS(cpu_)
    ...
    // this is adapted from Samuel neves ternary logic for sse avx etc.
    template < int K, typename T0, typename T1, typename T2>
    EVE_FORCEINLINE  bit_value_t<T0, T1, T2> bit_ternary_(EVE_SUPPORTS(cpu_)
