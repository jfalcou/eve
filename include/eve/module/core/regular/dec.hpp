...
namespace eve
{
  ...
  namespace detail
  {
    ...
    {
      if constexpr (integral_value<T> && O::contains(saturated)) return dec[a != valmin(as{a})](a);
    ...
    EVE_FORCEINLINE constexpr T dec_(EVE_REQUIRES(cpu_), C cond, O const& o, T const& a) noexcept
    {
      if constexpr (simd_value<T>)
      {
        ...
        if      constexpr (O::contains(saturated)) return dec[cond.mask(as<m_t>{}) && (a != valmin(as{a}))](a);
