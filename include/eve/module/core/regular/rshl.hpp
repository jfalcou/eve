...
//!
//! the call `rshl(x, n)` is equivalent to `if_else(n>0, shl(x, n), shr(x, n))` if `x`  is an  [simd
...
//!  **Return value**
//!
//!  A Callable object so that the expression `rshl[cond](x, ...)` is equivalent to
//!  `if_else(cond,rshl(x, ...),x)`
//!
//! ---
//!
