...
namespace eve ::detail
{
  ...
  EVE_FORCEINLINE auto shl_(EVE_SUPPORTS(sse2_), wide<T, N, sse_> const &a0, I a1) noexcept
  requires(wide<T, N, sse_>, Integral<T>, Integral<I>)
  {
    using t_t = wide<T, N, sse_>;
    EVE_ASSERT(detail::assert_good_shift<t_t>(a1),
               "[eve::shl sse2] -  At least one of " << a1 << "elements is out of the range [0, "
                                                     << sizeof(T) * 8 << "[.");
    ...
    if constexpr(sizeof(T) == 2) return t_t(_mm_slli_epi16(a0, a1));
    if constexpr(sizeof(T) == 4) return t_t(_mm_slli_epi32(a0, a1));
    if constexpr(sizeof(T) == 8) return t_t(_mm_slli_epi64(a0, a1));
  }

  template<typename T, typename N, typename I>
  EVE_FORCEINLINE auto shl_(EVE_SUPPORTS(sse2_),
                            wide<T, N, sse_> const &a0, wide<I, N, sse_> const &a1) noexcept
  requires(wide<T, N, sse_>, Integral<T>, Integral<I>)
  {
