...
namespace eve::detail
{
                       ...
                       , wide<T, N, avx_> const &v1) noexcept
  {
    ...
    {
      if constexpr(std::is_same_v<T, float>) return as_logical_t<t_t>( _mm256_cmp_ps(v0, v1, _CMP_NLE_UQ));
      if constexpr(std::is_same_v<T, double>) return as_logical_t<t_t>(_mm256_cmp_pd(v0, v1, _CMP_NLE_UQ));
    }
    else
    {
