...
namespace eve::detail
{
                                ...
                                wide<T, N, sse_> const &v1) noexcept
  {
    ...
    {
      if constexpr(std::is_same_v<T, float>) return as_logical_t<t_t>( _mm_cmp_ps(v0, v1, _CMP_LT_OQ));
      if constexpr(std::is_same_v<T, double>) return as_logical_t<t_t>(_mm_cmp_pd(v0, v1, _CMP_LT_OQ));
    }
    else
    {
