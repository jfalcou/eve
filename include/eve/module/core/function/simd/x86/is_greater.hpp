...
namespace eve::detail
{
  ...
  is_greater_(EVE_SUPPORTS(sse2_)
           ...
           , wide<T, N, sse_> const &v1) noexcept
  {
    ...
    else // if constexpr(std::is_integral_v<T>)
    {
      ...
      else // if constexpr(std::is_unsigned_v<T>)
      {
        ...
        return eve::bitwise_cast<l_t>(eve::is_greater(eve::bitwise_cast<s_t>(v0) - sm
                                                     , eve::bitwise_cast<s_t>(v1) - sm));
      }
    }
  }
           ...
           , wide<T, N, sse_> const &v1) noexcept
  {
    ...
    if constexpr(std::is_same_v<T, float>) return l_t(_mm_cmp_ps(v0, v1, _CMP_GT_OQ));
    else if constexpr(std::is_same_v<T, double>) return l_t(_mm_cmp_pd(v0, v1, _CMP_GT_OQ));
    else // if constexpr(std::is_integral_v<T>)
    {
      if constexpr(std::is_signed_v<T>)
      {
        ...
        else if constexpr(sizeof(T) == 8) return l_t(_mm_cmpgt_epi64(v0, v1));
      }
      else // if constexpr(std::is_unsigned_v<T>)
      {
        using s_t    = eve::wide<eve::detail::as_integer_t<T, signed>, N, sse_>;
