...
namespace eve::detail
{
  ...
  rshr_(EVE_SUPPORTS(simd_), wide<T, N, ABI> const &v0, U const &v1) noexcept
  {
    ...
    else
    {
      ...
      if constexpr(is_wide_v<U>)
        ...
        return is_gtz(v1) ?  shr(v0, max(Zero(as(v1)), v1)) : shl(v0, max(Zero(as(v1)), -v1));
#else
      if constexpr(is_vectorized_v<U>)
        return if_else(is_gtz(v1), shr(v0, v1), shl(v0, -v1));
      else
        return is_gtz(v1) ?  shr(v0, v1) : shl(v0, -v1);
#endif
    }
  }
  ...
  EVE_FORCEINLINE auto
  rshr_(EVE_SUPPORTS(simd_), wide<T, N, aggregated_> const &v0, U const &v1) noexcept
  {
  ...
  template<typename T, typename U, typename N>
  EVE_FORCEINLINE auto
  rshr_(EVE_SUPPORTS(simd_), wide<T, N, emulated_> const &v0, U const &v1) noexcept
  {
