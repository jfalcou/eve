...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE constexpr auto logical_xor_(EVE_SUPPORTS(cpu_), T const &a, U const &b) noexcept
  ...
  template<typename T, typename U>
  EVE_FORCEINLINE constexpr auto logical_xor_( EVE_SUPPORTS(cpu_),
  ...
  {
    return bitwise_cast<logical<T>>( bitwise_xor(bitwise_mask(a), bitwise_mask(b)) );
  }
}
