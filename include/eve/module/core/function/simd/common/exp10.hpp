...
**/
//==================================================================================================
#ifndef EVE_MODULE_CORE_FUNCTION_COMMON_SIMD_EXP10_HPP_INCLUDED
#define EVE_MODULE_CORE_FUNCTION_COMMON_SIMD_EXP10_HPP_INCLUDED
...
{
  template<typename T>
  EVE_FORCEINLINE auto exp10_(EVE_SUPPORTS(cpu_)
                            , const T &xx) noexcept
  ...
  requires(T, vectorized<T>)
  {
    ...
    else
    {
      ...
      if constexpr(std::is_floating_point_v<vt_t>)
      {
        ...
        c = pedantic_(ldexp)(c, k);
//        c = if_else(is_less(xx, Minlog10<T>()), eve::zero_, c);
        return if_else(is_greater_equal(xx, Maxlog10<T>()), Inf<T>(), c);
      }
      else
      {
