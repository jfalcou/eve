...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto div_ ( EVE_SUPPORTS(simd_)
  ...
  {
    return eve::div(v0, wide<T, N, ABI>(static_cast<T>(v1)));
  }
  ...
  EVE_FORCEINLINE auto div_(EVE_SUPPORTS(simd_),
                            ...
                            wide<T, N, ABI> const &v1) noexcept requires(wide<T, N, ABI>,
                                                                         detail::Convertible<U, T>)
  {
    return eve::div(wide<T, N, ABI>(static_cast<T>(v0)), v1);
  }

  // -----------------------------------------------------------------------------------------------
  // Aggregation
  template<typename T, typename N>
  EVE_FORCEINLINE wide<T, N, aggregated_> div_(EVE_SUPPORTS(simd_),
  ...
  // -----------------------------------------------------------------------------------------------
  // Edivation with auto-splat inside map for performance purpose
  template<typename T, typename N>
  EVE_FORCEINLINE auto div_(EVE_SUPPORTS(simd_),
  ...
  EVE_FORCEINLINE auto div_(EVE_SUPPORTS(simd_),
                            ...
                            U const &v1) noexcept requires(wide<T, N, emulated_>,
                                                           detail::Convertible<U, T>)
  {
    return map(eve::div, v0, static_cast<T>(v1));
  }

  template<typename T, typename N, typename U>
  EVE_FORCEINLINE auto
  div_(EVE_SUPPORTS(simd_),
       U const &                    v0,
       wide<T, N, emulated_> const &v1) noexcept requires(wide<T, N, emulated_>,
                                                          detail::Convertible<U, T>)
  {
    return map(eve::div, static_cast<T>(v0), v1);
  }
}
