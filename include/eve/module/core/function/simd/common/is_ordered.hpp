...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<as_logical_t<T>, N, ABI> is_ordered_(EVE_SUPPORTS(simd_),
                                                              ...
                                                              wide<T, N, ABI> const &v1) noexcept
  {
    ...
    if constexpr(std::is_integral_v<T> || eve::is_logical_v<T>)
      ...
      return logical_and(is_equal(v0, v0), is_equal(v1, v1));
  }
  
  // -----------------------------------------------------------------------------------------------
  // Support for mixed type with auto-splat
  template<typename T, typename N, typename ABI, typename U>
  EVE_FORCEINLINE auto is_ordered_(EVE_SUPPORTS(simd_),
                                     wide<T, N, ABI> const &v0,
                                     U const &v1) noexcept requires(as_logical_t<wide<T, N, ABI>>,
                                                                    detail::Convertible<U, T>)
  {
    return eve::is_ordered(v0, wide<T, N, ABI>(static_cast<T>(v1)));
  }
  
  template<typename T, typename N, typename ABI, typename U>
  EVE_FORCEINLINE auto
  is_ordered_(EVE_SUPPORTS(simd_),
                U const &              v0,
                wide<T, N, ABI> const &v1) noexcept requires(as_logical_t<wide<T, N, ABI>>,
                                                             detail::Convertible<U, T>)
  {
    return eve::is_ordered(wide<T, N, ABI>(static_cast<T>(v0)), v1);
  }
  ...
  {
    return aggregate(eve::is_ordered, v0, v1);
  }
  ...
  {
    return map(eve::is_ordered, v0, v1);
  }
}
