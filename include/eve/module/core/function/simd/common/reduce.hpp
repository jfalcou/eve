...
//         }
      ...
      {
//         auto [nd, xd, xcd] = reduce(convert(x, double_));
...
//       auto [sn, sr] =  reduce_fast(x);
//       auto xle120 = x <= 120.0f; 
//       if (all(xle120)) return std::tuple<t_t, t_t, t_t>(sn, sr, t_t(0)); 
...
//       auto bn = if_else(xle120, sn, convert(n, single_));
//       auto br = if_else(xle120, sr,  convert(xx * pi63, single_));
//       using r_t =  std::tuple<t_t, t_t, t_t>; 
//       return r_t( bn, br, t_t(0)); 
//     }
//  }
...
// //     auto a = t2 - t1;
// //     da += (t2 - a) - t1;
//     return std::tuple<t_t, t_t, t_t>(x, 0, 0); 
...
// //   {
// //     using t_t = wide<float, N, ABI>; 
// //     auto [n, xr, dxr] = reduce(convert(x, as<double>()));
