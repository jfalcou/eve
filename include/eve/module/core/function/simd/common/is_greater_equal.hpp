...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto is_greater_equal_(EVE_SUPPORTS(simd_),
                                wide<T, N, ABI> const &v0,
                                wide<T, N, ABI> const &v1) noexcept 
  {
    return eve::is_less(v1, v0) || is_equal(v0, v1); 
  }
  ...
  {
    return eve::is_greater_equal(v0, wide<T, N, ABI>(static_cast<T>(v1)));
  }

  template<typename T, typename N, typename ABI, typename U>
  EVE_FORCEINLINE auto
  is_greater_equal_(EVE_SUPPORTS(simd_),
           U const &              v0,
           wide<T, N, ABI> const &v1) noexcept requires(wide<logical<T>, N, ABI>,
                                                        detail::Convertible<U, T>)
  {
    return eve::is_greater_equal(wide<T, N, ABI>(static_cast<T>(v0)), v1);
  }
  ...
  {
    return aggregate(eve::is_greater_equal, v0, v1);
  }
  ...
  {
    return map(eve::is_greater_equal, v0, v1);
  }
}
