...
**/
//==================================================================================================
#ifndef EVE_MODULE_CORE_FUNCTION_SIMD_COMMON_DIV_UPWARD_HPP_INCLUDED
#define EVE_MODULE_CORE_FUNCTION_SIMD_COMMON_DIV_UPWARD_HPP_INCLUDED
...
namespace eve::detail
{
          ...
          )
  {
    ...
    {
      return map( eve::div, abi_cast<value_type_t<U>>(a), abi_cast<value_type_t<T>>(b), upt_);
    }
    else if constexpr( is_aggregated_v<t_abi> || is_aggregated_v<u_abi> )
    {
      return aggregate( eve::div, abi_cast<value_type_t<U>>(a)
                      , abi_cast<value_type_t<T>>(b), upt_ );
    }
    else if constexpr( is_vectorized_v<T> ^ is_vectorized_v<U> )
    {
      return eve::div(abi_cast<U>(a), abi_cast<T>(b), upt_ );
    }
    else if constexpr( is_vectorized_v<T> && is_vectorized_v<U> )
    {
      if constexpr(std::is_same_v<T, U>)
      {
        ...
        else if constexpr(std::is_signed_v<v_t>)
        {
          ...
          {
            return if_else(is_nez(b)
                          , saturated_(convert)(iceil(convert(a, double_)/
