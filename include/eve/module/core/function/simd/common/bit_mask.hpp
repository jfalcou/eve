...
namespace eve::detail
{
  ...
  {
    if constexpr(native<ABI>) { return is_nez(v).mask(); }
    else
    {
      if constexpr(aggregated<ABI>) return aggregate(eve::bit_mask, v);
      if constexpr(emulated<ABI>) return map(eve::bit_mask, v);
    }
  }
  ...
  {
    if constexpr(native<ABI>) { return v.mask(); }
    else
    {
      if constexpr(aggregated<ABI>) return aggregate(eve::bit_mask, v);
      if constexpr(emulated<ABI>) return map(eve::bit_mask, v);
