...
**/
//==================================================================================================
#ifndef EVE_MODULE_CORE_FUNCTION_SIMD_COMMON_BIT_XOR_HPP_INCLUDED
#define EVE_MODULE_CORE_FUNCTION_SIMD_COMMON_BIT_XOR_HPP_INCLUDED
...
namespace eve::detail
{
          ...
          )
  {
    ...
    if constexpr(is_vectorizable_v<T> && !is_vectorizable_v<U>)
    {
      ...
      {
        return eve::bit_xor(U(bit_cast(a,as_<vu_t>())), b);
      }
      else return U();
    }
    else if constexpr(is_vectorizable_v<U> && !is_vectorizable_v<T>)
    {
      ...
      // in the case vector and scalar not of the value type
      {
        return eve::bit_xor(a, T(bit_cast(b,as_<vt_t>())));
      }
      else return T();
    }    else if constexpr(is_emulated_v<t_abi> || is_emulated_v<u_abi>)
    {
      return map(eve::bit_xor, abi_cast<value_type_t<U>>(a), abi_cast<vt_t>(b));
    }
    else if constexpr(is_aggregated_v<t_abi> || is_aggregated_v<u_abi>)
    {
      return aggregate(eve::bit_xor, abi_cast<value_type_t<U>>(a), abi_cast<vt_t>(b));
    }
    else if constexpr(is_vectorized_v<T> && !is_vectorized_v<U>)
    {
      return eve::bit_xor(a, T{b});
    }
    else if constexpr(is_vectorized_v<T> && is_vectorized_v<U>)
    {
      return eve::bit_xor(a, bit_cast(b,as(a)));
    }
    else
    {
      static_assert(wrong<T, U>, "[eve::bit_xor] - Missing implementation");
      return T();
    }
  }
}
...
namespace eve
{
  ...
  EVE_FORCEINLINE auto operator^(T const &v0, U const &v1) noexcept
      -> decltype(eve::bit_xor(v0, v1))
  {
    return eve::bit_xor(v0, v1);
  }
}
