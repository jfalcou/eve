...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE as_logical_t<wide<T, N, ABI>> is_equal_with_equal_nans_(EVE_SUPPORTS(simd_),
                                                                          wide<T, N, ABI> const &v0,
                                                                          wide<T, N, ABI> const &v1) noexcept 
  {
    if constexpr(std::is_floating_point_v<T>)
      return logical_or(is_equal(v0, v1), logical_and(is_nan(v0),  is_nan(v1)));
    else
      return is_equal(v0, v1); 
  }
  ...
  EVE_FORCEINLINE auto is_equal_with_equal_nans_(EVE_SUPPORTS(simd_),
                                 wide<T, N, ABI> const &v0,
                                 U const &v1) noexcept requires(as_logical_t<wide<T, N, ABI>>,
                                                                detail::Convertible<U, T>)
  {
    return eve::is_equal_with_equal_nans(v0, wide<T, N, ABI>(static_cast<T>(v1)));
  }
  ...
  EVE_FORCEINLINE auto is_equal_with_equal_nans_(EVE_SUPPORTS(simd_),
                                 ...
                                 wide<T, N, ABI> const &v1) noexcept requires(as_logical_t<wide<T, N, ABI>>,
                                                                              detail::Convertible<U, T>)
  {
    return eve::is_equal_with_equal_nans(wide<T, N, ABI>(static_cast<T>(v0)), v1);
  }
  ...
  {
    return aggregate(eve::is_equal_with_equal_nans, v0, v1);
  }
  ...
  {
    return map(eve::is_equal_with_equal_nans, v0, v1);
  }
}
