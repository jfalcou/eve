...
namespace eve::detail
{
      ...
      detail::Either<is_vectorized_v<T>, is_vectorized_v<U>>)
  {
    ...
    if constexpr(is_vectorizable_v<U>  && !is_vectorizable_v<T>)
    {
      ...
      {
        return eve::bitwise_notor(a, T(bitwise_cast(b,as_<vt_t>())));
      }
      return T(); 
    }
    else if constexpr(is_vectorizable_v<T> && !is_vectorizable_v<U>)
    {
      ...
      if constexpr(sizeof(T) == sizeof(vt_u))
        ...
        // in the case vector and scalar not of the value type
      {
        return eve::bitwise_notor(t_t(a), bitwise_cast(b,as_<t_t>()));
      }
      return t_t(); 
    } 
    ...
    {
      return aggregate(
          eve::bitwise_notor, abi_cast<value_type_t<U>>(a), abi_cast<value_type_t<T>>(b));
    }
    ...
    {
      static_assert(wrong<T, U>, "[eve::bitwise_notor] - Missing implementation"); 
      return std::conditional_t<is_vectorized_v<T>, T, U>(); 
    }
    else
    {
      return eve::bitwise_or(bitwise_not(a), bitwise_cast(b,as(a)));
    }
  }
}
