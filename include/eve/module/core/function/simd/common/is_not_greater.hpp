...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto is_not_greater_(EVE_SUPPORTS(simd_),
                                wide<T, N, ABI> const &v0,
                                wide<T, N, ABI> const &v1) noexcept 
  {
    if constexpr(std::is_integral_v<T>) return is_less_equal(v0, v1); 
    if constexpr(std::is_floating_point_v<T>)
      return logical_or(is_less_equal(v0, v1), is_unordered(v0, v1));
  }
...
// -----------------------------------------------------------------------------------------------
  ...
  {
    return eve::is_not_greater(v0, wide<T, N, ABI>(static_cast<T>(v1)));
  }

  template<typename T, typename N, typename ABI, typename U>
  EVE_FORCEINLINE auto
  is_not_greater_(EVE_SUPPORTS(simd_),
           U const &              v0,
           wide<T, N, ABI> const &v1) noexcept requires(wide<logical<T>, N, ABI>,
                                                        detail::Convertible<U, T>)
  {
    return eve::is_not_greater(wide<T, N, ABI>(static_cast<T>(v0)), v1);
  }
  ...
  {
    return aggregate(eve::is_not_greater, v0, v1);
  }
  ...
  {
    return map(eve::is_not_greater, v0, v1);
  }
}
