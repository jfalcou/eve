...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N, ABI> minus_(EVE_SUPPORTS(cpu_),
                                               wide<T, N, ABI> const &v) noexcept
  {
    ...
    {
      if constexpr(is_aggregated_v<ABI>) return aggregate(eve::minus, v);
      if constexpr(is_emulated_v<ABI>) return map(eve::minus, v);
    }
  }
