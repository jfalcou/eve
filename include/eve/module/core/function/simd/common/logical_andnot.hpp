...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE constexpr auto logical_andnot_(EVE_SUPPORTS(cpu_), T const &a, U const &b) noexcept
                                    ...
                                    )
  {
    ...
    else
    {
      ...
      {
        static_assert( std::is_same_v<T,U>, "[eve::logical_andnot] - Incompatible types.");
        return {};
      }
    }
  }
  ...
  template<typename T, typename U>
  EVE_FORCEINLINE constexpr auto logical_andnot_( EVE_SUPPORTS(cpu_),
  ...
  {
    return bitwise_cast<logical<T>>( bitwise_andnot(bitwise_mask(a), bitwise_mask(b)) );
  }
}
