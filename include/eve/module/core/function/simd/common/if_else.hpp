...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T,N,aggregated_> if_else_ ( EVE_SUPPORTS(simd_)
  ...
  if_else_(EVE_SUPPORTS(simd_), wide<T, N, ABI> const &v0, T const &v1, T const &v2) noexcept
  {
  ...
  template<typename T, typename N, typename ABI>
  EVE_FORCEINLINE auto
  if_else_(EVE_SUPPORTS(simd_), wide<T, N, ABI> const &v0, wide<T, N, ABI> const &v1, T const &v2) noexcept
  {
    return eve::if_else(v0, v1, wide<T, N>(v2));
  }

  template<typename T, typename N, typename ABI>
  EVE_FORCEINLINE auto
  if_else_(EVE_SUPPORTS(simd_), wide<T, N, ABI> const &v0, T const &v1, wide<T, N, ABI> const &v2) noexcept
  {
    return eve::if_else(v0, wide<T, N>(v1), v2);
  }
