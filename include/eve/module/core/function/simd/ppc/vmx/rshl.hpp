...
**/
//==================================================================================================
#ifndef EVE_MODULE_CORE_FUNCTION_SIMD_PPC_VMX_RSHL_HPP_INCLUDED
#define EVE_MODULE_CORE_FUNCTION_SIMD_PPC_VMX_RSHL_HPP_INCLUDED
...
namespace eve::detail
{
  ...
  rshl_(EVE_SUPPORTS(vmx_), wide<T, N, ppc_> const &v0, wide<I, N, ppc_> const &v1) noexcept
  requires(wide<T, N, ppc_>, Integral<I>, Integral<T>)
  {
    using t_t = wide<T, N, ppc_>;
    EVE_ASSERT( detail::assert_good_shift<t_t>(v1)
              , "[eve::rshl ppc] - At least one of " << v1 << "elements is out of the range [0, "
                                                    << sizeof(T)*8 << "[."
              );
  ...
  template<typename T, typename N, typename I>
  EVE_FORCEINLINE auto
  rshl_(EVE_SUPPORTS(vmx_), wide<T, N, ppc_> const &v0, I v1) noexcept
  requires(wide<T, N, ppc_>, Integral<I>, Integral<T>)
  {
    using i_t = wide<as_integer_t<T,unsigned>,N>;
