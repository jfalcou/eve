  ...
  requires(contains<D>(types<regular_type, pedantic_type> {}))
  {
    if constexpr(has_native_abi_v<T>)
    {
      ...
      if constexpr(scalar_value<T>)
      {
        ...
        if (xgemaxlog) return Inf<T>();
        if (xlelogeps) return Mone<T>();
      }
      ...
      else if constexpr(std::is_same_v<elt_t, double>)
      {
        ...
        T x  = hi-lo;
        T hxs = sqr(x)*Half<T>();
        T r1 = horn<T, 0X3FF0000000000000ULL, 0XBFA11111111110F4ULL, 0X3F5A01A019FE5585ULL
          , 0XBF14CE199EAADBB7ULL, 0X3ED0CFCA86E65239ULL,  0XBE8AFDB76E09C32DULL> (hxs);
        T t  = T(3)-r1*Half<T>()*x;
        T e  = hxs*((r1-t)/(T(6) - x*t));
        T c = (hi-x)-lo;
        e  = (x*(e-c)-c)-hxs;
