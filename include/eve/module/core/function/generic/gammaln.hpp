...
namespace eve::detail
{
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  /// helpers
  template<floating_real_value T> EVE_FORCEINLINE T gammalnB(T x) noexcept
  {
    // log gamma(x+2), -.5 < x < .5
    return horn<T,
  ...
  template<floating_real_value T> EVE_FORCEINLINE T gammalnC(T x) noexcept
  {
  ...
  template<floating_real_value T> EVE_FORCEINLINE T gammaln2(T p) noexcept
  {
    return horn<T,
  ...
  template<floating_real_value T> EVE_FORCEINLINE T gammaln1(T x) noexcept
  {
    return horn<T,
  ...
  template<floating_real_value T> EVE_FORCEINLINE T gammalnA(const T &p) noexcept
  {
    return horn<T,
  ...
  EVE_FORCEINLINE constexpr T gammaln_(EVE_SUPPORTS(cpu_), D const &, T a0) noexcept
      requires(is_one_of<D>(types<regular_type, pedantic_type> {}))
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      if constexpr( std::is_same_v<elt_t, float> )
      {
        if constexpr( scalar_value<T> ) // float scalar
        {
          ...
          auto gammaln_pos = [Logsqrt2pi](T x) {
            ...
            if( x <= 1.0e4f )
            {
              T z = rec(x);
              T p = sqr(z);
              q   = fma(z, gammaln2(p), q);
            }
            return q;
          };

          if( (a0 > Maxgammaln) || is_eqz(a0) )
            return Inf<T>();
          T x = a0;
          T q = abs(x);
          if( x < 0.0f )
        ...
        {
          auto inf_result = is_lez(a0) && is_flint(a0);
          T    x          = if_else(inf_result, eve::allbits_, a0);
          T    q          = abs(x);
          if constexpr( eve::platform::supports_infinites )
          {
            inf_result = is_equal(x, Inf<T>()) || inf_result;
          }
          ...
          auto   other = [Logsqrt2pi](T x) {
            ...
            if( nb > 0 )
            {
              auto kernelC = False<T>();
              T    z       = One<T>();
              T    tx      = if_else(xlt650, x, eve::zero_);
              T    nx      = Zero<T>();
              ...
              if( any(xge125) )
              {
                ...
                r0s = if_else(xge125t, Mone<T>(), r0s);
              }
              // x >= 0.75&& x < 1.5
              ...
              if( any(xge075t) )
              {
                ...
                r0z     = if_else(xge075t, One<T>(), r0z);
                r0s     = if_else(xge075t, Mone<T>(), r0s);
                p       = gammalnC(r0x);
              }
              // tx < 1.5 && x < 0.75
              ...
              if( any(txlt150) )
              {
                ...
                r0s = if_else(orig, Mone<T>(), r0s);
              }
              p = if_else(kernelC, p, gammalnB(r0x));
              if( nb >= cardinal_v<T> )
            ...
            T m  = log(r0z);
            r1   = fma(r0x, p, r0s * m);
            T r2 = fma(x - Half<T>(), m, Logsqrt2pi - x);
            r2 += gammaln2(rec(sqr(x))) / x;
          ...
          if( nb > 0 ) // some are negative
          {
            ...
            // treat negative
            r = if_else(inf_result, Inf<T>(), negative(q, r1));
            if( nb >= cardinal_v<T> )
              return r;
          }
          T r2 = if_else(ltza0, r, r1);
          return if_else(is_equal(a0, Minf<T>()), Nan<T>(), if_else(inf_result, Inf<T>(), r2));
        }
      }
      else if constexpr( std::is_same_v<elt_t, double> ) // double
      {
        ...
        if constexpr( scalar_value<T> ) // scalar double
        {
          const T Maxgammaln =
              Ieee_constant<T, 0x7bc3f8eaU, 0x7f574c5dd06d2516ULL>(); // 2.035093e36f, 2.556348e305
          auto gammaln_pos = [Logsqrt2pi](double x) {
            if( x < 13.0 )
            {
              T z = One<T>();
              T p = Zero<T>();
          ...
          if( (a0 == Inf<T>()) || is_eqz(a0) )
            return Inf<T>(); // 2.556348e305
          T x = a0;
          T q = abs(x);
          if( x > Maxgammaln )
            return Inf<T>();
        ...
        else if constexpr( simd_value<T> ) // simd double
        {
          auto other = [Logsqrt2pi](T xx) {
            ...
            T r2 = fma(xx - Half<T>(), log(xx), Logsqrt2pi - xx);
            T p  = rec(sqr(xx));
            r2 += gammalnA(p) / xx;
          ...
          T    x          = if_else(inf_result, eve::allbits_, a0);
          T    q          = abs(x);
          if (eve::platform::supports_infinites) inf_result = inf_result || is_equal(q, Inf<T>());
          ...
          if( nb > 0 )
          {
            ...
            r = large_negative(q);
            if( nb >= T::static_size )
              return if_else(is_equal(a0, Minf<T>()), eve::allbits_, if_else(inf_result, Inf<T>(), r));
          }
          T r1 = other(a0);
          T r2 = if_else(test, r, r1);
          return if_else(is_equal(a0, Minf<T>()), eve::allbits_, if_else(inf_result, Inf<T>(), r2));
        }
      }
    }
    else
      return apply_over(gammaln, a0);
  }

  template<floating_real_value T>
  EVE_FORCEINLINE constexpr T gammaln_(EVE_SUPPORTS(cpu_), T const &x) noexcept
  {
    return gammaln(regular_type(), x);
  }
}
