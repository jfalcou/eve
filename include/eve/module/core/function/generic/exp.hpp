...
namespace eve::detail
{
  ...
  requires std::same_as<D,pedantic_type> || std::same_as<D,regular_type>
  {
    if constexpr(native<T>)
    {
      ...
      {
        if (xgemaxlog) return Inf(as(x));
        if (xltminlog) return Zero(as(x));
      }
      ...
      else if constexpr(std::is_same_v<elt_t, double>)
      {
        ...
        x = hi-lo;
        auto t = sqr(x);
        c = fnma(t, horn<T, 0x3fc555555555553eull, 0xbf66c16c16bebd93ull, 0x3f11566aaf25de2cull
                , 0xbebbbd41c5d26bf1ull, 0x3e66376972bea4d0ull>(t), x); //x-h*t
