...
namespace eve::detail
{
                                     ...
                                     , T a0) noexcept
  {
    ...
    {
      if constexpr(scalar_value<T>) if (is_not_less_equal(eve::abs(a0), Pio_4<T>())) return Nan<T>();
                                      ...
                                      , T a0) noexcept
  {
    if constexpr(native<T>)
    {
      ...
      {
        auto fn = binarize(is_not_less_equal(x, Pio_4(as(x))));
  ...
  //////////////////////////////////////////////////////////////////
  /// medium,  big
  template<typename D, floating_real T>
  EVE_FORCEINLINE constexpr auto cot_(EVE_SUPPORTS(cpu_)
                                     , D const &
                                     , T a0) noexcept
  {
