...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE wide<T, N> minabs_(EVE_SUPPORTS(avx512_),
                                     wide<T, N> const &v0,
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else  return minabs_(EVE_RETARGET(cpu_), v0, v1);
  }
  ...
  EVE_FORCEINLINE
  wide<T, N> minabs_(EVE_SUPPORTS(avx512_), C const &cx, wide<T, N> const &v, wide<T, N> const &w) noexcept
  requires x86_abi<abi_t<T, N>>
  {
    ...
    else
    {
      ...
      else     return minabs_(EVE_RETARGET(cpu_),cx,v,w);
    }
  }
}
