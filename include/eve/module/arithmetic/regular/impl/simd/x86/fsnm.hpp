...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE
  wide<T, N> fsnm_(EVE_SUPPORTS(sse2_), C const &cx
                 , wide<T, N> const &v
                 , wide<T, N> const &w
  ...
  requires x86_abi<abi_t<T, N>>
  {
    ...
    {
      return fsnm_(EVE_RETARGET(cpu_),cx,v,w,x);
    }
    else
    {
      ...
      {
              if constexpr(c == category::float32x16) return _mm512_mask3_fnmsub_ps(w,x,v,m);
        ...
        else  return fsnm_(EVE_RETARGET(cpu_),cx,v,w,x);
      }
      else
      {
