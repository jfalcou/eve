...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto ellint_rd_(EVE_SUPPORTS(cpu_)
                              , T x
                              , U y
                              , V z) noexcept
  -> decltype(arithmetic_call(ellint_rd, x, y, z))
  {
    return arithmetic_call(ellint_rd, x, y, z);
  }

  template<floating_real_value T, floating_real_value U,  floating_real_value V>
  EVE_FORCEINLINE T ellint_rd_(EVE_SUPPORTS(cpu_)
                              , raw_type const &
                              , T x
                              , U y
                              , V z) noexcept
                              ...
                              , T z) noexcept
  {
    ...
    if constexpr(has_native_abi_v<T>)
    {
      ...
      T q = epsi * (eve::max)((eve::max)(eve::abs(an - xn), eve::abs(an - yn)), eve::abs(an - zn))*elt_t(1.2);
      T fn(one(as(x)));
      T rd_sum(zero(as(x)));
      ...
      return apply_over(raw(ellint_rd), x, y, z);
  }
                              ...
                              , T z) noexcept
  {
    if constexpr(has_native_abi_v<T>)
    {
      ...
      // z equal to zero or any parameter nan or less than zero or more than one of x and y zero implies nan
      auto br0 = [x, y, z](){return raw(ellint_rd)(x, y, z);};
      last_interval(br0, notdone, r);
      return r;
    }
     else
      return apply_over(ellint_rd, x, y, z);
   }
