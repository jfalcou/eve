...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE bool all_(EVE_SUPPORTS(sse2_), logical<wide<T, N, ABI>> const& v) noexcept
  {
    if constexpr( N::value == 1 )
      ...
      return eve::detail::all(eve::detail::top_bits {v});
  }
  ...
  all_(EVE_SUPPORTS(sse2_), C const& cond, logical<wide<T, N, ABI>> const& v) noexcept
  {
  ...
  EVE_FORCEINLINE bool
  all_(EVE_SUPPORTS(sse2_), C const& cond, logical<wide<T, N, aggregated_>> const& v) noexcept
  {
    return all_ignore_impl(cond, v);
  }
}
