...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto is_not_less_equal_(EVE_SUPPORTS(cpu_)
                                ...
                                , T const &a
                                , U const &b) noexcept
  -> decltype(arithmetic_call(definitely(is_not_less_equal), a, b, 3*eps(as(a))))
  {
    return arithmetic_call(definitely(is_not_less_equal), a, b, 3*eps(as(a)));
  }
  ...
  requires compatible_values<T, U>
  {
    if constexpr(integral_value<V>)
    {
      ...
      c_t aa(a);
      c_t bb(b);
      return definitely(is_not_less_equal)(aa, bb, tol);
    }
    else return arithmetic_call(definitely(is_not_less_equal), a, b, tol);
  }
                                ...
                                , [[maybe_unused]] V const &tol) noexcept
  {
    if constexpr(integral_value<V>)
    {
      if constexpr(simd_value<V> && scalar_value<T>)
      {
        ...
        return definitely(is_not_less_equal)(c_t(a), c_t(b), convert(tol, as<i_t>()));
      }
      else
      {
        return is_not_less_equal(a, saturated(next)(b, tol));
      }
    }
    else
    {
      return is_not_less_equal(a, saturated(add)(b, tol*max(eve::abs(a), eve::abs(b))));
    }
  }
}
