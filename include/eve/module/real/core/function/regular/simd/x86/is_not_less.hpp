...
namespace eve::detail
{
    ...
    requires std::same_as<abi_t<T, N>, x86_128_>
  {
    ...
    else if constexpr(std::is_same_v<T, float>)  return l_t(_mm_cmpnlt_ps(v0, v1));
    ...
    requires std::same_as<abi_t<T, N>, x86_256_>
  {
    ...
    else if constexpr(std::is_same_v<T, float>)  return l_t(_mm256_cmp_ps(v0, v1, _CMP_NLT_UQ));
