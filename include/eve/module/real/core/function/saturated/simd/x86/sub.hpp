...
namespace eve::detail
{
 ...
 // -----------------------------------------------------------------------------------------------
  ...
  EVE_FORCEINLINE
  wide<T, N> sub_(EVE_SUPPORTS(sse2_), C const &cx, saturated_type st, wide<T, N> const &v, wide<T, N> const &w) noexcept
  requires x86_abi<abi_t<T, N>>
  {
    constexpr auto c = categorize<wide<T, N>>();
    ...
    else
    {
      ...
      else if constexpr ( current_api >= avx2 && c == category::int16x16  ) return _mm256_mask_subs_epi16(src,m,v,w);
      else if constexpr ( current_api >= avx2 && c == category::uint16x16 ) return _mm256_mask_subs_epu16(src,m,v,w);
      else if constexpr ( current_api >= avx2 && c == category::int8x32   ) return _mm256_mask_subs_epi8 (src,m,v,w);
      else if constexpr ( current_api >= avx2 && c == category::uint8x32  ) return _mm256_mask_subs_epu8 (src,m,v,w);
