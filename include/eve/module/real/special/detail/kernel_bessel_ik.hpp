...
namespace eve::detail
{
  ...
  auto  kernel_bessel_ik(T nu, T x) noexcept
  {
    ...
    else if (is_eqz(x))
    {
      if (is_eqz(nu))
      {
        return kumi::make_tuple(T(1), T(0), inf(as(x)), minf(as(x))); //jnu, jpnu, nnu, npnu
      }
      else if (nu ==one(as(x)))
      {
        return kumi::make_tuple(T(0), T(0.5), inf(as(x)), minf(as(x))); //jnu, jpnu, nnu, npnu
      }
      else
      {
        return kumi::make_tuple(T(0), T(0), inf(as(x)), minf(as(x))); //jnu, jpnu, nnu, npnu
      }
    }
    T Inu,  Knu,  Ipnu,  Kpnu;
...
//     if (i > max_iter)
//         eve::throw_runtime_error(N("Argument x too large "
//                                        "in bessel_ik; "
...
//       if (i > max_iter)
//         eve::throw_runtime_error(N("Bessel k series failed to converge "
//                                    "in bessel_ik."));
      Kmu = sum;
      Knu1 = sum1 * xi2;
    }
    else
    {
      ...
      for (i = 2; i <= max_iter; ++i)
      {
        ...
        const T dels = q * delh;
        s += dels;
        if ( eve::abs(dels / s) < Eps ) break;
      }
//       if (i > max_iter)
//         eve::throw_runtime_error(N("Steed's method failed "
//                                    "in bessel_ik."));
      h = a1 * h;
