...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto cyl_bessel_in_(EVE_SUPPORTS(cpu_), I nu, T x) noexcept
  {
    ...
    {
      auto [in, ipn, kn, kpn] = kernel_bessel_ik(T(nu), x);
      return in;
...
//   EVE_FORCEINLINE T cyl_bessel_in_(EVE_SUPPORTS(cpu_), T nu, T x) noexcept
//   {
//     if constexpr(scalar_value<T>)
//     {
//       auto [in, ipn, kn, kpn] = kernel_bessel_ik(nu, x);
//       return in;
// //       auto flintx = is_flint(nu);
