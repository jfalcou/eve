...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_flint_, auto const& z ) noexcept
  {
    return trunc(z) == z;
  }
  ...
  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_even_, auto const& z ) noexcept
  {
    return 2*trunc(z/2) == z;
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_gez_, auto const& z ) noexcept
  {
    return is_gez(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_gtz_, auto const& z ) noexcept
  {
    return is_gtz(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_lez_, auto const& z ) noexcept
  {
    return is_lez(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_ltz_, auto const& z ) noexcept
  {
    return is_ltz(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_negative_, auto const& z ) noexcept
  {
  ...
  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_ngez_, auto const& z ) noexcept
  {
    return is_ngez(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_ngtz_, auto const& z ) noexcept
  {
    return is_ngtz(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_nlez_, auto const& z ) noexcept
  {
    return is_nlez(high(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_nltz_, auto const& z ) noexcept
  {
  ...
  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_not_flint_, auto const& z ) noexcept
  {
    return z != trunc(z);
  }
  ...
  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_odd_, auto const& z ) noexcept
  {
    return (z != inc(z)) && is_even(inc(z));
  }

  EVE_FORCEINLINE auto dd_unary_dispatch( tag::is_positive_, auto const& z ) noexcept
  {
