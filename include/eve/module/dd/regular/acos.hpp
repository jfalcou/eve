...
namespace eve
{
  ...
  namespace detail
  {
    template<floating_value V> EVE_FORCEINLINE V acos_(EVE_SUPPORTS(cpu_), cmplx_type const &, V const & v) noexcept
    {
      using r_t = eve::as_complex_t<V>;
      return if_else(is_lez(v, V(1)), r_t(acos(v)), r_t(0, acosh(v)));
    }

    template<typename V> EVE_FORCEINLINE V internal_acos(V const & a0) noexcept
    {
      // here V is like<complex>
      //
