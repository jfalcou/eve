...
namespace eve
{
  namespace detail
  {
    template<floating_value V> EVE_FORCEINLINE V atanh_( EVE_SUPPORTS(cpu_)
                                                       , cmplx_type const &
                                                       , V const & v) noexcept
    {
      using r_t = eve::as_complex_t<V>;
      return if_else(is_lez(v, V(1)), r_t(asin(v)), r_t(0, asinh(v)));
    }

    template<typename V> EVE_FORCEINLINE V internal_atanh(V const & a0) noexcept
    {
