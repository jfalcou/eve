...
//!  auto operator()(value auto x, std::inputiterator auto first, std::inputiterator auto sentinel)
//!  const noexcept; auto operator()(value auto x, callable_one_ const & o, value auto... args)
//!  const noexcept; auto operator()(value auto x, callable_one_ const & o, range auto r) const
//!  noexcept; auto operator()(value auto x, callable_one_ const & o, std::inputiterator auto first,
//!  std::inputiterator auto sentinel) const noexcept;
//!  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//!
//! ---
//!
...
//!  **Return value**
//!
//!  A Callable object so that the expression `reverse_horner[cond](x, ...)` is equivalent to
//!  `if_else(cond,reverse_horner(x, ...),x)`
//!
//! ---
//!
...
//!
//!
//!     The expression `pedantic(reverse_horner)(...)` computes the result using
//!     `pedantic(fma)(a,x,b)` for `a*x+b` instead of
//!      `fma(a,x,b)`.
//!
//!  * eve::numeric
//!
//!
//!     The expression `numeric(reverse_horner)(...)` computes the result using
//!     `numeric(fma)(a,x,b)` for `a*x+b` instead of
//!      `fma(a,x,b)`.
//!
//! #### Example
//!
