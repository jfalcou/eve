...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto jacobi_(EVE_SUPPORTS(cpu_), I n, U alpha, V beta, T x) noexcept
  {
  ...
  EVE_FORCEINLINE auto jacobi_(EVE_SUPPORTS(cpu_)
                              , I nn, U aalpha, V abeta, T xx) noexcept
  {
   ...
   if constexpr(has_native_abi_v<T>)
    {
      ...
      while(eve::any(test))
      {
        ...
        yk = if_else(test, fma(gamma1, y1, gamma0*y0)/denom, yk);//sum_of_prod(gamma1, y1, gamma0, y0)/denom, yk);
        y0 = y1;
        y1 = yk;
        k = inc(k);
        k2 += 2;
    ...
    return yk;
      ...
      return apply_over(jacobi, alpha, beta, n, x);
  }
}
