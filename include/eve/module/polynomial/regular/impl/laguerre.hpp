...
// Recurrence relation for Laguerre polynomials:
  ...
  template<scalar_value I, floating_value T, callable_options O>
  ...
  template<simd_value I, floating_scalar_value T, callable_options O>
  EVE_FORCEINLINE auto
  laguerre_(EVE_REQUIRES(cpu_), O const&, I nn, T x) noexcept
  {
    using f_t = as_wide_t<T, cardinal_t<I>>;
    return laguerre(nn, f_t(x));
  }

  template<simd_value I, floating_simd_value T, callable_options O>
  EVE_FORCEINLINE auto
  laguerre_(EVE_REQUIRES(cpu_), O const&, I nn, T x) noexcept
  {
...
// Recurrence relation for Laguerre associated polynomials:
  template<value N, value L, floating_value T, callable_options O>//successor
  EVE_FORCEINLINE auto
  laguerre_(EVE_REQUIRES(cpu_), O const&, N n, L l, T x, T pl, T plm1) noexcept
  {
    auto np1 = inc(n);
    auto npl = n + l;
...
// associated laguerre polynomials
  template<integral_scalar_value M, integral_scalar_value N, floating_value T, callable_options O>
  EVE_FORCEINLINE auto
  laguerre_(EVE_REQUIRES(cpu_), O const&, N n, M m, T x) noexcept
  {
    // Special cases:
    ...
    {
      std::swap(p0, p1);
      p1 = laguerre[successor](c, m, x, p0, p1);
      ++c;
    }
    return p1;
  }

  template<value M, value N, floating_value T, callable_options O>
  EVE_FORCEINLINE auto
  laguerre_(EVE_REQUIRES(cpu_), O const&, N nn, M mm, T x) noexcept
  {
