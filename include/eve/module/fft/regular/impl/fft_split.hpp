...
namespace eve::detail
{
  ...
  //  requires(std::is_floating_point_v<typename R::value_type>)
  {
    ...
    auto flip = [](auto  c){ return c_t(imag(c), real(c)); };
    ...
    for (i_t k=0; k<ldn; k++)
    {
      ...
      for (i_t j=0; j<n4; j++)
      {
        ...
        auto sc1 = flip(cs1);
//         auto [ss1, cc1] = sincos(a);
//         auto [ss3, cc3] = sincos(3*a);
// //         T cc1, ss1, cc3, ss3;
//             SinCos(a, &ss1, &cc1);
...
// #endif
        ...
        while ( ix<n )
        {
          for (i_t i0=ix; i0<n; i0+=id)
          {
           ...
           { auto [rr, ii] = sc1*c_t(t4, tr0); *fri2 = rr;  *fii2 = ii; }
            //           cmult(ss1, cc1, t4, tr0, *fri2], *fii2]);

            { auto [rr, ii] =  cs3*c_t(tr1, t3); *fii3 = rr;  *fri3 = ii; }
            //          cmult(cc3, ss3, tr1, t3, *fii3], *fri3]);
          }
