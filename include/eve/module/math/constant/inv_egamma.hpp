...
namespace eve
{
  ...
  //! | Member       | Effect                                                     |
  //! |:-------------|:-----------------------------------------------------------|
  //! | `operator()` | Computes the inv_egamma constant                              |
  //!
  //! ---
  //!
  //!  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~{.cpp}
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto inv_egamma_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.bb8226p+0;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.bb8226f502bf8p+0;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.bb8228p+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.bb8226f502bf8p+0;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.bb8226p+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.bb8226f502bf7p+0;
      }
    }
  }
}
