...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto half_pi_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return T(0x1.921fb6p+0);
      else if constexpr(std::is_same_v<t_t, double>) return T(0x1.921fb54442d18p+0);
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return T(0x1.921fb6p+0);
        else if constexpr(std::is_same_v<t_t, double>) return T(0x1.921fb54442d19p+0);
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return T(0x1.921fb4p+0);
        else if constexpr(std::is_same_v<t_t, double>) return T(0x1.921fb54442d18p+0);
      }
    }
  }
