...
namespace eve
{
  ...
  struct inv_pi_t : constant_callable<inv_pi_t, Options, lower_option, upper_option>
  {
    ...
    static EVE_FORCEINLINE constexpr T value(as<T>, Opts const&)
    {
      ...
      {
        if      constexpr (Opts::contains(upper)) return T{0x1.45f308p-2};
        else if constexpr (Opts::contains(lower)) return T{0x1.45f306p-2};
        else                                      return T{0x1.45f306p-2};
      }
      else
      {
        if      constexpr (Opts::contains(upper)) return T{0x1.45f306dc9c883p-2};
        else if constexpr (Opts::contains(lower)) return T{0x1.45f306dc9c882p-2};
