...
struct two_o_sqrt_pi_t : constant_callable<two_o_sqrt_pi_t, Options, downward_option, upward_option>
{
  ...
  static EVE_FORCEINLINE constexpr T value(eve::as<T> const&, Opts const&)
  {
    ...
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.20dd76p+0);
      else if constexpr(Opts::contains(downward2)) return T(0x1.20dd74p+0);
      else                                         return T(0x1.20dd76p+0);
    }
    else
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.20dd750429b6ep+0);
      else if constexpr(Opts::contains(downward2)) return T(0x1.20dd750429b6dp+0);
