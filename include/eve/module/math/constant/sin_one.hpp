...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto sin_one_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.aed548p-1;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.aed548f090ceep-1;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.aed54ap-1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.aed548f090cefp-1;
      }
