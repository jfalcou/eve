...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto sinh_one_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.2cd9fcp+0;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.2cd9fc44eb982p+0;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.2cd9fep+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.2cd9fc44eb983p+0;
      }
