...
struct cos_1_t : constant_callable<cos_1_t, Options, downward_option, upward_option>
{
  ...
  static EVE_FORCEINLINE constexpr T value(eve::as<T> const&, Opts const&)
  {
    ...
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.14a282p-1);
      else if constexpr(Opts::contains(downward2)) return T(0x1.14a28p-1);
      else                                         return T(0x1.14a28p-1);
    }
    else
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.14a280fb5068cp-1);
      else if constexpr(Opts::contains(downward2)) return T(0x1.14a280fb5068bp-1);
