...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto glaisher_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.484d24p+0;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.484d24f2fd873p+0;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.484d26p+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.484d24f2fd874p+0;
      }
