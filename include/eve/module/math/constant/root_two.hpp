...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto root_two_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return T(0x1.6a09e6p+0);
      else if constexpr(std::is_same_v<t_t, double>) return T(0x1.6a09e667f3bcdp+0);
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return T(0x1.6a09e8p+0);
        else if constexpr(std::is_same_v<t_t, double>) return T(0x1.6a09e667f3bcdp+0);
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return T(0x1.6a09e6p+0);
        else if constexpr(std::is_same_v<t_t, double>) return T(0x1.6a09e667f3bccp+0);
      }
    }
  }
}
