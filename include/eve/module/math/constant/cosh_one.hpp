...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto cosh_one_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.8b0756p+0;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.8b07551d9f55p+0;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.8b0756p+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.8b07551d9f551p+0;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.8b0754p+0;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.8b07551d9f55p+0;
      }
    }
  }
