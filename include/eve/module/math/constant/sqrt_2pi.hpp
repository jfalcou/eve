...
struct sqrt_2pi_t : constant_callable<sqrt_2pi_t, Options, downward_option, upward_option>
{
  ...
  static EVE_FORCEINLINE constexpr T value(eve::as<T> const&, Opts const&)
  {
    ...
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.40d932p+1);
      else if constexpr(Opts::contains(downward2)) return T(0x1.40d93p+1);
      else                                         return T(0x1.40d932p+1);
    }
    else
    {
      if constexpr(Opts::contains(upward2))        return T(0x1.40d931ff62706p+1);
      else if constexpr(Opts::contains(downward2)) return T(0x1.40d931ff62705p+1);
