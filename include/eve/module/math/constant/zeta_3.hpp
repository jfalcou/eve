...
namespace eve
{
  ...
  struct zeta_3_t : constant_callable<zeta_3_t, Options, lower_option, upper_option>
  {
    ...
    static EVE_FORCEINLINE constexpr T value(as<T>, Opts const&)
    {
      ...
      {
        if      constexpr (Opts::contains(upper)) return T{0x1.33ba02p+0};
        else if constexpr (Opts::contains(lower)) return T{0x1.33bap+0};
        else                                      return T{0x1.33bap+0};
      }
      else
      {
        if      constexpr (Opts::contains(upper)) return T{0x1.33ba004f00622p+0};
        else if constexpr (Opts::contains(lower)) return T{0x1.33ba004f00621p+0};
