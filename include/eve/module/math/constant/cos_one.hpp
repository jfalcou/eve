...
namespace eve
{
  ...
  namespace detail
  {
    ...
    EVE_FORCEINLINE auto cos_one_(EVE_SUPPORTS(cpu_), eve::as<T> const & ) noexcept
    {
      ...
      if constexpr(std::is_same_v<t_t, float>)       return 0x1.14a28p-1;
      else if constexpr(std::is_same_v<t_t, double>) return 0x1.14a280fb5068cp-1;
    }
    ...
    requires(is_one_of<D>(types<upward_type, downward_type> {}))
    {
      ...
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.14a282p-1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.14a280fb5068cp-1;
      }
      else if constexpr(std::is_same_v<D, downward_type>)
      {
        if constexpr(std::is_same_v<t_t, float>)  return 0x1.14a28p-1;
        else if constexpr(std::is_same_v<t_t, double>) return 0x1.14a280fb5068bp-1;
      }
    }
  }
}
