...
{
  // up to 255*pi/4 ~200
  template<typename T> EVE_FORCEINLINE auto rempio2_small(T const &xx) noexcept
  {
  ...
  template<typename T> EVE_FORCEINLINE auto rempio2_medium(T const &xx) noexcept
  {
  ...
  EVE_FORCEINLINE auto rempio2_pass(double x1) noexcept
  {
    ...
    //    auto z = bit_and(zero_lo, bit_cast(x1, as<ui64_t>())); //TODO why this line  does not
    //    compile in the doc test for cos
  ...
  template<floating_real_scalar_value T> EVE_FORCEINLINE auto rempio2_big(T const &xx) noexcept
  {
    if( is_not_finite(xx) )
      return std::make_tuple(T(0), nan(eve::as<T>()), T(0));
    else if( xx < Rempio2_limit(restricted_type(), as(xx)) )
      return std::make_tuple(T(0), xx, T(0));
    if constexpr( std::is_same_v<T, float> )
    {
      if( xx <= 2000.0f /*Rempio2_limit(medium_type(), as(xx))/1.0e10*/ )
