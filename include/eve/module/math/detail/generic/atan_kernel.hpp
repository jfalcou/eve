...
namespace eve::detail
{
  ...
  {
    const auto flag1 = x < ieee_cts<0x1.3504f40p+1f, 0x1.3504f333f9de6p+1>(eve::as<T>{}); // tan(3pi/8)
    const auto flag2 =
      x >= ieee_cts<0x1.a8279a0p-2f, 0x1.a827999fcef31p-2>(eve::as<T>{}) && flag1; // tan(pi/8)
    ...
    z = eve::fma(xx, z, xx);
    z = add[flag2](z, ieee_cts<-0x1.777a5c0p-26f, 0x1.1a62633145c07p-55>(eve::as<T>{}));  // pio_4lo
    z = add[!flag1](z, ieee_cts<-0x1.777a5c0p-25f, 0x1.1a62633145c07p-54>(eve::as<T>{})); // pio_2lo
    return yy + z;
  }
}
