  ...
  namespace detail
  {
    ...
    cos_eval(T const& z) noexcept
    {
      ...
      if constexpr( std::is_same_v<elt_t, float> )
      {
        T y =
          ...
          ;
        return inc(fma(z, mhalf(as<T>{}), y * sqr(z)));
      }
    ...
    tan_finalize(T a0, T fn, T xr, T dxr = T(0)) noexcept
    {
      ...
      if constexpr( scalar_value<T> )
      {
        ...
        if( is_not_finite(xr) ) return nan(as<T>{});
        y = (int(fn) & 1) ? -rec[pedantic](y) : y;
        if( dxr ) y += dxr * fma(y, y, one(as<T>{}));
      ...
      else
      {
        ...
        y             = if_else(test, y, -rec[pedantic](y));
        y             = fma(dxr, fma(y, y, one(as<T>{})), y);
    ...
    cot_finalize(T a0, T fn, T xr, T dxr = T(0)) noexcept
    {
      ...
      else
      {
        ...
        auto swap_bit = (fma(T(-2), tmp, fn));
        auto test     = is_eqz(swap_bit);
        y             = if_else(test, rec[pedantic](y), -y);
        y             = fma(dxr, fma(y, y, one(as<T>{})), y);
        return if_else(aa0lteps, rec[pedantic](a0), bit_xor(y, bitofsign(a0)));
      }
    }
  }
}
