...
{
  template<floating_real_value T> EVE_FORCEINLINE constexpr auto cos_eval(T const &z) noexcept
  {
  ...
  template<floating_real_value T> EVE_FORCEINLINE constexpr auto tancot_eval(const T &z) noexcept
  {
  ...
  template<floating_real_value T>
  EVE_FORCEINLINE constexpr auto sin_finalize(T a0, T fn, T xr, T dxr = T(0)) noexcept
  {
    if constexpr( scalar_value<T> )
    {
      using i_t = detail::as_integer_t<T, signed>;
