...
/* float use   x < 0x1.7d4998p+38 (4.09404e+11) */
  ...
  EVE_FORCEINLINE constexpr auto rempio2_big(T xx) noexcept
  {
    ...
    if constexpr( std::is_same_v<elt_t, double> )
    {
      ...
      using ui64_tv = as_wide_t<uint64_t, width_v<T>>;
      using i32_tv  = as_wide_t<int32_t, 2 * width_v<T>>;
      ...
      auto pass = [&](auto vx1, T& vb1, T& vbb1)
        {
          ...
          for( int i = 0; i < 6; ++i )
          {
            auto values = gather(eve::as_aligned(toverp, width_t<T> {}), inds);
            inds        = inc(inds);
            r[i]        = vx1 * values * tmp1;
            tmp1 *= tm24;
          }
    ...
    else if constexpr( std::is_same_v<elt_t, float> )
    {
      ...
      using ui_tv  = as_wide_t<uint32_t, width_v<T>>;
      using wui_tv = as_wide_t<uint64_t, width_v<T>>;
      using i_tv   = as_wide_t<int64_t, width_v<T>>;
      ...
      auto index                  = ((xi >> 26) & 15);
      auto arr0                   = gather(eve::as_aligned(&__inv_pio4[0], width_t<T>{}), index);
      auto arr4 = convert(gather(eve::as_aligned(&__inv_pio4[0], width_t<T>{}), index + 4), as<uint64_t>());
      auto arr8 = convert(gather(eve::as_aligned(&__inv_pio4[0], width_t<T>{}), index + 8), as<uint64_t>());
