...
{
  template<typename T> EVE_FORCEINLINE constexpr auto tancot_eval(const T &z) noexcept
  {
    ...
    else if constexpr( std::is_same_v<value_type_t<T>, double> )
    {
      T       zz = sqr(z);
      const T num =
          horn<T, 0xc1711fead3299176ull, 0x413199eca5fc9dddull, 0xc0c992d8d24f3f38ull>(zz);
      T den = horn1<T,
