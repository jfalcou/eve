...
namespace eve::detail
{
  ...
  template<floating_real_value T> EVE_FORCEINLINE T kernel1_finalize2(T &res, const T &y) noexcept
  {
      ...
      requires(std::same_as<element_type_t<T>, double>)
  {
    ...
    return if_else(x < -26.628 /*Constant<T, 0XC03AA0C49BA5E354ULL>()*/,
                   inf(as<T>()),
                   (y + y) - res); //-26.628
  }
  ...
  // x >=  6 rx = 1/x
  template<floating_real_value T>
  EVE_FORCEINLINE T erfc4(const T &rx) noexcept requires(std::same_as<element_type_t<T>, double>)
  {
    return horn<T,
