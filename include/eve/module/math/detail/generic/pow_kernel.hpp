...
{
  template<floating_real_value T> EVE_FORCEINLINE T kernel_pow1(const T &x, const T &z) noexcept
  {
  ...
  template<floating_real_value T> EVE_FORCEINLINE T kernel_pow2(const T &x) noexcept
  {
    if constexpr( std::is_same_v<element_type_t<T>, float> )
    {
      return horn<T,
  ...
  template<integral_value I> EVE_FORCEINLINE as_floating_point_t<I> twomio16(const I &i) noexcept
  {
    /* 2^(-i/16)
  ...
  template<integral_value I> EVE_FORCEINLINE auto continuation(const I &i) noexcept
  {
  ...
  template<floating_real_value T> EVE_FORCEINLINE auto kernel_select(const T &xx) noexcept
  {
