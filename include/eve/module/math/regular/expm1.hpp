  ...
  namespace detail
  {
    ...
    T  expm1_(EVE_REQUIRES(cpu_), O const& o, T const& xx)
    {
      ...
      if constexpr( scalar_value<T> )
      {
        ...
        if( xgemaxlog ) return inf(as<T>{});
        if( xlelogeps ) return mone(as<T>{});
      }
      if constexpr( std::is_same_v<elt_t, float> )
      {
        ...
        T   two2mk = bit_cast((maxexponent(as<T>{}) - ik) << nbmantissabits(as<elt_t>{}), as<T>{});
      ...
      else if constexpr( std::is_same_v<elt_t, double> )
      {
        ...
        T t    = T(3) - r1 * half(as<T>{}) * x;
        T e    = hxs * ((r1 - t) / (T(6) - x * t));
        T c    = (hi - x) - lo;
        e      = (x * (e - c) - c) - hxs;
        i_t ik = convert(k, int_from<T>());
        T   two2mk = bit_cast((maxexponent(as<T>{}) - ik) << nbmantissabits(as<T>{}), as<T>{});
      ...
      {
        k = if_else(xgemaxlog, inf(as<T>{}), k);
