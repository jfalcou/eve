...
#include <eve/module/math/detail/generic/cot_kernel.hpp>

namespace eve
{
  template<typename Options>
  struct cot_t : elementwise_callable<cot_t, Options, quarter_circle_option,
...
//!
//!      // Semantic options
//!      constexpr auto acos[rad](floating_value auto x)                     noexcept; // 1.a
//!      constexpr auto acos[deg](floating_value auto x)                     noexcept; // 1.b
//!      constexpr auto acos[pirad](floating_value auto x)                   noexcept; // 1.c
...
//!      3. assume a parameter in \f$\pi\f$ multiples.
  ...
  namespace detail
  {
    ...
    constexpr EVE_FORCEINLINE T cot_(EVE_REQUIRES(cpu_), O const& o , T const& a0)
    {
      if constexpr(std::same_as<eve::element_type_t<T>, eve::float16_t>)
        return eve::detail::apply_fp16_as_fp32(eve::cot_kernel[o], a0);
      else
