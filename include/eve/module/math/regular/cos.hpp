  ...
  namespace detail
  {
    ...
    constexpr EVE_FORCEINLINE T cos_(EVE_REQUIRES(cpu_), O const& o , T const& a0)
    {
      if constexpr(O::contains(quarter_circle))
      {
        ...
        if constexpr( scalar_value<T> ) return (x2nlepi2_16) ? nan(as<T>{}) : cos_eval(x2);
        else return if_else(x2nlepi2_16, eve::allbits, cos_eval(x2));
      }
      else if constexpr(O::contains(half_circle))
      {
        T    x         = eve::abs(a0);
        auto xnlepio2 = is_not_less_equal(x, pio_2(as<T>{}));
        if constexpr( scalar_value<T> )
          if( xnlepio2 ) return nan(as<T>{});
        ...
        {
          auto n        = one[is_not_less_equal(x, pio_4(as{x}))](as{x});
          auto sign_bit   = signmask[is_nez(n)](as{x});
          auto xr       = reduce(x);
          xr            = if_else(n, xr, x);
          auto z        = sqr(xr);
