...
cotpi_(EVE_SUPPORTS(cpu_), quarter_circle_type const&, T a0) noexcept
{
  ...
  else return apply_over(quarter_circle(cotpi), a0);
}

template<decorator D, floating_ordered_value T>
EVE_FORCEINLINE constexpr auto
cotpi_(EVE_SUPPORTS(cpu_), D const&, T a0) noexcept
...
template<floating_ordered_value T>
EVE_FORCEINLINE constexpr auto
cotpi_(EVE_SUPPORTS(cpu_), T const& a0) noexcept
{
  if constexpr( has_native_abi_v<T> )
  {
    auto x = abs(a0);
    if( eve::all(eve::abs(x) <= T(0.25)) ) return quarter_circle(cotpi)(a0);
    else return big(cotpi)(a0);
  }
...
cotpi_(EVE_SUPPORTS(cpu_), C const& cond, U const& t) noexcept
{
  return mask_op(cond, eve::cotpi, t);
}

template<conditional_expr C, decorator D, value U>
EVE_FORCEINLINE auto
cotpi_(EVE_SUPPORTS(cpu_), C const& cond, D const & d, U const& t) noexcept
{
  return mask_op(cond, d(eve::cotpi), t);
}
}
