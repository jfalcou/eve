...
acos_(EVE_SUPPORTS(cpu_), raw_type const&, T const& a0) noexcept
{
  if constexpr( has_native_abi_v<T> )
  {
    auto tmp =
      pio_2(eve::as(a0)) + (ieee_constant<-0x1.777a5c0p-25f, 0x1.1a62633145c07p-54>(eve::as<T>{}) - asin(a0));
...
template<floating_ordered_value T>
EVE_FORCEINLINE auto
acos_(EVE_SUPPORTS(cpu_), T const& a0) noexcept
{
...
EVE_FORCEINLINE auto
acos_(EVE_SUPPORTS(cpu_), C const& cond, U const& t) noexcept
{
  return mask_op(cond, eve::acos, t);
}

template<conditional_expr C, decorator D, value U>
EVE_FORCEINLINE auto
acos_(EVE_SUPPORTS(cpu_), C const& cond, D const & d, U const& t) noexcept
{
  return mask_op(cond, d(eve::acos), t);
}
}
