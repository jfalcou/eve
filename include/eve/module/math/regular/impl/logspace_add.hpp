...
logspace_add_(EVE_SUPPORTS(cpu_), T const& a, U const& b) noexcept requires compatible_values<T, U>
{
...
logspace_add_(EVE_SUPPORTS(cpu_), T0 a0, T1 a1, Ts... args)
{
...
logspace_add_(EVE_SUPPORTS(cpu_), Ts tup)
{
  ...
  else return eve::abs(kumi::apply( [&](auto... m) { return logspace_add(m...); }, tup));
}
