...
EVE_FORCEINLINE auto
pow_abs_(EVE_SUPPORTS(cpu_), C const& cond, U const& t) noexcept
{
  return mask_op(cond, eve::pow_abs, t);
}

template<conditional_expr C, decorator D, value U>
EVE_FORCEINLINE auto
pow_abs_(EVE_SUPPORTS(cpu_), C const& cond, D const & d, U const& t) noexcept
{
  return mask_op(cond, d(eve::pow_abs), t);
}
}
