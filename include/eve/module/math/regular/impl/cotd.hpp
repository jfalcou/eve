...
cotd_(EVE_SUPPORTS(cpu_), C const& cond, U const& t) noexcept
{
  return mask_op(cond, eve::cotd, t);
}

template<conditional_expr C, decorator D, value U>
EVE_FORCEINLINE auto
cotd_(EVE_SUPPORTS(cpu_), C const& cond, D const & d, U const& t) noexcept
{
  return mask_op(cond, d(eve::cotd), t);
}
}
