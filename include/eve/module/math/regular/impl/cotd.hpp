...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE constexpr auto cotd_(EVE_SUPPORTS(cpu_), quarter_circle_type const &, T a0) noexcept
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      return apply_over(quarter_circle(cotd), a0);
  }
  ...
  EVE_FORCEINLINE constexpr auto cotd_(EVE_SUPPORTS(cpu_), T const &a0) noexcept
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      if( eve::all(eve::abs(x) <= T(45)) )
        return quarter_circle(cotd)(a0);
      else
        return big(cotd)(a0);
    }
    else
      return apply_over(cotd, a0);
  }
}
