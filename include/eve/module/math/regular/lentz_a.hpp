...
namespace eve
{
  ...
  struct lentz_a_t : strict_elementwise_callable<lentz_a_t, Options>
  {
    ...
    constexpr EVE_FORCEINLINE auto operator()(G g, T eps, std::size_t m) const noexcept -> decltype(g())
    {
      return this->behavior(as<decltype(g())>{}, eve::current_api, this->options(), g, eps, m);
    }
