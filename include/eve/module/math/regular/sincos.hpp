  ...
  namespace detail
  {
    ...
    constexpr EVE_FORCEINLINE auto sincos_(EVE_REQUIRES(cpu_), O const& o , T const& a0)
    {
      ...
      if constexpr(O::contains(quarter_circle))
      {
        ...
        {
          return  (x2nlepi2_16) ? eve::zip(nan(as<T>{}), nan(as<T>{}))
      ...
      else if constexpr(O::contains(half_circle))
      {
        ...
        if constexpr( scalar_value<T> )
        {
          ...
          if( is_less_equal(x, eps(as<T>{})) ) return eve::zip(a0, one(as<T>{}));
          if( is_not_less_equal(x, pio_2(as<T>{})) ) return eve::zip(nan(as<T>{}), nan(as<T>{}));
        ...
        {
          x           = if_else(is_not_less_equal(x, pio_2(as<T>{})), eve::allbits, x);
          auto test   = is_not_less_equal(x, pio_4(as<T>{}));
      ...
      else if constexpr(O::contains(full_circle) || O::contains(medium) || O::contains(big) )
      {
        ...
        if constexpr( scalar_value<T> )
        {
          if( xnlelim ) return eve::zip(nan(as<T>{}), nan(as<T>{}));
        }
