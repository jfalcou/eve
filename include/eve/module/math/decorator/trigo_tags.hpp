...
//! @var half_circle
//!
//! @brief  Higher-order @callable imbuing  a limited range standard semantic onto other
//! @callable{s}.
//!
//! #### Members Functions
//!
...
namespace detail
{
  ...
  //! @return
  //! A @callable performing the same kind of operation,
  //! but  gives the correct result for \f$|x| < 1.76858e+15f\f$ (float) or  \f$ |x| < 2.0e14 \f$
  //! (double)
  //! (\f$x\f$ in radian) and degrades gently for greater values.
  //! (the bounds are to be converted if the input is  \f$\pi\f$ multiples, and in degrees the call
  //! is currently equivalent to big) medium use a relaxed reduction scheme
  //!
  //================================================================================================
  struct medium_
  {
