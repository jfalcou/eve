...
**/
//==================================================================================================
#ifndef EVE_MODULE_MATH_FUNCTION_GENERIC_SINPI_HPP_INCLUDED
#define EVE_MODULE_MATH_FUNCTION_GENERIC_SINPI_HPP_INCLUDED
  ...
  EVE_FORCEINLINE constexpr auto sinpi_(EVE_SUPPORTS(cpu_), T const &a0) noexcept
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      if( all(eve::abs(x) <= T(0.25)) )
        return restricted_(sinpi)(a0);
      else
        return big_(sinpi)(a0);
    }
    else
