      ...
      if (any(notdone))
      {
        ...
        if (any(notdone))
        {
          ...
          if (any(notdone))
          {
            ...
            if (any(notdone))
            {
              auto br_phigepio2 =  [](auto phi) //phi >= pi/2 || phi < 0
                {
                  return zero(as(phi)); //TODO
                };
              notdone = next_interval(br_phigepio2, notdone, (phi >= pio_2(as(phi))) || (phi < 0) , r, phi);
              if (any(notdone))
              {
...
//                              , U x) noexcept
//   {
//     return arithmetic_call(ellint_3_, phi, x);
//   }
...
//     }
//     else
//       return apply_over(ellint_3, phi0, x);
//   }
