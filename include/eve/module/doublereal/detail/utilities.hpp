...
//   template<> struct supports_conditional<tag::three_fma_> : std::false_type{};
  ...
  template <ordered_value T>
  EVE_FORCEINLINE auto  make_doublereal (T a,  T b) noexcept
  {
    auto [l, h] = two_add(a, b);
    return as_doublereal_t<T>(l, h);
  }

  template <ordered_value T>
  EVE_FORCEINLINE auto  make_doublereal (kumi::tuple<T, T> const & t) noexcept
  {
    return make_doublereal(get<0>(t), get<1>(t));
  }

  template <ordered_value T, ordered_value O>
  EVE_FORCEINLINE auto  make_doublereal (O const & o, as<T> const &) noexcept
  requires(is_doublereal_v<T>)
  {
