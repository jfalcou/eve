  ...
  struct needle_checker
  {
        ...
        , first_wide_ignore(iterator_cardinal_v<I2>)
    {
      ...
      else
      {
        ...
        auto remainder   = static_cast<std::size_t>(needle_len) % iterator_cardinal_v<I2>;
        long_tail_offset = (remainder != 0) ? remainder : static_cast<std::size_t>(iterator_cardinal_v<I2>);

        long_tail_start = eve::unalign(f) + long_tail_offset;
        long_tail_n     = static_cast<std::size_t>(needle_len - long_tail_offset) / iterator_cardinal_v<I2>;
      }
    }
