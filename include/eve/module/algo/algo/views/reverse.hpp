...
namespace eve::algo::views
{
  ...
  struct reverse_
  {
    ...
    auto operator()(Wrapped&& wrapped) const
    {
      if constexpr( relaxed_range<Wrapped> )
      {
        ...
        if constexpr ( eve::_::instance_of<Rng, reverse_range> ) return rng.base;
      ...
      else
      {
        ...
        if constexpr ( eve::_::instance_of<I, reverse_iterator> ) return wrapped.base;
  ...
  struct reverse_iterator : operations_with_distance
  {
    ...
    EVE_ABI explicit reverse_iterator(I b) : base(b) {}

    template <std::convertible_to<I> I1>
    EVE_ABI reverse_iterator(reverse_iterator<I1> x) : base(x.base) {}

    EVE_ABI auto unalign()  const noexcept { return reverse(eve::unalign(base)); }
    EVE_ABI auto read()     const noexcept { return eve::read( eve::unalign(base) - 1);   }

    EVE_ABI void write(value_type v) const noexcept { return eve::write(v,eve::unalign(base) - 1); }

    template <relaxed_sentinel_for<I> I1>
    EVE_ABI bool operator==(reverse_iterator<I1> y) const
    {
      return base == y.base;
    }

    template <relaxed_sentinel_for<I> I1>
    EVE_ABI auto operator<=>(reverse_iterator<I1> y) const
    {
    ...
    EVE_ABI friend std::ptrdiff_t operator-(reverse_iterator const & x, reverse_iterator const & y)
    {
    ...
    EVE_ABI auto load(O const& opts, as<wide_value_type_t<I>> tgt) const
      requires iterator<I>
    {
    ...
    template<callable_options O>
    EVE_ABI void store(O const& opts, wide_value_type_t<I> v) const noexcept
      requires iterator<I>
    {
  ...
  struct reverse_range
  {
    ...
    EVE_ABI auto begin() const { return reverse(base.end()); }
    EVE_ABI auto end()   const { return reverse(base.begin()); }

    template<typename Traits>
    EVE_ABI friend auto tagged_dispatch(preprocess_range_, Traits tr, reverse_range self)
    {
