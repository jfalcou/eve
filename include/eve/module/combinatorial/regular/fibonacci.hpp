...
namespace eve
{
  ...
  {
    template<unsigned_value N, floating_value T0, floating_value T1>
  ...
  namespace detail
  {
    template<callable_options O, typename N, typename T, typename U>
    constexpr EVE_FORCEINLINE as_wide_as_t<common_value_t<T, U>, N> fibonacci_(EVE_REQUIRES(cpu_), O const&, N n, T a, U b)
    {
