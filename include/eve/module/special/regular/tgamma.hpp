...
namespace eve
{
  ...
  struct tgamma_t : elementwise_callable<tgamma_t, Options, raw_option, fast_option, pedantic_option>
  {
...
//================================================================================================
  ...
  namespace _
  {
    ...
    T tgamma_(EVE_REQUIRES(cpu_), O const& o, T a0) noexcept
    {
      using elt_t  = element_type_t<T>;
      if constexpr(O::contains(raw) || O::contains(fast))
      {
        auto nega0 = is_lez(a0);
        auto x = if_else(a0 < T(32.4), eve::allbits, if_else(a0 > T(32.4), T(32.4), a0));
        x = if_else(nega0,oneminus(x), x); //x is >= 0
        auto xx =  dec(x);
        auto lex1 = xx < 1;
        xx = eve::add[lex1](xx, elt_t(3));
        auto xx2 = sqr(xx);
