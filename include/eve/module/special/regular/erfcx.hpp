...
//!   *  [C. Ren & al.](https://rmets.onlinelibrary.wiley.com/doi/epdf/10.1002/asl.154)
//!
//!  @groupheader{Example}
...
//================================================================================================
  ...
  namespace _
  {
    ...
    constexpr T
    erfcx_(EVE_REQUIRES(cpu_), O const& o , T x) noexcept
    {
      using elt_t = element_type_t<T>;
      if constexpr(O::contains(fast)|| O::contains(raw))
      {
       /*
        * Based on: C. Ren* and A. R. MacKenzie,  Closed-form approximations to the error and
        complementary error functions and their applications in
        atmospheric science.
        *
        */
        auto xpos = is_gez(x);
        constexpr elt_t a = elt_t(2.9110); //pi(as<elt_t>())/(pi(as<elt_t>())-2);
        if constexpr(scalar_value<T>)
        {
          if (xpos)
          {
            auto pix2 = pi(as<elt_t>())*sqr(x);
            return  a/(dec(a)*sqrt(pix2)+ sqrt(pix2+sqr(a)));
          }
          else
          {
            return expx2[o](x)*erfc[o](x);
          }
        }
        else
        {
          auto pix2 = pi(as<elt_t>())*sqr(x);
          auto r = a/(dec(a)*sqrt(pix2)+ sqrt(pix2+sqr(a)));
          if (all(xpos))
            return r;
          else
            return if_else(xpos,  r,  expx2[o](x)*erfc[o](x));
        }
      }
      else
      {
