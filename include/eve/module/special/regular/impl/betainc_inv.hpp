...
namespace eve::detail
{
  ...
  betainc_inv_(EVE_REQUIRES(cpu_), O const&, T p, U a, V b) noexcept

  {
    if constexpr(std::same_as<T, U> && std::same_as<T, V> && has_native_abi_v<T>)
    {
    ...
    {
      return arithmetic_call(betainc_inv, p, a, b);
    }
  }
}
