  ...
  requires(is_one_of<D>(types<regular_type, pedantic_type> {}))
  {
    ...
    if constexpr( has_native_abi_v<T> )
    {
      ...
      const T Stirlingsplitlim =
        ieee_cts<0x1.ac51ec0p+4f, 0x1.1e083ba3443d4p+7>(eve::as<T>{});
      const T Stirlinglargelim =
        ieee_cts<0x1.1851e60p+5f, 0x1.5800000000000p+7>(eve::as<T>{});
      const T Sqrt_2pi = ieee_cts<0x1.40d9320p+1f, 0x1.40d931ff62704p+1>(eve::as<T>{});
