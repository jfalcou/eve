...
//================================================================================================
  ...
  namespace _
  {
    ...
    constexpr T  digamma_(EVE_REQUIRES(cpu_), O const& o, T a) noexcept
    {
      using elt_t  = element_type_t<T>;
      if constexpr(O::contains(fast))
      {
        auto aneg = is_ltz(a);
        auto aa = if_else(is_ltz(a), oneminus(a), a);

        auto twopaa = 2 + aa;
        auto lg = log[o](twopaa);
        auto r = fam(-48, aa, fam(-157, aa, fam(-127, -30, aa)))/(12*aa*inc(aa)*sqr(twopaa)) + lg;
      ...
      else if constexpr( O::contains(raw))
      {
        ...
        auto aa = if_else(is_ltz(a), oneminus(a), a);
        auto onepaa = inc(aa); ;
        auto r = -rec[pedantic](aa) - rec[pedantic](2*onepaa) + log[o](onepaa);
        if (none(aneg)) return r;
        else return sub[aneg](r,  pi(as<elt_t>())*cot[radpi](a));
      }
      else
