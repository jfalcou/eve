//==================================================================================================
/**
  EVE - Expressive Vector Engine
  Copyright 2020 Joel FALCOU
  Copyright 2020 Jean-Thierry LAPRESTE

  Licensed under the MIT License <http://opensource.org/licenses/MIT>.
  SPDX-License-Identifier: MIT
**/
//==================================================================================================
#ifndef EVE_MODULE_SPECIAL_FUNCTION_GENERIC_GAMMA_HPP_INCLUDED
#define EVE_MODULE_SPECIAL_FUNCTION_GENERIC_GAMMA_HPP_INCLUDED

#include <eve/concept/value.hpp>
#include <eve/constant/inf.hpp>
#include <eve/constant/minf.hpp>
#include <eve/constant/mone.hpp>
#include <eve/constant/nan.hpp>
#include <eve/constant/one.hpp>
#include <eve/constant/pi.hpp>
#include <eve/detail/apply_over.hpp>
#include <eve/detail/implementation.hpp>
#include <eve/function/abs.hpp>
#include <eve/function/copysign.hpp>
#include <eve/function/dec.hpp>
#include <eve/function/inc.hpp>
#include <eve/function/is_eqz.hpp>
#include <eve/function/is_even.hpp>
#include <eve/function/is_ltz.hpp>
#include <eve/function/logical_and.hpp>
#include <eve/function/nbtrue.hpp>
#include <eve/function/sinpi.hpp>
#include <eve/function/stirling.hpp>
#include <eve/module/core/detail/generic/horn.hpp>
#include <eve/platform.hpp>

#include <type_traits>

namespace eve::detail
{
  template<floating_real_value T, decorator D>
  EVE_FORCEINLINE T gamma_(EVE_SUPPORTS(cpu_), D const &, T a0) noexcept
      requires(is_one_of<D>(types<regular_type, pedantic_type> {}))
  {
    if constexpr( has_native_abi_v<T> )
    {
      using elt_t = element_type_t<T>;
      auto gamma1 = [](T x) {
        if constexpr( std::is_same_v<elt_t, float> )
        {
          return horn<T,
                      0X3F800000U, //  9.999999757445841E-01
                      0X3ED87799U, //  4.227874605370421E-01
                      0X3ED2D411U, //  4.117741948434743E-01
                      0X3DA82A34U, //  8.211174403261340E-02
                      0X3D93AE7CU, //  7.211014349068177E-02
                      0X3B91DB14U, //  4.451165155708328E-03
                      0X3BA90C99U, //  5.158972571345137E-03
                      0X3AD28B22U  //  1.606319369134976E-03
                      >(x);
        }
        else if constexpr( std::is_same_v<elt_t, double> )
        {
          return horn<T,
                      0X3FF0000000000000ULL, // 9.99999999999999996796E-1
                      0X3FDFA1373993E312ULL, // 4.94214826801497100753E-1
                      0X3FCA8DA9DCAE7D31ULL, // 2.07448227648435975150E-1
                      0X3FA863D918C423D3ULL, // 4.76367800457137231464E-2
                      0X3F8557CDE9DB14B0ULL, // 1.04213797561761569935E-2
                      0X3F5384E3E686BFABULL, // 1.19135147006586384913E-3
                      0X3F24FCB839982153ULL  // 1.60119522476751861407E-4
                      >(x)
                 / horn<T,
                        0X3FF0000000000000ULL, //  1.00000000000000000320E00
                        0X3FB24944C9CD3C51ULL, //  7.14304917030273074085E-2
                        0XBFCE071A9D4287C2ULL, // -2.34591795718243348568E-1
                        0X3FA25779E33FDE67ULL, //  3.58236398605498653373E-2
                        0X3F8831ED5B1BB117ULL, //  1.18139785222060435552E-2
                        0XBF7240E4E750B44AULL, // -4.45641913851797240494E-3
                        0X3F41AE8A29152573ULL, //  5.39605580493303397842E-4
                        0XBEF8487A8400D3AFULL  // -2.31581873324120129819E-5
                        >(x);
        }
      };
      if constexpr( scalar_value<T> )
      {
        auto inftest = [](T a0) {
          if constexpr( std::is_same_v<elt_t, float> )
          {
            return a0 > 35.4f;
          }
          else if constexpr( std::is_same_v<elt_t, double> )
          {
            return a0 > 171.624;
          };
        };
        if( is_eqz(a0) )
          return copysign(Inf(as(a0)), a0);
        if constexpr( eve::platform::supports_nans )
        {
          if( is_nan(a0) || (a0 == Minf(as(a0))) )
            return Nan(as(a0));
          if( a0 == Inf(as(a0)) )
            return a0;
        }
        auto x = a0;
        if( inftest(a0) )
          return Inf(as(a0));
        auto q = abs(x);
        if( x < T(-33) )
        {
          T    st     = stirling(q);
          T    p      = floor(q);
          auto iseven = is_even((int32_t)p);
          if( p == q )
            return Nan(as(a0));
          T z = q - p;
          if( z > Half(as(a0)) )
          {
            p += One(as(a0));
            z = q - p;
          }
          z = q * sinpi(z);
          if( is_eqz(z) )
            return Nan(as(a0));
          st = Pi(as(a0)) / (abs(z) * st);
          return iseven ? -st : st;
        }
        T z = One(as(a0));
        while( x >= T(3) )
        {
          x -= One(as(a0));
          z *= x;
        }
        while( is_ltz(x) )
        {
          z /= x;
          x += One(as(a0));
        }
        while( x < T(2) )
        {
          if( is_eqz(x) )
            return Nan(as(a0));
          z /= x;
          x += One(as(a0));
        }
        if( x == T(2) )
          return (z);
        x -= T(2);
        return z * gamma1(x);
      }
      else if constexpr( simd_value<T> )
      {
        auto large_negative = [](T q) {
          auto st     = stirling(q);
          auto p      = floor(q);
          auto sgngam = if_else(is_even(p), One(as(q)), eve::mone_);
          auto z      = q - p;
          auto test2  = is_less(z, Half(as(q)));
          z           = dec[test2](z);
          z           = q * sinpi(z);
          z           = abs(z);
          return sgngam * Pi(as(q)) / (z * st);
        };
        auto other = [gamma1](T q, const auto &test) {
          auto x = if_else(test, T(2), q);
          if constexpr( eve::platform::supports_nans )
          {
            auto inf_result = is_equal(q, Inf<T>());
            x               = if_else(inf_result, T(2), x);
          }
          auto z     = One<T>();
          auto test1 = is_greater_equal(x, T(3));
          while( any(test1) )
          {
            x     = dec[test1](x);
            z     = if_else(test1, z * x, z);
            test1 = is_greater_equal(x, T(3));
          }
          // all x are less than 3
          test1 = is_ltz(x);
          while( any(test1) )
          {
            z     = if_else(test1, z / x, z);
            x     = inc[test1](x);
            test1 = is_ltz(x);
          }
          // all x are greater than 0 and less than 3
          auto test2 = is_less(x, T(2));
          while( any(test2) )
          {
            z     = if_else(test2, z / x, z);
            x     = inc[test2](x);
            test2 = is_less(x, T(2));
          }
          // all x are greater equal 2 and less than 3
          x = z * gamma1(x - T(2));
          if constexpr( eve::platform::supports_infinites )
          {
            auto inf_result = is_equal(q, Inf<T>());
            return if_else(inf_result, q, x);
          }
          else
            return x;
        };

        auto nan_result = is_ltz(a0) && is_flint(a0);
        //       if constexpr(eve::platform::supports_nans) nan_result = is_nan(a0) || nan_result;
        auto        q    = abs(a0);
        auto        test = is_less(a0, T(-33.0));
        std::size_t nb   = nbtrue(test);
        auto        r    = Nan(as(a0));
        if( nb > 0 )
        {
          // treat negative large with reflection
          r = large_negative(q);
          if( nb >= cardinal_v<T> )
            return if_else(nan_result, eve::allbits_, r);
        }
        auto r1 = other(a0, test);
        auto r2 = if_else(test, r, r1);
        return if_else(
            is_eqz(a0), copysign(Inf(as(a0)), a0), if_else(nan_result, eve::allbits_, r2));
      }
    }
    else
      return apply_over(gamma, a0);
  }

  template<floating_real_value T>
  EVE_FORCEINLINE constexpr T gamma_(EVE_SUPPORTS(cpu_), T const &x) noexcept
  {
    return gamma(regular_type(), x);
  }
}

#endif
