...
{
  template<floating_real_scalar_value T> EVE_FORCEINLINE T erfc_(EVE_SUPPORTS(cpu_), T x) noexcept
  {
    if constexpr( eve::platform::supports_invalids )
      if( is_nan(x) )
        return x;
    if constexpr( eve::platform::supports_infinites )
      if( eve::is_infinite(x) )
        return oneminus(signnz(x));
    if( is_eqz(x) )
      return T(1);
  ...
  template<floating_real_simd_value T> EVE_FORCEINLINE T erfc_(EVE_SUPPORTS(cpu_), T a0) noexcept
  {
