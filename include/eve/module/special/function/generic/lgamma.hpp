...
namespace eve::detail
{
  ...
  template<floating_real_value T> EVE_FORCEINLINE T lgammaC(T x) noexcept
  {
  ...
  template<floating_real_value T> EVE_FORCEINLINE T lgamma2(T p) noexcept
  {
    return horn<T,
  ...
  template<floating_real_value T> EVE_FORCEINLINE T lgamma1(T x) noexcept
  {
    return horn<T,
  ...
  template<floating_real_value T> EVE_FORCEINLINE T lgammaA(const T &p) noexcept
  {
    return horn<T,
  ...
  EVE_FORCEINLINE constexpr T lgamma_(EVE_SUPPORTS(cpu_), D const &, T a0) noexcept
      requires(is_one_of<D>(types<regular_type, pedantic_type> {}))
  {
    if constexpr( has_native_abi_v<T> )
    {
      ...
      return apply_over(lgamma, a0);
  }

  template<floating_real_value T>
  EVE_FORCEINLINE constexpr T lgamma_(EVE_SUPPORTS(cpu_), T const &x) noexcept
  {
    return lgamma(regular_type(), x);
  }
}
