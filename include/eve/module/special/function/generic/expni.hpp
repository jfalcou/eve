...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE T expni_(EVE_SUPPORTS(cpu_), I in, T x) noexcept
  {
  ...
  requires compatible_values<T, I>
  {
    ...
    return expni(convert(in, as<elt_t>()), x);
  }
  ...
  requires compatible_values<T, I>
  {
    ...
    return expni(convert(in, as<i_t>()), w_t(x));
  }

  template<floating_real_value T>
  EVE_FORCEINLINE T expni_(EVE_SUPPORTS(cpu_), T n, T x) noexcept
  {
   using elt_t =  element_type_t<T>;
     ...
     if (any(notdone))
     {
       ...
       if (any(notdone))
       {
         auto br_xlt1 = [](auto n,  auto x){ // here x is always le 1
           ...
           auto in = int_(n);
           return add[eqzx]((eve::pow(z, dec(in)) * psi / gamma(n)) - ans, inf(as(x)));
         ...
         if(any(notdone))
         {
           auto br_xge1 = [](auto n,  auto x){ // here x is always gt 1
             auto expnibig = (sizeof(elt_t) == 8) ? T(1ull << 57) : T(1ull << 24);
             auto r =  expnibig;
             int32_t sk = 1;
             T t;
             auto pkm2 = one(as(x));
