...
namespace kumi::_
{
  ...
  template <typename T> concept stream = 
  requires(T& os, typename T::char_type c, const typename T::char_type* buff, const char* str)
  {
  ...
  template<typename T, typename Os> requires ( stream<Os> )
...
namespace kumi::_
{
  ...
  consteval auto make_value_as()
  {
    if constexpr (std::same_as<decltype(name_of(as<T>{})), kumi::str>)
  ...
  template <std::size_t I, kumi::str Name, typename T> 
  struct unique_name<I, kumi::field_capture<Name, T>> 
  {
...
namespace kumi::_
{
  ...
  template<typename T> concept container_like = requires(T const& t)
  {
...
{  
  ...
  template<typename T> struct container_size<T const &&>  : container_size<T> {};
  template<typename T>
  inline constexpr auto container_size_v = container_size<T>::value;
...
namespace kumi
{
  ...
  template<typename T>
  requires ( _::non_empty_tuple<T> || _::empty_tuple<T> )
  struct is_product_type<T> : std::true_type{};
}
#include <cstddef>
#include <utility>
namespace kumi::_
{
  ...
  template<typename Ref, typename... Fields> using get_field_by_name_t = 
      typename get_field_by_name<Ref, std::index_sequence_for<Fields...>,Fields...>::type;
...
namespace kumi::_
{
  ...
  KUMI_ABI constexpr auto &get_leaf(binder<ISeq, Ts...> &arg) noexcept
  {
    constexpr std::size_t idx = get_index_by_name_v<field_capture<Name, unit>, Ts...>;
...
namespace kumi
{
  ...
  struct record<Ts...>
  {
    ...
    template<_::stream Os> friend auto &operator<<(Os &os, record const& t) noexcept
    {
      os << "( ";
      [&]<std::size_t...I>(std::index_sequence<I...>)
      {
  ...
  template<> struct record<>  
  {
    ...
    template<_::stream Os> friend auto &operator<<(Os &os, record) noexcept
    {
      return os << "()";
    }
  };
...
{ 
  ...
  template<static_container S> requires ( !product_type<S> )
...
namespace kumi
{
  ...
  namespace result
  {
    template<monoid M, product_type T, typename Value = void>
    struct reduce
    {
      using type = decltype(kumi::reduce( std::declval<M>()
                                        , std::declval<T>()
                                        , std::declval<Value>()
                                        ));
    };
