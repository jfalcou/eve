...
namespace eve
{
  ...
  //! ordering. However, one can specialize `eve::supports_ordering_v` for a given type to evaluates
  ...
  //! treated as if `eve::supports_ordering_v<T>` evaluates to `false`, thus disabling
  //! ordering operators for `eve::wide<T>`.
  //================================================================================================
  template<typename T>
  inline constexpr bool supports_ordering_v = true; 

  template<typename T>
  requires { typename T::eve_disable_ordering; }
  inline constexpr bool supports_ordering_v<T> = false;
  ...
  //! **Required header:** `#include <eve/traits/product_type.hpp>`
  //!
  //! `eve::supports_like_v` is meant to be specialized to indicates that `Wrapper` models
  //! `eve::like<Wrapper,T>`.
  //!
  ...
  inline constexpr bool supports_like_v = false;

  template <typename Wrapper, typename Self>
  requires requires(Wrapper) { typename Wrapper::is_like; }
  inline constexpr bool supports_like_v<Wrapper, Self> =
  std::same_as<Self, typename std::remove_cvref_t<Wrapper>::is_like>;
  ...
  concept like = std::same_as<std::remove_cvref_t<Wrapper>, Self> ||
                 supports_like_v<std::remove_cvref_t<Wrapper>, Self>;
  ...
  //!   - Lexicographial order is provided unless `eve::supports_ordering_v<Self>` evaluates to `false`
