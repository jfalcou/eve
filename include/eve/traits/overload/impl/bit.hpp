...
namespace eve
{
  ...
  struct bit_callable : strict_tuple_callable<Func, OptionsValues, Options...>
  {
    ...
    constexpr static EVE_ABI auto process_input(T v, as<Tgt>) noexcept
    {
    ...
    template<typename T, typename Tgt>
    constexpr static EVE_ABI auto process_alternative(T v, as<Tgt>) noexcept
    {
    ...
    constexpr EVE_ABI bit_value_t<Ts...> execute(auto arch, O const& opts, Ts... xs) const
    {
    ...
    constexpr EVE_ABI kumi::apply_traits_t<bit_value, Tup> behavior(auto arch, O const& opts, Tup x) const
    {
      return kumi::apply([&](auto... xs) { return behavior(arch, opts, xs...); }, x);
    }

    template<callable_options O, typename... Ts>
    constexpr EVE_ABI bit_value_t<Ts...> behavior(auto arch, O const& opts, Ts... xs) const
    {
