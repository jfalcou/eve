...
namespace eve
{
  template<typename Type, typename Size = expected_cardinal_t<Type>> struct as_wide;

  template<typename Type, typename Size>
  requires(std::is_arithmetic_v<Type>) struct as_wide<Type, Size>
  {
    using type = eve::wide<Type, Size>;
  };

  template<typename T, typename Size> struct as_wide<eve::logical<T>, Size>
  {
    using type = eve::logical<typename as_wide<T, Size>::type>;
  };

  template<typename Type, typename N, typename Size> struct as_wide<eve::wide<Type, N>, Size>
  {
    using type = eve::wide<Type, Size>;
  };
  ...
  requires(rebindable<T> && !detail::is_wide<T>::value) struct as_wide<T, Size>
  {
  ...
  requires(!detail::is_wide<std::pair<T, U>>::value) struct as_wide<std::pair<T, U>, Size>
  {
  ...
  requires(!detail::is_wide<std::array<T, N>>::value) struct as_wide<std::array<T, N>, Size>
  {
