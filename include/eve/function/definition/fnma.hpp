...
namespace eve
{
 namespace tag { struct fnma_; }

  namespace detail
  {
    template<typename T, typename U, typename V>
    EVE_FORCEINLINE void check(EVE_MATCH_CALL(eve::tag::fnma_), T const&, U const &, V const &)
    {
      ...
      if constexpr(is_vec_t && is_vec_u && is_vec_v)
      {
        static_assert(std::is_same_v<T, U> && std::is_same_v<T, V>, 
                    "[eve::fnma]  - SIMD arguments have not the same type");

      }
      if constexpr(!is_vec_t && !is_vec_u && !is_vec_v)
      {
        static_assert(std::is_same_v<T, U> && std::is_same_v<T, V>, 
                    "[eve::fnma]  - scalar arguments have not the same type");
