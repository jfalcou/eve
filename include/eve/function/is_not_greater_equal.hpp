...
namespace eve
{
  ...
  //!  **Return value**
  //!
  //!  A Callable object so that the expression `is_not_greater_equal[cond](x, ...)` is equivalent to `if_else(cond,is_not_greater_equal(x, ...),x)`
  //!
  //! ---
  //!
  ...
  //!  
  //!      - if `t` is a floating_value then  $(x \le y - t \max(|x|, |y|))$
  //!      - if `t` is a positive integral_value then $(x \le \mbox{prev}(y, t)$;
