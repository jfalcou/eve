...
namespace eve
{
  ...
  struct upward_
  {
    template<typename D> static constexpr auto combine( D const& ) noexcept =delete;
     static const  auto value = _MM_FROUND_TO_POS_INF | _MM_FROUND_NO_EXC;
  };

  struct downward_
  {
    template<typename D> static constexpr auto combine( D const& ) noexcept =delete;
     static const auto value = _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC;
  };

  struct to_nearest_
  {
    template<typename D> static constexpr auto combine( D const& ) noexcept =delete;
    static const auto value = _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC;
  };

  struct toward_zero_
  {
    template<typename D> static constexpr auto combine( D const& ) noexcept =delete;
    static const auto value = _MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC;
  };
