...
namespace eve::detail
{
  ...
  template<typename T> struct is_scalar_value<eve::logical<T>> : std::is_arithmetic<T>
  {
  };

  template<typename Type>
  requires(rebindable<Type> && !detail::is_wide<Type>::value) struct is_scalar_value<Type>
  {
...
namespace eve
{
  ...
  concept real_scalar_value =
      scalar_value<T> &&std::same_as<detail::value_type_t<T>, element_type_t<T>>;
  template<typename T>
  concept floating_real_scalar_value =
      real_scalar_value<T> &&std::floating_point<detail::value_type_t<T>>;
  template<typename T>
  concept integral_real_scalar_value =
      real_scalar_value<T> &&std::integral<detail::value_type_t<T>>;
}
